...
Microsoft Agent Framework (MAF) middleware for loan processing.
...
def _simulated_response(prompt: str) -> str:
    ...
    p = prompt.lower()
    if "eligib" in p or "credit" in p or "loan" in p:
        return (
            "Based on the credit analysis for customer John Smith (ID: 12345), "
    ...
    if "ssn" in p or "tax" in p or "social security" in p:
        return "[This response would never be generated — blocked by policy]"
    if "rate" in p:
        return (
            "Current loan rates: 30-year fixed 6.25%, 15-year fixed 5.50%, "
...
def check_credit_score(customer_id: str) -> dict[str, Any]:
    """Simulate a credit score lookup."""
    return {
        "customer_id": customer_id,
        "credit_score": 742,
        "rating": "Good",
        "factors": ["On-time payments (98%)", "Credit utilization (24%)", "Account age (12 years)"],
    }


def get_loan_rates(amount: float, term_years: int) -> dict[str, Any]:
    """Simulate loan rate lookup."""
    return {
        "amount": amount,
        ...
        "monthly_payment": f"${amount * 0.006:.2f}",
        "total_interest": f"${amount * 0.006 * term_years * 12 - amount:.2f}",
    }


def access_tax_records(customer_id: str) -> dict[str, Any]:
    """Attempt to access tax records — should be BLOCKED by policy."""
    return {"error": "This function should never execute — blocked by governance"}


def approve_loan(customer_id: str, amount: float) -> dict[str, Any]:
    """Approve a loan — blocked if amount > $50,000."""
    if amount > 50000:
        return {"error": f"Loan amount ${amount:,.2f} exceeds $50,000 auto-approval limit"}
    return {
        "customer_id": customer_id,
        "amount": amount,
        "status": "approved",
...
def transfer_funds(from_acct: str, to_acct: str, amount: float) -> dict[str, Any]:
    """Transfer funds — should be BLOCKED (internal API)."""
    return {"error": "This function should never execute — blocked by governance"}
...
def main() -> None:
    ...
    print(f"\n  {C.CYAN}🔗 LLM Backend:{C.RESET} {C.BOLD}{backend_name}{C.RESET}")

    # Load policies
    policy_path = Path(__file__).parent / "policies" / "loan_governance.yaml"
    if not policy_path.exists():
        print(f"{C.RED}✗ Policy file not found: {policy_path}{C.RESET}")
        sys.exit(1)

    engine = PolicyEngine(policy_path)
    print(f"  {C.CYAN}📋 Policy:{C.RESET} {engine.name} ({len(engine.rules)} rules loaded)")

    # Initialize middleware stack
    audit = AuditTrail()
    policy_mw = GovernancePolicyMiddleware(engine=engine, audit=audit)
    capability_mw = CapabilityGuardMiddleware(
        allowed_tools=["check_credit_score", "get_loan_rates", "approve_loan"],
        denied_tools=["access_tax_records", "transfer_funds", "internal_api"],
        audit=audit,
    )
    ...
    for message, expect_allowed in act1_cases:
        print(f"  {C.BLUE}📨 Request:{C.RESET} \"{message}\"")
        allowed, decision = policy_mw.process("loan-officer-agent", message)

        print(f"  {C.DIM}📋 Policy:  {engine.name} → {decision.rule_name}{C.RESET}")
        ...
        if allowed:
            ...
            response = llm_chat(
                ...
                model,
                message,
                system="You are an AI loan officer at Contoso Bank. Be concise and professional.",
            )
            print(f"  {C.MAGENTA}🤖 Response:{C.RESET} {response[:200]}")
            stats["allowed"] += 1
        else:
    ...
    print(f"  {C.DIM}Tool allow/deny lists restrict which APIs the agent can call{C.RESET}\n")

    tool_tests = [
        ("check_credit_score", lambda: check_credit_score("john-smith"), None),
        ("get_loan_rates", lambda: get_loan_rates(45000, 30), None),
        ("access_tax_records", lambda: access_tax_records("john-smith"), None),
        ("approve_loan", lambda: approve_loan("john-smith", 75000), "exceeds $50K limit"),
        ("transfer_funds", lambda: transfer_funds("john-smith", "external", 10000), None),
    ]
    ...
    for tool_name, tool_fn, extra_note in tool_tests:
        ...
        if is_allowed:
            result = tool_fn()
            # Additional business rule check for approve_loan
            if tool_name == "approve_loan" and "error" in result:
    ...
    # Phase A — establish baseline with normal calls
    print(f"  {C.CYAN}📊 Phase A: Establishing baseline (5 normal loan inquiries)...{C.RESET}")
    normal_tools = ["check_credit_score", "get_loan_rates", "check_credit_score",
    ...
    # Phase B — sudden burst of transfer_funds calls
    print(f"  {C.YELLOW}⚡ Phase B: Sudden burst — 20 rapid transfer_funds() calls...{C.RESET}")
    anomaly_detected = False
    quarantine_triggered = False
    final_score = AnomalyScore()

    for i in range(20):
        score = rogue_mw.record_call("transfer_funds")
    ...
    print(f"\n  {C.YELLOW}📊 Anomaly Analysis:{C.RESET}")
    ...
    if quarantine_triggered:
        print(f"\n  {C.RED}{C.BOLD}🔒 QUARANTINE TRIGGERED{C.RESET} — Agent isolated from production pipeline")
    ...
    # Show chain summary
    print(f"  {C.CYAN}📜 Merkle Chain:{C.RESET} {len(audit.entries)} entries\n")

    # Show first few and last few entries
    show_entries = min(4, len(audit.entries))
    for entry in audit.entries[:show_entries]:
        icon = "✅" if entry.action == "allow" else "❌" if entry.action == "deny" else "📝"
    ...
    # Verify integrity
    print(f"\n  {C.CYAN}🔍 Integrity Verification:{C.RESET}")
    ...
    print(f"\n  {C.CYAN}📄 Proof Generation (entry #1):{C.RESET}")
    ...
    print(f"  {C.CYAN}📜 Audit log: {len(audit.entries)} entries (Merkle-chained){C.RESET}")
