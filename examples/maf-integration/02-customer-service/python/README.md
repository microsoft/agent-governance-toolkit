...
- **Python 3.9+**
- (Optional) `openai` and `pyyaml` packages for live LLM mode
...
```bash
cd demo/maf-scenarios/02-customer-service/python
...
### Example: Lower the refund limit to $200
...
- name: "refund_limit"
  condition:
    field: "message"
    operator: "regex"
    value: "refund.*\\$\\s*([2-9]\\d{2}|[1-9]\\d{3,})"
  action: "deny"
  priority: 90
  message: "Refunds over $200 require manager approval"
```
