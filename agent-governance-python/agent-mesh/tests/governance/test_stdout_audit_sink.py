...
class TestAuditEntryContextFields:
    ...
    def test_missing_context_fields_appear_as_null_in_stdout(self):
        ...
        assert rec["compute_driver"] is None
...
class TestAuditLogEnvAutoDetection:
    ...
    def test_sandbox_id_fallback_to_openshell(self, monkeypatch):
    ...
    def test_sandbox_id_primary_wins_over_fallback(self, monkeypatch):
        monkeypatch.setenv("SANDBOX_ID", "sb-wins")
        monkeypatch.setenv("OPENSHELL_SANDBOX_ID", "sb-loses")
        log = AuditLog()
        entry = log.log(
            event_type="test",
            agent_did="did:web:a1",
