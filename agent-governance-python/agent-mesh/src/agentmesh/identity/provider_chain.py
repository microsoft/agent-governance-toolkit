...
class ExternalJWKSProviderAdapter(IdentityProvider):
    ...
    DID), this adapter expects the token to be stashed in
    ``resolve_context`` by the handshake caller. If no token is
    available, it returns ``None`` so the chain can continue.
    """
    ...
    def set_pending_token(self, token: str) -> None:
        """Stash a token for the next resolve call."""
