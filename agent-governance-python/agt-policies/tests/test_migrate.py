...
"""Tests for cli.migrate: discovery, rendering, atomic --write with rollback."""
...
def _write_simple_governance(path: Path, rule_name: str) -> None:
    doc = {
        "rules": [
            {
                "name": rule_name,
...
def _write_dangerous_tool_governance(path: Path, rule_name: str = "deny_dangerous_tool") -> None:
    doc = {
        "rules": [
            {
                "name": rule_name,
...
def _write_policy_source(path: Path, source: str) -> None:
    path.write_text(source.strip(), encoding="utf-8")


def _write_rollback_chain_governance(path: Path, rule_name: str) -> None:
    doc = {
        "rules": [
            {
                "name": rule_name,
...
def test_write_migration_backs_up_only_chain_root_local_governance(
    ...
    _write_simple_governance(parent_governance, "deny_parent")
    _write_simple_governance(local_governance, "deny_local")
...
def test_write_chain_rolls_back_backups_when_second_move_fails(
    ...
    _write_dangerous_tool_governance(first_governance, "deny_first")
    _write_dangerous_tool_governance(second_governance, "deny_second")
...
def test_write_report_failure_warns_after_successful_migration(
    ...
    _write_dangerous_tool_governance(tmp_path / "governance.yaml")
...
def test_write_chain_rollback_restores_manifest_and_generated_bundle(
    ...
    _write_rollback_chain_governance(first_governance, "deny_first")
    _write_rollback_chain_governance(second_governance, "deny_second")
