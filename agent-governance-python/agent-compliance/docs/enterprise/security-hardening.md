...
```bash
# Scan with Trivy
trivy image ghcr.io/aaif/agent-governance-toolkit/agent-os:latest --severity HIGH,CRITICAL

# In CI/CD: fail the pipeline on critical vulnerabilities
trivy image --exit-code 1 --severity CRITICAL ghcr.io/aaif/agent-governance-toolkit/agent-os:latest
```
...
syft ghcr.io/aaif/agent-governance-toolkit/agent-os:latest -o spdx-json > agent-os-sbom.json
