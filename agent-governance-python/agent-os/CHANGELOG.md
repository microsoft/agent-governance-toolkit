...
### Changed
- **`EscalationHandler.resolve()` now waits the full `timeout_seconds` window for M-of-N quorum, instead of defaulting on the first vote** ([#3255](https://github.com/microsoft/agent-governance-toolkit/pull/3255)). Previously, with `InMemoryApprovalQueue` and a `QuorumConfig` requiring more than one approval, the first vote alone could trip `resolve()` into applying `default_action` before the remaining votes had a chance to arrive; a quorum requiring 2 approvals could resolve after just 1. `resolve()` now blocks (via a new `InMemoryApprovalQueue.wait_for_quorum()`) until either quorum is actually satisfied or `timeout_seconds` elapses. `InMemoryApprovalQueue.approve()`/`deny()` are also now quorum-aware: a stored `EscalationRequest.decision` stays `PENDING` (and the request stays in `list_pending()`) until quorum is actually satisfied, instead of finalizing, and disappearing from `list_pending()`, on the first vote regardless of quorum. `resolved_by`/`resolved_at` now attribute to whichever vote actually satisfied quorum, not whichever vote arrived first. Non-quorum escalations are unaffected. **Known limitation, unchanged by this fix:** non-`InMemoryApprovalQueue` backends (e.g. `WebhookApprovalBackend`) still cannot block, so an unmet quorum on those backends still resolves to `default_action` on the very first `resolve()` call rather than waiting, see the `resolve()` docstring. The warning log for this case no longer falsely claims `timeout_seconds` was waited.
