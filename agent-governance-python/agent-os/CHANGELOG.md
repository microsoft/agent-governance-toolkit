...
### Changed
- **`EscalationHandler.resolve()` now correctly waits for M-of-N quorum instead of resolving on the first vote** ([#3255](https://github.com/microsoft/agent-governance-toolkit/pull/3255)). `approve()`/`deny()`/`resolve()` are now quorum-aware end-to-end. Non-blocking backends (e.g. `WebhookApprovalBackend`) still can't block within a single call; see `resolve()` docstring.
