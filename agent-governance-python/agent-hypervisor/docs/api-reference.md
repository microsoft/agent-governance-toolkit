...
| `enable_blockchain_commitment` | bool | `false` | Reserved. Blockchain anchoring is not implemented in Public Preview, so this has no effect (the commitment is stored in memory only). |
