...
class TrustManager:
    ...
    def verify_peer(
        ...
        if trust_score < self.min_trust_score:
            return self._fail(
                "Trust score %.2f below minimum %.2f"
    ...
    def verify_workflow_step(
        ...
        if not self.identity:
            return self._fail(
                "No identity configured", "%s:%s" % (workflow_id, step_id)
            )

        # Check capability for step type
        capability = required_capability or "workflow:%s" % step_type
        if not self.identity.has_capability(
            capability
        ) and not self.identity.has_capability("*"):
            return self._fail(
                "Missing capability: %s" % capability, "%s:%s" % (workflow_id, step_id)
            )
        ...
        self._log_audit(
            "verify_step", "%s:%s" % (workflow_id, step_id), True, trust_score
        )
        return result
    ...
    def _get_cached(
        self, peer_did: str, peer_public_key: str
    ) -> Optional[TrustVerificationResult]:
        """Get cached result if valid (key includes DID + public key for security)."""
        cache_key = "%s:%s" % (
            peer_did,
            peer_public_key[:32] if peer_public_key else "",
        )
        with self._lock:
    ...
    def _cache(
        self, peer_did: str, peer_public_key: str, result: TrustVerificationResult
    ) -> None:
    ...
    def _fail(
        self, reason: str, peer_did: str, trust_score: float = 0.0
    ) -> TrustVerificationResult:
