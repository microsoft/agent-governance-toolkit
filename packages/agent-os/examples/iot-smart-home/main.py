        ...
        self.rules.append(
            SafetyRule(
                ...
                condition=lambda devices: (
                    any(
                        d.device_type == DeviceType.THERMOSTAT
                        and d.state == DeviceState.ON
                        and d.value
                        and d.value > 70
                        for d in devices.values()
                    )
                    and any(
                        d.device_type == DeviceType.WINDOW and d.state == DeviceState.OPEN
                        for d in devices.values()
                    )
                ),
                action="block",
    ...
    def check_safety(self, devices: dict, proposed_change: dict = None) -> tuple[bool, list[str]]:
        ...
        for rule in sorted(self.rules, key=lambda r: r.priority):
            if rule.condition(test_devices):
                violations.append(
                    {"rule": rule.rule_id, "message": rule.message, "action": rule.action}
                )
...
class SmartHomeAgent:
    ...
    def _check_rate_limit(self, user: str) -> bool:
        ...
        recent = [c for c in self.command_history if c["time"] > cutoff and c["user"] == user]
        return len(recent) < self.max_commands_per_minute

    async def execute_command(
        self, device_id: str, command: str, value: float = None, user: str = "user"
    ) -> dict:
        """
        ...
        if not self._check_rate_limit(user):
            return {"status": "blocked", "reason": "Rate limit exceeded - please wait"}
        ...
        if device.name.lower().replace(" ", "_") in self.NEVER_DISABLE and new_state in [
            DeviceState.OFF,
            "disable",
        ]:
            self.audit_log.log(
                ...
                str(new_state),
                blocked=True,
                reason="Safety device cannot be disabled",
            )
            return {"status": "blocked", "reason": "Safety devices cannot be disabled"}
        ...
        self.audit_log.log(command, device_id, user, str(old_state), str(new_state))
    ...
    async def emergency_command(self, command: str, user: str = "system") -> dict:
        ...
        self.audit_log.log(f"EMERGENCY:{command}", "all", user, reason="Emergency override")

        return {"status": "emergency_executed", "command": command, "results": results}
...
async def demo():
    ...
    devices = [
        ...
        Device("lock-1", "Front Door", DeviceType.LOCK, "Entryway", state=DeviceState.LOCKED),
        Device(
            "camera-1",
            "Living Room Camera",
            DeviceType.CAMERA,
        ...
        Device("camera-2", "Bedroom Camera", DeviceType.CAMERA, "Bedroom", requires_consent=True),
        Device(
            "smoke-1",
            "Smoke Detector",
            DeviceType.SENSOR,
        ...
        Device("stove-1", "Kitchen Stove", DeviceType.APPLIANCE, "Kitchen", state=DeviceState.OFF),
    ]
