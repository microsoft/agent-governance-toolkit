...
NL_TO_SQL_TEMPLATES = {
    r"(?i)show\s+(?:me\s+)?(?:the\s+)?top\s+(\d+)\s+customers?\s+by\s+revenue": "SELECT name, email, revenue FROM customers ORDER BY revenue DESC LIMIT {0}",
    r"(?i)(?:get|show|list)\s+all\s+(?:the\s+)?transactions?\s+from\s+(\w+)": "SELECT * FROM transactions WHERE EXTRACT(YEAR FROM created_at) = '{0}'",
    r"(?i)total\s+(?:revenue|sales)\s+by\s+(\w+)": "SELECT {0}, SUM(amount) as total FROM transactions GROUP BY {0}",
    r"(?i)count\s+(?:of\s+)?(?:all\s+)?(\w+)": "SELECT COUNT(*) FROM {0}",
    r"(?i)delete\s+inactive\s+users?\s+from\s+(?:the\s+)?last\s+(\w+)": "DELETE FROM users WHERE last_active < NOW() - INTERVAL '{0}'",
    r"(?i)drop\s+(?:the\s+)?(\w+)\s+table": "DROP TABLE {0}",
}
...
class QueryAnalyzer:
    ...
    def __init__(self):
        self.patterns = {name: re.compile(p["pattern"]) for name, p in DANGEROUS_PATTERNS.items()}
...
class AuditLogger:
    ...
    def log(self, result: QueryResult, user_id: str = "anonymous") -> str:
        ...
        entry = {
            ...
            "estimated_cost_usd": result.cost_estimate.estimated_cost_usd
    ...
    def get_stats(self, days: int = 7) -> dict:
        ...
        recent = [
            e for e in self.entries if datetime.fromisoformat(e["timestamp"]).timestamp() > cutoff
        ]
        ...
        return {
            ...
            "avg_execution_time_ms": (
                sum(e.get("execution_time_ms", 0) for e in recent) / len(recent) if recent else 0
            ),
        }
...
"""
    ...
    async def query(self, natural_language: str, user_id: str = "anonymous") -> QueryResult:
        ...
        """
        result = QueryResult(query_id=str(uuid.uuid4())[:8], natural_language=natural_language)
        ...
        if cost_estimate.estimated_rows > self.max_rows:
            result.blocked = True
            result.block_reason = (
                f"Estimated {cost_estimate.estimated_rows:,} rows exceeds limit {self.max_rows:,}"
            )
            result.action = QueryAction.COST_EXCEEDED
            result.audit_id = self.audit.log(result, user_id)
            return result
    ...
    async def execute_approved(
        self, approval_token: str, approver_id: str = "admin"
    ) -> QueryResult:
    ...
    async def _execute_query(self, result: QueryResult) -> QueryResult:
        ...
        result.execution_time_ms = (
            int((time.time() - start_time) * 1000) + 45
        )  # Add simulated DB time
        result.actual_cost_usd = (
            result.cost_estimate.estimated_cost_usd if result.cost_estimate else 0.01
        )
...
def demo():
    ...
    async def run_demo():
        agent = SQLAgent(max_cost_usd=50.0, max_rows=50000, max_execution_time_ms=30000)
