...
BLOCKED_PATTERNS = {
    "rm_rf": {"pattern": r"rm\s+-rf\s+/", "message": "Dangerous rm -rf command on root directory"},
    "chmod_777": {"pattern": r"chmod\s+777", "message": "Insecure chmod 777 permission"},
    "disable_ssl": {
...
class SecretScanner:
    ...
    def __init__(self, patterns: dict = None):
        ...
        self._compiled = {name: re.compile(p["pattern"]) for name, p in self.patterns.items()}
...
"""
    ...
    async def review_pr(
        self, pr_url: str = None, diff_content: str = None, files: dict[str, str] = None
    ) -> ReviewResult:
        """
    ...
    def format_github_comment(self, result: ReviewResult) -> str:
        ...
        lines.append(
            "🤖 *Powered by [Agent OS](https://github.com/microsoft/agent-governance-toolkit)*"
        )
