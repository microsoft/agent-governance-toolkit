...
def run_demo():
    ...
    cleanup_agent = SimulatedCrewAIAgent(name="Cleanup Bot", role="System Cleaner", kernel=kernel)

    analyst_agent = SimulatedCrewAIAgent(name="Data Analyst", role="Analytics", kernel=kernel)

    deploy_agent = SimulatedCrewAIAgent(name="Deploy Bot", role="DevOps", kernel=kernel)

    # Create the crew
    crew = SimulatedCrew(agents=[cleanup_agent, analyst_agent, deploy_agent], kernel=kernel)
