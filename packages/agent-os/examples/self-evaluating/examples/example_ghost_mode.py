...
from src.ghost_mode import BehaviorPattern, ContextShadow, GhostModeObserver, ObservationResult
...
def surfacing_callback(observation: ObservationResult):
    ...
    print("=" * 60)
    print(
        f"Confidence: {observation.confidence:.2f} ({observation.get_confidence_level().value.upper()})"
    )
...
def demo_ghost_mode_basics():
    ...
    observer = GhostModeObserver(confidence_threshold=0.7, surfacing_callback=surfacing_callback)
    ...
    observer.observe_signal(
        {
            "type": "file_change",
            "data": {"file_path": "/src/utils.py", "change_type": "modified"},
            "user_id": "user123",
        }
    )
    ...
    observer.observe_signal(
        {
            "type": "log_stream",
            "data": {"level": "ERROR", "message": "Database connection failed: timeout after 30s"},
            "user_id": "user123",
        }
    )
...
def demo_context_shadow():
    ...
    matching_patterns = shadow.query_patterns(trigger="open_expense_form", min_confidence=0.5)
...
def demo_integrated_workflow():
    ...
    observer.observe_signal(
        {
            "type": "user_action",
            "data": {"action": "open_pull_request", "sequence": ["open_pull_request"]},
            "user_id": "user456",
        }
    )
