...
def simulate_learning_phase():
    ...
    agent = SelfEvolvingAgent(
        memory_file="system_instructions.json", score_threshold=0.8, max_retries=2
    )
    ...
    print(
        f"   Wisdom database has {len(agent.memory.instructions.get('improvements', []))} lessons"
    )
...
def demonstrate_upgrade_purge():
    ...
    if redundant_count > 0:
        ...
        purge_results = manager.purge_redundant_lessons(audit_results=audit_results, verbose=True)
        ...
        print(
            f"  Efficiency gain: {(purge_results['purged_count'] / max(current_lessons, 1)) * 100:.1f}%"
        )
