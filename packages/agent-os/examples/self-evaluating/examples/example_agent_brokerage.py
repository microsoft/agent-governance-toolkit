...
def demo_task_execution():
    ...
    result = broker.execute_task(task=task, selection_strategy="best_value", verbose=True)
...
def demo_cost_optimization():
    ...
    for strategy in strategies:
        ...
        result = broker.execute_task(task=task, selection_strategy=strategy, verbose=False)
...
def demo_user_constraints():
    ...
    result = broker.execute_task(task=task, user_constraints={"max_budget": 0.02}, verbose=True)

    # Constraint 2: Speed critical (max 1000ms)
    print("\n[SCENARIO 2] Speed Critical User (max 1000ms)")
    result = broker.execute_task(task=task, user_constraints={"max_latency_ms": 1000}, verbose=True)
...
def demo_api_economy_comparison():
    ...
    for i in range(num_tasks):
        broker.execute_task(
            f"Process document_{i}.pdf", selection_strategy="best_value", verbose=False
        )

    report = broker.get_usage_report()

    print("\n[OLD WORLD: Subscription Model]")
    print("  Monthly Cost: $20.00")
    print("  Cost Per Day: $0.67 (assuming 30 days)")
    print(f"  Cost for {num_tasks} tasks: $0.67")
    print("  → User pays even if they don't use the agent!")
    ...
    print("  → User pays ONLY for what they use!")
...
def demo_agent_integration_with_doer():
    ...
    result = broker.execute_task("What is 25 * 47 + 100?", verbose=True)

    # Test with a PDF task (specialized agents' specialty)
    print("\n[TASK 2] PDF Processing")
    result = broker.execute_task("Extract text from invoice.pdf", verbose=True)
