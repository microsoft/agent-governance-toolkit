...
def demo_prioritized_context():
    ...
    context = framework.get_prioritized_context(
        query=query, global_wisdom=global_wisdom, user_id="alice", verbose=True
    )
    ...
    context = framework.get_prioritized_context(
        query=query, global_wisdom=global_wisdom, user_id="bob", verbose=True
    )
    ...
    context = framework.get_prioritized_context(
        query=query, global_wisdom=global_wisdom, user_id=None, verbose=True
    )
...
def demo_with_doer_agent():
    ...
    result = doer.run(query="What is 15 * 24 + 50?", user_id="alice", verbose=True)

    print("\n\n--- Task 2: String query for Bob (with safety warning) ---")
    result = doer.run(query="What is the length of 'hello world'?", user_id="bob", verbose=True)

    print("\n\n--- Task 3: Anonymous query (no personalization) ---")
    result = doer.run(query="What is the current time?", user_id=None, verbose=True)
