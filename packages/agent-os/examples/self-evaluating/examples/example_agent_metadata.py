...
from src.agent_metadata import AgentMetadata, AgentMetadataManager, create_default_manifest
...
def example_1_basic_manifest():
    ...
    metadata.add_constraint(
        type="access", description="No internet access outside of GitHub API", severity="high"
    )

    metadata.add_constraint(
        type="resource", description="4k token context window limit", severity="medium"
    )
...
def example_3_agent_composition():
    ...
    fetcher.set_io_contract(
        input_schema={"type": "object", "properties": {"api_url": {"type": "string"}}},
        output_schema={
            "type": "object",
            "properties": {
                "data": {"type": "array"},
    ...
    transformer.set_io_contract(
        ...
        output_schema={
            "type": "object",
            "properties": {"cleaned_data": {"type": "array"}, "summary": {"type": "object"}},
        },
    )
    ...
    print("   Input: API URL")
    print("   Output: Data array")
    print()
    print("       ↓")
    print()
    print(f"2. {transformer.name}")
    print("   Input: Data array")
    print("   Output: Cleaned data + summary")
    print()
    print("       ↓")
    print()
    print(f"3. {reporter.name}")
    print("   Input: Cleaned data + summary")
    print("   Output: Report (PDF/HTML)")
    print()
...
def example_4_trust_score_updates():
    ...
    metadata = AgentMetadata(
        agent_id="test-agent", name="Test Agent", version="1.0.0", description="Test agent"
    )

    metadata.set_trust_score(success_rate=0.80, avg_latency_ms=1000.0, total_executions=100)
