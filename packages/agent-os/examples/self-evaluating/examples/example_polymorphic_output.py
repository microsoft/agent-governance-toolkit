...
def scenario_b_ide_to_ghost_text():
    ...
    response = create_ghost_text_response(
        suggestion=code_suggestion, cursor_position={"line": 42, "column": 16}
    )
...
def scenario_c_sql_results_to_table():
    ...
    response = create_table_response(
        rows=sql_results, title="Users Table", sortable=True, filterable=True
    )
...
def scenario_f_automatic_modality_detection():
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        print("    ✅ Match!" if response.modality == test["expected"] else "    ❌ Mismatch")
