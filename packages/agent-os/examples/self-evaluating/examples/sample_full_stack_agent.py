...
from src.polymorphic_output import InputContext, PolymorphicOutputEngine
...
class FullStackAgent:
    ...
    def process_raw_signal(self, raw_signal: dict, verbose: bool = False) -> dict:
        ...
        execution_result = self.doer.run(
            query=context.query, user_id=context.user_id, verbose=False
        )
        ...
        if self.enable_polymorphic_output:
            ...
            input_context = input_context_map.get(context.signal_type, InputContext.CHAT)
        ...
        if self.enable_telemetry:
            event = TelemetryEvent(
                ...
                metadata={
                    ...
                    "output_modality": polymorphic_response.modality.value
                    if polymorphic_response
                    else None,
...
def demonstrate_batch_processing():
    ...
    signals = [
        ...
        create_signal_from_log(
            level="WARNING", message="High memory usage detected: 85%", service="worker-node-3"
        ),
    ]
