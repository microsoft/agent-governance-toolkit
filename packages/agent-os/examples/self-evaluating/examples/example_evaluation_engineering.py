...
def create_date_parsing_rubric() -> ScoringRubric:
    ...
    def correctness_evaluator(input_text: str, output_text: str, context: Any) -> float:
        ...
        if expected == "ERROR" and (
            "error" in output_clean.lower() or "invalid" in output_clean.lower()
        ):
            return 1.0
    ...
    rubric = ScoringRubric(
        name="Date Parsing Rubric", description="Evaluates date parsing on correctness and clarity"
    )
...
def create_customer_service_rubric() -> ScoringRubric:
    ...
    rubric = ScoringRubric(
        name="Customer Service Rubric", description="Correctness AND tone both matter"
    )
