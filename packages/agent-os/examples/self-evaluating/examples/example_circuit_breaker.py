...
from src.circuit_breaker import CircuitBreakerConfig, CircuitBreakerController, RolloutPhase
...
def run_scenario_1_successful_rollout():
    ...
    controller = CircuitBreakerController(
        config=config, state_file="circuit_breaker_example_1.json"
    )
    ...
    while controller.watchdog.current_phase != RolloutPhase.FULL and request_count < 200:
        ...
        if request_count % 10 == 0:
            ...
            if new_metrics:
                print(
                    f"[Request {request_count:3d}] Phase: {controller.watchdog.current_phase.name:8s} | "
    ...
    print(
        f"  Traffic Split: {status['traffic_split']['old']} old, {status['traffic_split']['new']} new"
    )

    new_metrics = status.get("new_version_metrics")
    if new_metrics:
        print("  New Version Performance:")
...
def run_scenario_2_automatic_rollback():
    ...
    controller = CircuitBreakerController(
        config=config, state_file="circuit_breaker_example_2.json"
    )
...
def run_scenario_3_gradual_advancement():
    ...
    controller = CircuitBreakerController(
        config=config, state_file="circuit_breaker_example_3.json"
    )
