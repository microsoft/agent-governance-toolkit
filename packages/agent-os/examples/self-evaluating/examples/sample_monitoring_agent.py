...
from src.ghost_mode import ContextShadow, GhostModeObserver, ObservationResult
...
class MonitoringAgent:
    ...
    def _handle_high_confidence_observation(self, observation: ObservationResult):
        ...
        event = TelemetryEvent(
            ...
            metadata={"confidence": observation.confidence, "context": observation.context},
        )
    ...
    def ingest_log(self, log_level: str, message: str, service: str = "unknown", **kwargs):
        ...
        signal = create_signal_from_log(level=log_level, message=message, service=service, **kwargs)
...
def simulate_production_logs(agent: MonitoringAgent, duration_seconds: int = 10):
    ...
    log_scenarios = [
        ...
        {
            "level": "WARNING",
            "message": "Rate limit approaching: 90% of quota",
            "service": "api-gateway",
        },
        ...
        {
            "level": "CRITICAL",
            "message": "Service unavailable - no healthy instances",
            "service": "user-api",
        },
        {
            "level": "CRITICAL",
