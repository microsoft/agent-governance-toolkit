...
from .polymorphic_output import OutputModality, PolymorphicResponse
...
class UIComponentGenerator:
    ...
    def _generate_ghost_text_component(self, response: PolymorphicResponse) -> UIComponentSpec:
        ...
        return UIComponentSpec(
            ...
            style={"color": "#888", "fontStyle": "italic", "display": "inline"},
        )
        ...
        return UIComponentSpec(
            ...
            children=[
                UIComponentSpec(
                    ...
                    props={"value": metric_value, "fontSize": "32px", "fontWeight": "bold"},
                ),
                UIComponentSpec(
    ...
    def _generate_chart_component(self, response: PolymorphicResponse) -> UIComponentSpec:
        ...
        return UIComponentSpec(
            ...
            style={"width": "100%", "height": "400px", "padding": "16px"},
        )
    ...
    def _generate_table_component(self, response: PolymorphicResponse) -> UIComponentSpec:
        ...
        if rows and isinstance(rows[0], dict):
            columns = [
                {"field": k, "headerName": k.replace("_", " ").title()} for k in rows[0].keys()
            ]
        ...
        for key, value in data.items():
            children.append(
                UIComponentSpec(
                    ...
                    props={"label": key.replace("_", " ").title(), "value": str(value)},
                )
            )

        return UIComponentSpec(
            component_type=ComponentType.CARD,
            props={"title": response.rendering_hints.get("title", "Information")},
            style={
    ...
    def _generate_list_component(self, response: PolymorphicResponse) -> UIComponentSpec:
        ...
        children = [
            UIComponentSpec(component_type="ListItem", props={"content": str(item)})
            for item in items
        ]

        return UIComponentSpec(
            component_type=ComponentType.LIST,
            props={"ordered": response.rendering_hints.get("ordered", False)},
            children=children,
        )
...
def wrap_agent_response_with_ui(
    agent_response: str, input_context: str, data_type: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    """
    from polymorphic_output import InputContext, PolymorphicOutputEngine
    ...
    poly_response = output_engine.generate_response(data=agent_response, input_context=context)
