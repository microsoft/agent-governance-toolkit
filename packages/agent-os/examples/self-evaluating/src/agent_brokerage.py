...
class AgentListing:
    ...
    def calculate_score(
        self, task: str, user_constraints: Optional[Dict[str, Any]] = None
    ) -> float:
        """
    ...
    def discover_agents(
        ...
        for agent in self.agents.values():
            ...
            if capability_filter:
                if not any(capability_filter.lower() in cap.lower() for cap in agent.capabilities):
                    continue
...
class AgentBroker:
    ...
    def select_agent(
        self, bids: List[AgentBid], selection_strategy: str = "best_value"
    ) -> Optional[AgentBid]:
        """
    ...
    def execute_task(
        ...
        if verbose:
            ...
            for i, bid in enumerate(bids[:3], 1):  # Show top 3
                print(
                    f"  {i}. {bid.agent_name}: ${bid.estimated_cost:.4f}, "
