...
class FileChangeSignalNormalizer:
    ...
    def _extract_intent(
        self, change_type: str, file_path: str, content_before: str, content_after: str
    ) -> str:
    ...
    def _assess_urgency(self, file_path: str, change_type: str, content: str) -> tuple[str, float]:
...
class LogStreamSignalNormalizer:
    ...
    def _extract_intent(self, log_level: str, message: str, error_code: Optional[str]) -> str:
    ...
    def _create_query(self, log_level: str, message: str, error_code: Optional[str]) -> str:
    ...
    def _assess_urgency(
        self, log_level: str, message: str, error_code: Optional[str]
    ) -> tuple[str, float]:
...
class AudioStreamSignalNormalizer:
    ...
    def _assess_urgency(self, transcript: str, duration: float) -> tuple[str, float]:
        ...
        # Urgent keywords
        if any(
            word in lower_transcript for word in ["urgent", "emergency", "critical", "immediately"]
        ):
            return "critical", 0.9
...
class UniversalSignalBus:
    ...
    def register_normalizer(self, signal_type: SignalType, normalizer: SignalNormalizer) -> None:
        """
    ...
    def ingest(
        self, raw_signal: Dict[str, Any], signal_type: Optional[SignalType] = None
    ) -> ContextObject:
        """
    ...
    def _detect_signal_type(self, raw_signal: Dict[str, Any]) -> SignalType:
        ...
        if "file_path" in raw_signal and "change_type" in raw_signal:
            ...
            "log_level" in raw_signal
            or "level" in raw_signal
            or "error_code" in raw_signal
            or (
                "message" in raw_signal
                and any(k in raw_signal for k in ["service", "host", "stack_trace"])
            )
        ):
    ...
    def get_history(
        self, limit: Optional[int] = None, signal_type: Optional[SignalType] = None
    ) -> List[ContextObject]:
        """
...
def create_signal_from_text(text: str, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {"text": text, "timestamp": datetime.now().isoformat(), "user_id": user_id}


def create_signal_from_file_change(
    file_path: str, change_type: str, content_before: str = "", content_after: str = "", **kwargs
) -> Dict[str, Any]:
...
def create_signal_from_log(
    level: str, message: str, error_code: Optional[str] = None, **kwargs
) -> Dict[str, Any]:
...
def create_signal_from_audio(
    transcript: str, speaker_id: Optional[str] = None, **kwargs
) -> Dict[str, Any]:
