...
class PrioritizationFramework:
    ...
    def _load_safety_corrections(self) -> List[SafetyCorrection]:
        ...
        try:
            with open(self.safety_db_file) as f:
                data = json.load(f)
    ...
    def _load_user_preferences(self) -> Dict[str, List[UserPreference]]:
        ...
        try:
            with open(self.preferences_db_file) as f:
                data = json.load(f)
    ...
    def add_safety_correction(
        ...
        for existing in self.safety_corrections:
            if existing.task_pattern == task_pattern and existing.user_id == user_id:
    ...
    def _get_recent_safety_corrections(
        ...
        for correction in self.safety_corrections:
            ...
            if overlap >= self.MIN_KEYWORD_OVERLAP or any(
                word in query_lower for word in pattern_words
            ):
    ...
    def _get_user_preferences_formatted(self, user_id: Optional[str]) -> List[str]:
        ...
        prefs = sorted(self.user_preferences[user_id], key=lambda p: p.priority, reverse=True)
    ...
    def get_prioritized_context(
        self, query: str, global_wisdom: str, user_id: Optional[str] = None, verbose: bool = False
    ) -> PrioritizedContext:
        """
    ...
    def learn_from_failure(
        self, query: str, critique: str, user_id: Optional[str] = None, verbose: bool = False
    ) -> None:
        """
        ...
        if verbose:
            print("  Added safety correction for future similar tasks")

    def learn_user_preference(
        self, user_id: str, query: str, user_feedback: str, verbose: bool = False
    ) -> None:
        """
        ...
        if "calculator" in feedback_lower and (
            "use" in feedback_lower or "always" in feedback_lower
        ):
            self.add_user_preference(
                user_id=user_id,
        ...
        if verbose:
            print("  Extracted and stored user preferences")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        recent_corrections = sum(
            1
            for c in self.safety_corrections
...
def main():
    ...
    context = framework.get_prioritized_context(
        query=query, global_wisdom=global_wisdom, user_id="user123", verbose=True
    )
