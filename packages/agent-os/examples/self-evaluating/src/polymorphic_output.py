...
class OutputModalityDetector:
    ...
    def detect_data_type(self, data: Any) -> str:
        ...
        if isinstance(data, str):
            ...
            if any(
                keyword in data
                for keyword in ["def ", "class ", "function ", "import ", "const ", "let "]
            ):
                return "code"
            return "text"
        ...
        if isinstance(data, list):
            if len(data) > 0:
                ...
                if isinstance(data[0], dict) and any(
                    k in data[0] for k in ["timestamp", "time", "date"]
                ):
                    return "time_series"
...
class PolymorphicOutputEngine:
    ...
    def generate_response(
        ...
        response = PolymorphicResponse(
            ...
            metadata={"data_type": data_type, "input_context": input_context.value},
...
def create_ghost_text_response(
    suggestion: str, cursor_position: Optional[Dict[str, int]] = None
) -> PolymorphicResponse:
...
def create_dashboard_widget_response(
    ...
    return PolymorphicResponse(
        ...
        timestamp=datetime.now().isoformat(),
        data=data,
        rendering_hints={"widget_type": "metric", "alert_level": alert_level or "normal"},
...
def create_chart_response(
    ...
    return PolymorphicResponse(
        ...
        timestamp=datetime.now().isoformat(),
        data=data,
        rendering_hints={"chart_type": chart_type, "interactive": True},
...
def create_table_response(
    ...
    return PolymorphicResponse(
        ...
        timestamp=datetime.now().isoformat(),
        data=rows,
        rendering_hints={"title": title, "sortable": sortable, "filterable": filterable},
        text_fallback=f"{title or 'Table'} with {len(rows)} rows",
    )


def create_notification_response(
    message: str, level: str = "info", action: Optional[Dict[str, str]] = None
) -> PolymorphicResponse:
    """Create a notification response for alerts."""
    data = {"message": message, "level": level, "action": action}
