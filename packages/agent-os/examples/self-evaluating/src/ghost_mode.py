...
class ContextShadow:
        ...
        if os.path.exists(self.storage_file):
            try:
                with open(self.storage_file) as f:
                    ...
                    self.patterns = {
                        k: BehaviorPattern.from_dict(v) for k, v in patterns_data.items()
                    }
    ...
    def query_patterns(
        self, trigger: Optional[str] = None, min_confidence: float = 0.5, reload: bool = False
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if not self.patterns:
            return {"total_patterns": 0, "high_confidence_patterns": 0, "average_confidence": 0.0}
        ...
        return {
            ...
            "most_frequent_pattern": max(self.patterns.values(), key=lambda p: p.frequency).name
            if self.patterns
            else None,
        }
...
class GhostModeObserver:
    ...
    def start_observing(self, poll_interval: float = 1.0) -> None:
        ...
        self.observation_thread = threading.Thread(
            target=self._observation_loop, args=(poll_interval,), daemon=True
        )
    ...
    def _analyze_user_action(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        matching_patterns = self.context_shadow.query_patterns(trigger=action, min_confidence=0.5)
        ...
        return {"observation": f"User action: {action}", "recommendation": None, "pattern": None}

    def _calculate_confidence(self, signal: Dict[str, Any], analysis: Dict[str, Any]) -> float:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        surfacing_rate = (
            self.signals_surfaced / self.signals_processed if self.signals_processed > 0 else 0.0
        )
