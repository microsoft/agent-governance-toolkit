...
from .agent import AgentTools, MemorySystem
...
class ModelUpgradeManager:
    ...
    def _load_upgrade_log(self) -> Dict[str, Any]:
        ...
        if os.path.exists(self.upgrade_log_file):
            try:
                with open(self.upgrade_log_file) as f:
                    content = f.read().strip()
                    if content:
                        return json.loads(content)
            except (OSError, json.JSONDecodeError):
                pass
        return {"upgrades": [], "current_model": None}
    ...
    def _test_scenario_with_baseline(
        self, query: str, new_model: str, baseline_instructions: str
    ) -> Tuple[str, float]:
        """
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=new_model, messages=messages, temperature=0.7
            )
...
"""
    ...
    def audit_wisdom_database(
        ...
        for i, scenario in enumerate(scenarios, 1):
            ...
            response, score = self._test_scenario_with_baseline(
                query=query, new_model=new_model, baseline_instructions=baseline_instructions
            )
            ...
            if score >= score_threshold:
                ...
                if verbose:
                    print(
                        f"  ✅ REDUNDANT: New model solves this natively (score {score:.2f} >= {score_threshold})"
                    )
                ...
                if verbose:
                    print(
                        f"  ⚠️ CRITICAL: New model still needs this lesson (score {score:.2f} < {score_threshold})"
                    )
    ...
    def purge_redundant_lessons(
        self, audit_results: Dict[str, Any], verbose: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        filtered_improvements = [
            imp for imp in original_improvements if imp.get("version") not in redundant_versions
        ]
    ...
    def _purge_from_prioritization(
        self, audit_results: Dict[str, Any], verbose: bool = False
    ) -> None:
        """
    ...
    def perform_upgrade(
        ...
        if auto_purge:
            purge_results = self.purge_redundant_lessons(
                audit_results=audit_results, verbose=verbose
            )
        else:
            if verbose:
                print(
        ...
        if verbose:
            ...
            if purge_results:
                ...
                print(
                    f"Remaining Lessons: {purge_results['remaining_count']} (specialized edge cases)"
                )
