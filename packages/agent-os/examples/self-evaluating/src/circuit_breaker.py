...
class CircuitBreakerMetrics:
    ...
    def get_metrics_for_version(
        self, version: str, window_minutes: Optional[int] = None
        ...
        version_metrics = [
            ...
            if m["version"] == version and datetime.fromisoformat(m["timestamp"]) >= cutoff_time
        ]
    ...
    def check_thresholds(self, snapshot: MetricSnapshot) -> Tuple[bool, str]:
        ...
        if snapshot.task_completion_rate < self.config.min_task_completion_rate:
            return (
                False,
                f"Task completion rate {snapshot.task_completion_rate:.2%} below threshold {self.config.min_task_completion_rate:.2%}",
            )

        if snapshot.avg_latency_ms > self.config.max_latency_ms:
            return (
                False,
                f"Latency {snapshot.avg_latency_ms:.0f}ms exceeds threshold {self.config.max_latency_ms:.0f}ms",
            )
    ...
    def clear_old_metrics(self, hours: int = 24) -> None:
        ...
        self.metrics_log = [
            m for m in self.metrics_log if datetime.fromisoformat(m["timestamp"]) >= cutoff_time
        ]
...
class CircuitBreakerWatchdog:
    ...
    def evaluate(self, verbose: bool = False) -> Dict[str, Any]:
        ...
        if verbose:
            print(
                f"  Metrics - Completion: {new_snapshot.task_completion_rate:.2%}, Latency: {new_snapshot.avg_latency_ms:.0f}ms"
            )
        ...
        if not passes:
            ...
            if verbose:
                print("  Action: ROLLBACK - Circuit breaker tripped!")
            ...
            if verbose:
                print("  Action: MAINTAIN - Full rollout stable")

        elif (
            new_snapshot.task_completion_rate >= self.config.advancement_threshold
            and new_snapshot.avg_latency_ms <= self.config.max_latency_ms * 0.9
        ):  # 10% buffer
...
class CircuitBreakerController:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "state": self.watchdog.state.value,
            "current_phase": self.watchdog.current_phase.name,
            "traffic_split": {"old": f"{old_pct:.1%}", "new": f"{new_pct:.1%}"},
            ...
            "recent_decisions": self.watchdog.decision_history[-5:]
            if self.watchdog.decision_history
            else [],
        }
