...
class AgentMetadata:
    ...
    def add_capability(
        ...
        capability = Capability(
            name=name, description=description, tags=tags or [], version=version
        )
        self.capabilities.append(capability)
        self.updated_at = datetime.now().isoformat()

    def add_constraint(self, type: str, description: str, severity: str = "high") -> None:
        """Add a constraint to the agent."""
        constraint = Constraint(type=type, description=description, severity=severity)
    ...
    def set_io_contract(
        ...
        self.io_contract = IOContract(
            input_schema=input_schema, output_schema=output_schema, examples=examples or []
        )
        self.updated_at = datetime.now().isoformat()
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentMetadata":
        ...
        # Parse constraints
        constraints = [Constraint(**con) for con in data.get("constraints", [])]
...
class AgentMetadataManager:
    ...
    def create_manifest(
        self, agent_id: str, name: str, version: str, description: str
    ) -> AgentMetadata:
        """Create a new metadata manifest."""
        metadata = AgentMetadata(
            agent_id=agent_id, name=name, version=version, description=description
        )
        self.metadata = metadata
        return metadata
        ...
        return [
            {
                ...
                "capabilities": [
                    cap.name for cap in (self.metadata.capabilities if self.metadata else [])
                ],
            }
        ]
    ...
    def validate_compatibility(self, other_manifest: AgentMetadata) -> Dict[str, Any]:
        ...
        compatibility_report = {"compatible": True, "warnings": [], "errors": []}
...
def create_default_manifest(
    agent_id: str = "self-evolving-agent", name: str = "Self-Evolving Agent", version: str = "1.0.0"
) -> AgentMetadata:
    """
    ...
    metadata.add_constraint(
        type="resource", description="Token limit of 4096 tokens per request", severity="medium"
    )

    metadata.add_constraint(
        type="security", description="Cannot execute arbitrary shell commands", severity="high"
    )
    ...
    metadata.set_io_contract(
        input_schema={
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "The user's query to process"},
                "user_id": {
                    "type": "string",
        ...
        output_schema={
            "type": "object",
            "properties": {
                "response": {"type": "string", "description": "The agent's response to the query"},
