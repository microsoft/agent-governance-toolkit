...
def test_circuit_breaker_config():
    ...
    config = CircuitBreakerConfig(
        min_task_completion_rate=0.85, max_latency_ms=2000.0, initial_phase=RolloutPhase.PROBE
    )
...
def test_metrics_tracking():
    ...
    print(
        f"✓ Metrics calculation works (completion: {snapshot.task_completion_rate:.2%}, latency: {snapshot.avg_latency_ms:.0f}ms)"
    )
...
def test_watchdog_decisions():
    ...
    assert decision["action"] == "rollback", (
        f"Expected rollback but got {decision['action']}: {decision.get('reason', 'no reason')}"
    )
...
def test_controller_full_cycle():
    ...
    try:
        ...
        for cycle in range(200):  # Enough cycles to go through all phases
            ...
            controller.record_execution(version="new", success=True, latency_ms=1200.0)
            ...
            if (cycle + 1) % 5 == 0:
                ...
                if current_phase not in phases_completed:
                    ...
                    print(
                        f"  Cycle {cycle + 1}: {decision['action'].upper()} - Phase: {current_phase}"
                    )
...
def test_traffic_splitting():
    ...
    print("✓ FULL phase traffic split: 100% (expected 100%)")

    # After rollback (0%)
    controller.watchdog.state = CircuitBreakerState.OPEN
    new_count = sum(1 for i in range(100) if controller.should_use_new_version(f"request_{i}"))
    assert new_count == 0
    print("✓ OPEN state traffic split: 0% (expected 0%)")
...
def test_metric_thresholds():
    ...
    config = CircuitBreakerConfig(min_task_completion_rate=0.85, max_latency_ms=2000.0)
    metrics = CircuitBreakerMetrics(config)
