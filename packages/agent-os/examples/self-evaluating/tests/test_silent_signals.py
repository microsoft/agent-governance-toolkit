...
def test_event_stream_signal_filtering():
    ...
    try:
        ...
        events = [
            TelemetryEvent(
                event_type="task_start", timestamp="2024-01-01T12:00:00", query="Query 1"
            ),
            TelemetryEvent(
                event_type="signal_undo",
                timestamp="2024-01-01T12:01:00",
        ...
        print("✓ Found 1 abandonment signal")
        ...
        print("✓ Found 1 acceptance signal")
...
def test_doer_agent_signal_emission():
    ...
    try:
        ...
        doer = DoerAgent(stream_file=test_stream, enable_telemetry=True)
        ...
        doer.emit_abandonment_signal(
            query="Test query", agent_response="Test response", interaction_count=3, verbose=False
        )

        doer.emit_acceptance_signal(
            query="Test query", agent_response="Test response", next_task="Next task", verbose=False
        )
        ...
        print("✓ Emitted 3 signals successfully")

        signal_events = stream.get_signal_events()
        assert len(signal_events) == 3
        print("✓ All 3 events are signal events")
...
def test_observer_signal_analysis():
    ...
    try:
        observer = ObserverAgent(checkpoint_file=test_checkpoint)
