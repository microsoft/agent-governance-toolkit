...
def test_workflow_definition():
    ...
    workflow = WorkflowDefinition(
        name="test_workflow", description="Test workflow", goal="Test goal"
    )
    ...
    invalid_workflow = WorkflowDefinition(
        name="invalid_workflow", description="Invalid workflow", goal="Test goal"
    )
...
def test_workflow_context():
    ...
    context.add_history(worker_type="test_worker", input_data="input_1", output_data="output_1")

    context.add_history(worker_type="another_worker", input_data="input_2", output_data="output_2")
...
def test_orchestrator_registration():
    ...
    workflow.add_step(
        WorkflowStep(
            step_id="step1", worker_type=WorkerType.GENERIC, description="Step 1", is_terminal=True
        ),
        is_initial=True,
    )
...
def test_simple_workflow_execution():
    ...
    workflow = WorkflowDefinition(
        name="simple_workflow", description="Simple workflow", goal="Execute one step"
    )
    ...
    result = orchestrator.execute_workflow(
        workflow_name="simple_workflow", goal="Test goal", verbose=False
    )
...
def test_multi_step_workflow():
    ...
    workflow = WorkflowDefinition(
        name="multi_step", description="Multi-step workflow", goal="Execute three steps"
    )
    ...
    workflow.add_step(
        WorkflowStep(
            step_id="step2", worker_type=WorkerType.CODER, description="Step 2", on_success="step3"
        )
    )

    workflow.add_step(
        WorkflowStep(
            step_id="step3", worker_type=WorkerType.REVIEWER, description="Step 3", is_terminal=True
        )
    )
    ...
    # Execute workflow
    result = orchestrator.execute_workflow(
        workflow_name="multi_step", goal="Test multi-step", verbose=False
    )
...
def test_workflow_with_failure():
    ...
    workflow = WorkflowDefinition(
        name="failure_test", description="Test failure handling", goal="Test failure"
    )
    ...
    # Execute workflow
    result = orchestrator.execute_workflow(
        workflow_name="failure_test", goal="Test failure path", verbose=False
    )
