...
def test_output_modality_detector():
    ...
    modality = detector.detect_modality(input_context=InputContext.IDE, data_type="code")
    ...
    modality = detector.detect_modality(input_context=InputContext.IDE, data_type="error")
    ...
    modality = detector.detect_modality(
        input_context=InputContext.MONITORING, input_signal_type="log_stream"
    )
    ...
    modality = detector.detect_modality(input_context=InputContext.CHAT, urgency=0.9)
    ...
    modality = detector.detect_modality(input_context=InputContext.CHAT, data_type="tabular")
    assert modality == OutputModality.TABLE, "Tabular data should produce table"
    print("✓ Tabular data → Table")

    # Test time series → Chart
    modality = detector.detect_modality(input_context=InputContext.CHAT, data_type="time_series")
...
def test_data_type_detection():
    ...
    data_type = detector.detect_data_type(
        [{"timestamp": "2024-01-01", "value": 100}, {"timestamp": "2024-01-02", "value": 120}]
    )
...
def test_polymorphic_output_engine():
    ...
    response = engine.generate_response(
        data="Hello, how can I help?", input_context=InputContext.CHAT
    )
    ...
    response = engine.generate_response(
        data="def calculate(): pass", input_context=InputContext.IDE
    )
    ...
    response = engine.generate_response(
        data=[{"id": 1, "name": "Alice"}, {"id": 2, "name": "Bob"}], input_context=InputContext.CHAT
    )
    ...
    response = engine.generate_response(
        data="Critical error detected", input_context=InputContext.CHAT, urgency=0.95
    )
...
def test_helper_functions():
    ...
    response = create_ghost_text_response(
        suggestion="def function():", cursor_position={"line": 10, "column": 5}
    )
    assert response.modality == OutputModality.GHOST_TEXT
    ...
    response = create_dashboard_widget_response(
        metric_name="CPU Usage", metric_value="85%", trend="up", alert_level="warning"
    )
    assert response.modality == OutputModality.DASHBOARD_WIDGET
    ...
    response = create_chart_response(
        chart_type="line", data_points=[{"x": 1, "y": 10}, {"x": 2, "y": 20}], title="Test Chart"
    )
    ...
    # Test table helper
    response = create_table_response(rows=[{"id": 1, "name": "Test"}], title="Test Table")
    assert response.modality == OutputModality.TABLE
    assert len(response.data) == 1
    print("✓ create_table_response")

    # Test notification helper
    response = create_notification_response(message="Test notification", level="info")
    assert response.modality == OutputModality.NOTIFICATION
...
def test_ui_component_generator():
    ...
    response = PolymorphicResponse(
        modality=OutputModality.TEXT, timestamp="2024-01-01T00:00:00", data="Hello world"
    )
    ...
    response = create_dashboard_widget_response(
        metric_name="Latency", metric_value="100ms", alert_level="normal"
    )
    component = generator.generate_component(response)
    ...
    response = create_chart_response(chart_type="bar", data_points=[{"x": 1, "y": 10}])
    component = generator.generate_component(response)
    ...
    response = create_table_response(rows=[{"id": 1, "name": "Test"}])
    component = generator.generate_component(response)
...
def test_component_serialization():
    ...
    component = UIComponentSpec(
        ...
        children=[UIComponentSpec(component_type="Text", props={"content": "Hello"})],
    )
...
def test_end_to_end_scenarios():
    ...
    poly_response = output_engine.generate_response(
        data="def calculate_total():", input_context=InputContext.IDE
    )
    ui_component = ui_engine.render(poly_response)
