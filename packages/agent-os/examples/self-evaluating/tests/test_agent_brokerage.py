...
class TestAgentPricing(unittest.TestCase):
    ...
    def test_per_execution_pricing(self):
        ...
        pricing = AgentPricing(model=PricingModel.PER_EXECUTION, base_price=0.01)
...
class TestAgentBroker(unittest.TestCase):
    ...
    def test_select_agent_fastest(self):
        ...
        self.assertEqual(selected.estimated_latency_ms, min(b.estimated_latency_ms for b in bids))

    def test_execute_task(self):
        """Test executing a task."""
        result = self.broker.execute_task("Extract text from document.pdf", verbose=False)
    ...
    def test_execute_task_with_constraints(self):
        ...
        result = self.broker.execute_task(
            "Extract text from document.pdf", user_constraints={"max_budget": 0.02}, verbose=False
        )
