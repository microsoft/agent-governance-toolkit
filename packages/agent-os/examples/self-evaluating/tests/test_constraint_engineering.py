...
from src.constraint_engine import ConstraintEngine, ViolationSeverity, create_default_engine
...
def test_sql_injection_rule():
    ...
    plan = {"action_type": "sql_query", "action_data": {"query": "DROP TABLE users"}}
    ...
    plan = {
        ...
        "action_data": {"query": "SELECT * FROM users WHERE id = 1; DROP TABLE users"},
    }
    violations = rule.validate(plan)
    ...
    plan = {"action_type": "sql_query", "action_data": {"query": "DELETE FROM users WHERE 1=1"}}
    violations = rule.validate(plan)
...
def test_file_operation_rule():
    ...
    plan = {"action_type": "file_operation", "action_data": {"command": "rm -rf /", "path": "/"}}
    violations = rule.validate(plan)
    ...
    plan = {
        ...
        "action_data": {"command": "rm config.txt", "path": "/etc/config.txt"},
    }
    violations = rule.validate(plan)
    ...
    plan = {
        "action_type": "file_operation",
        "action_data": {"command": "rm temp.txt", "path": "/home/user/temp.txt"},
    }
    violations = rule.validate(plan)
...
def test_cost_limit_rule():
    ...
    # Test 1: Over limit
    plan = {"action_type": "api_call", "action_data": {"estimated_cost": 0.10}}
    violations = rule.validate(plan)
    ...
    plan = {"action_type": "api_call", "action_data": {"estimated_cost": 0.045}}
    violations = rule.validate(plan)
    ...
    plan = {"action_type": "api_call", "action_data": {"estimated_cost": 0.01}}
    violations = rule.validate(plan)
...
def test_email_domain_rule():
    ...
    plan = {"action_type": "email", "action_data": {"recipient": "user@untrusted.com"}}
    violations = rule.validate(plan)
    ...
    plan = {"action_type": "email", "action_data": {"recipient": "user@example.com"}}
    violations = rule.validate(plan)
    assert len(violations) == 0, "Should allow approved domain"
    print("✓ Allowed: Email to example.com")

    # Test 3: Another approved domain
    plan = {"action_type": "email", "action_data": {"recipient": "admin@company.com"}}
    violations = rule.validate(plan)
    assert len(violations) == 0, "Should allow approved domain"
...
def test_rate_limit_rule():
    ...
    plan = {"action_type": "api_call", "action_data": {"current_rate": 15}}
    violations = rule.validate(plan)
    ...
    plan = {"action_type": "api_call", "action_data": {"current_rate": 5}}
    violations = rule.validate(plan)
...
def test_intercept_and_validate():
    ...
    plan = {
        "action_type": "sql_query",
        "action_data": {"query": "SELECT * FROM users WHERE id = ?", "estimated_cost": 0.01},
    }
...
def test_custom_rules():
    ...
    class CustomAPIRule(constraint_engine.ConstraintRule):
        def __init__(self):
            super().__init__(
                name="custom_api_restriction", description="Custom rule for API restrictions"
            )
        ...
        def validate(self, plan):
            ...
            if plan.get("action_type") == "api_call":
                ...
                if api_name == "forbidden_api":
                    violations.append(
                        constraint_engine.ConstraintViolation(
                            rule_name=self.name,
    ...
    # Test custom rule
    plan = {"action_type": "api_call", "action_data": {"api_name": "forbidden_api"}}
    result = engine.validate_plan(plan, verbose=False)
    assert not result.approved, "Should block forbidden API"
    print("✓ Custom rule blocked forbidden API")

    # Test allowed API
    plan = {"action_type": "api_call", "action_data": {"api_name": "allowed_api"}}
    result = engine.validate_plan(plan, verbose=False)
