...
def test_evaluation_dataset():
    ...
    # Create dataset
    dataset = EvaluationDataset(name="Test Dataset", description="A test dataset")
    ...
    # Add cases
    dataset.add_case(
        id="case_001", input="Test input 1", expected_output="Test output 1", tags=["test", "basic"]
    )
...
def test_dataset_persistence():
    ...
    try:
        ...
        dataset.add_case(id="case_001", input="Input 1", expected_output="Output 1", tags=["test"])
...
def test_scoring_rubric():
    ...
    # Create rubric
    rubric = ScoringRubric(name="Test Rubric", description="A test rubric")
    ...
    # Test evaluation
    result = rubric.evaluate("test input", "test output", {"expected_output": "test output"})
...
def test_exact_match_evaluator():
    ...
    score = exact_match_evaluator(
        "input", "expected output", {"expected_output": "expected output"}
    )
    assert score == 1.0

    # No match
    score = exact_match_evaluator(
        "input", "different output", {"expected_output": "expected output"}
    )
    assert score == 0.0

    # No expected output
    score = exact_match_evaluator("input", "any output", {})
    assert score == 0.5
...
def test_evaluation_runner():
    ...
    dataset = EvaluationDataset("Test", "Test dataset")
    dataset.add_case(id="case_001", input="2+2", expected_output="4", tags=["math"])
    dataset.add_case(id="case_002", input="3+3", expected_output="6", tags=["math"])
...
def test_evaluation_runner_with_perfect_score():
    ...
    rubric.add_criteria(
        dimension="correctness", weight=1.0, description="Correct", evaluator=exact_match_evaluator
    )
    rubric.set_pass_threshold(0.9)
