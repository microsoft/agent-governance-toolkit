...
def test_ghost_mode_observer_basic():
    ...
    observer = GhostModeObserver(confidence_threshold=0.7, surfacing_callback=test_callback)
    ...
    observer.observe_signal(
        {"type": "log_stream", "data": {"level": "ERROR", "message": "Test error"}}
    )
...
def test_ghost_mode_dry_run():
    ...
    signals = [
        {"type": "file_change", "data": {"file_path": "/src/app.py", "change_type": "modified"}},
        {
            "type": "file_change",
            "data": {"file_path": "/config/password.yaml", "change_type": "modified"},
        },
        {"type": "log_stream", "data": {"level": "INFO", "message": "Server started"}},
        {"type": "log_stream", "data": {"level": "CRITICAL", "message": "System failure"}},
    ]
...
def test_ghost_mode_confidence_threshold():
    ...
    observer = GhostModeObserver(confidence_threshold=0.5, surfacing_callback=count_callback)
    ...
    assert (
        stats["signals_surfaced"] <= stats["signals_processed"]
    )  # Can't surface more than processed
    assert surfaced_count[0] == stats["signals_surfaced"]  # Callback was called correctly

    print(
        f"✓ Confidence threshold works (surfaced {stats['signals_surfaced']}/{stats['signals_processed']})"
    )
...
def test_ghost_mode_file_analysis():
    ...
    # Test normal file
    observer.observe_signal(
        {"type": "file_change", "data": {"file_path": "/src/utils.py", "change_type": "modified"}}
    )
...
def test_ghost_mode_log_analysis():
    ...
    for log_data in log_signals:
        observer.observe_signal({"type": "log_stream", "data": log_data})
    ...
    critical_obs = [
        ...
        if "critical" in o.observation.lower() or "shutdown" in o.observation.lower()
    ]
