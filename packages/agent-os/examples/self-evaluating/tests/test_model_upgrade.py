...
def test_model_upgrade_manager_initialization():
    ...
    try:
        ...
        assert len(manager.wisdom.instructions.get("improvements", [])) == 3, (
            "Should load 3 improvements"
        )
...
def test_extract_failure_scenarios():
    ...
    try:
        ...
        # Initialize manager
        manager = ModelUpgradeManager(wisdom_file=temp_wisdom_file, enable_prioritization=False)
...
def test_purge_redundant_lessons():
    ...
    try:
        ...
        purge_results = manager.purge_redundant_lessons(
            audit_results=mock_audit_results, verbose=True
        )

        # Verify purge
        assert purge_results["purged_count"] == 2, (
            f"Expected 2 purged, got {purge_results['purged_count']}"
        )
        assert purge_results["remaining_count"] == 1, (
            f"Expected 1 remaining, got {purge_results['remaining_count']}"
        )
