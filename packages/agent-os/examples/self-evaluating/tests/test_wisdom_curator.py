...
def test_design_proposal():
    ...
    curator = WisdomCurator(
        review_queue_file=review_queue_file, design_proposals_file=design_proposals_file
    )
...
def test_strategic_sampling():
    ...
    sample_item = curator.create_strategic_sample(
        query="What is 10 + 20?", agent_response="The result is 30", metadata={"user_id": "user123"}
    )
...
def test_policy_review_workflow():
    ...
    approved = curator.approve_review(
        policy_review.review_id, reviewer_notes="False positive - safe to apply"
    )
    ...
    rejected = curator.reject_review(
        policy_review_2.review_id, reviewer_notes="Harmful pattern - will cause silent failures"
    )
...
def test_review_queue_management():
    ...
    curator.create_policy_review(
        proposed_wisdom=bad_wisdom, current_wisdom="Check everything", critique="Missing checks"
    )
