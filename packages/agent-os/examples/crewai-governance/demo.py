...
def _log(level: str, msg: str):
    colour = {"INFO": C.CY, "WARN": C.Y, "BLOCK": C.R + C.BD, "ALLOW": C.G, "AUDIT": C.M}.get(
        level, C.W
    )
...
class AuditLog:
    ...
    def print_report(self):
        ...
        for i, e in enumerate(self.entries, 1):
            ...
            print(
                f"  {C.W}{i:>3}.{C.RS} {icon} {colour}{e.decision:7s}{C.RS} "
...
class RolePolicyEngine:
    ...
    def check(self, role: str, tool: str, params: str) -> PolicyResult:
        ...
        if tool not in allowed_tools:
            return PolicyResult(False, f"Tool '{tool}' not in allowlist for role '{role}'")
        ...
        if max_actions:
            ...
            if count >= max_actions:
                return PolicyResult(False, f"Role '{role}' exceeded action budget ({max_actions})")
...
class GovernedKernel:
    ...
    def execute(self, *, agent_id: str, role: str, tool: str, params: str) -> str:
...
class MockCrewAgent:
    ...
    def run(self, tasks: List[tuple]):
        ...
        for tool, params in tasks:
            try:
                self.kernel.execute(
                    agent_id=self.agent_id, role=self.role, tool=tool, params=params
                )
