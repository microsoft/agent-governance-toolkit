...
class PharmaComplianceDemo:
    ...
    def run_analysis(self, verbose: bool = True) -> dict:
        ...
        if verbose:
            ...
            print(
                f"Total pages: ~{sum(r.page_count for r in self.lab_reports) + self.ind_draft.page_count:,}"
            )
        ...
        results = {
            ...
            "by_severity": {
                ...
                "high": len(
                    [c for c in contradictions if c.severity == ContradictionSeverity.HIGH]
                ),
                "medium": len(
                    [c for c in contradictions if c.severity == ContradictionSeverity.MEDIUM]
                ),
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["full", "contradiction_only"], default="full", help="Analysis mode"
    )
