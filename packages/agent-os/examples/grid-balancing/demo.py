...
class ForecastAgent:
    ...
    def predict_solar_output(self, hours: int = 4) -> list[float]:
        ...
        return [
            base_curve[(hour + h) % len(base_curve)]
            * self.der.capacity_kwh
...
class TraderAgent:
    ...
    def submit_bid(self, price: float, grid_requirement: float):
        ...
        available = min(self.der.available_energy, self.der.max_discharge_rate)
...
class DispatchAgent:
    ...
    def on_dispatch(self, msg: GridMessage) -> Optional[GridMessage]:
        ...
        # 5. Send ACK
        ack = GridMessage(type=MessageType.ACK, sender=self.agent_id, payload=dispatch_record).sign(
            self.agent_id
        )
...
class GridOperator:
    ...
    def broadcast_price_signal(self, price: float, required_kw: float):
        ...
        msg = GridMessage(
            ...
            payload={"price_per_kwh": price, "required_kw": required_kw, "deadline_seconds": 5},
        ).sign("GRID_OPERATOR")
    ...
    def __init__(self, num_ders: int = 100):
        ...
        for i in range(num_ders):
            ...
            der = DER(
                ...
                type=der_type,
                capacity_kwh=random.uniform(5, 20)
                if der_type != DERType.EV
...
def main():
    ...
    parser.add_argument(
        "--scenario",
        choices=["price_spike", "solar_surplus", "emergency"],
        default="price_spike",
