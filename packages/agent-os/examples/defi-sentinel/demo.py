...
class Transaction:
    ...
    hash: str = field(
        default_factory=lambda: "0x" + hashlib.sha256(str(uuid4()).encode()).hexdigest()[:64]
    )
    from_addr: str = ""
    to_addr: str = ""
    value_wei: int = 0
    ...
    def _generate_call_trace(self, tx: Transaction) -> list[str]:
        ...
        if tx.is_attack:
            if tx.attack_type == AttackType.REENTRANCY:
                ...
                return [
                    "CALL Lender.flashLoan(100000000 USDC)",
                    "  → CALL DEX.swap(USDC → ETH)",
                    "  → CALL Oracle.manipulatePrice()",
                    "  → CALL Vault.liquidate(victims)",
                    "  → CALL DEX.swap(ETH → USDC)",
                    "  → CALL Lender.repay(100000000 USDC)",
                    "  → PROFIT: $50,000,000",
                ]
            elif tx.attack_type == AttackType.ORACLE_MANIPULATION:
                return [
                    "CALL Oracle.updatePrice(ETH, $10)",  # Way below market
                    "  → Vault.positions updated",
                    "  → 1000 positions now liquidatable",
                    "  → LOSS: $5,000,000 in bad liquidations",
                ]

        return [f"CALL {tx.to_addr}.transfer({tx.value_wei} wei)"]

    def _detect_attack(
        self, tx: Transaction, call_trace: list[str]
    ) -> tuple[bool, Optional[AttackType], float]:
...
class DeFiSentinel:
    ...
    def run_all_attacks(self) -> dict:
        ...
        print(
            f"Average response time: {sum(r['response_time_ms'] for r in results) / len(results):.1f}ms"
        )
...
def main():
    ...
    parser.add_argument(
        "--attack",
        choices=["reentrancy", "flash_loan", "oracle", "governance", "sandwich", "all"],
        default="reentrancy",
