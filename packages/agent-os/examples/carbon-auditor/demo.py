...
class CMVK:
    ...
    def verify(self, claim: Claim, observation: Observation) -> VerificationResult:
        ...
        audit_trail.append(
            f"NDVI drift: {ndvi_drift:.2%} (claimed: {claim.claimed_ndvi}, observed: {observation.observed_ndvi})"
        )

        # Calculate carbon drift
        carbon_drift = (
            abs(claim.claimed_carbon_tonnes - observation.observed_carbon_tonnes)
            / claim.claimed_carbon_tonnes
        )
        audit_trail.append(
            f"Carbon drift: {carbon_drift:.2%} (claimed: {claim.claimed_carbon_tonnes}, observed: {observation.observed_carbon_tonnes})"
        )
...
class GeoAgent:
    ...
    def fetch_observation(
        self, location: tuple[float, float], is_fraud: bool = False
    ) -> Observation:
