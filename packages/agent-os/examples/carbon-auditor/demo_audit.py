...
from agents import AuditorAgent, ClaimsAgent, GeoAgent

# Import new amb-core 0.2.0 features
from amb_core import TraceContext
...
def run_demo(simulate_fraud: bool = True):
    ...
    print(
        f"    Simulation mode: {'DEFORESTATION (Fraud)' if simulate_fraud else 'HEALTHY FOREST (Verified)'}"
    )
    ...
    print("    Verification threshold: 0.15")
    print("    Uses: cmvk.verify_embeddings(metric='euclidean', threshold_profile='carbon')")
    print("    NEW: Audit trail enabled, explainability enabled")
    ...
    if result["status"] == "FRAUD":
        ...
        print("!!")
        print("!!  The MATH has spoken (via cmvk):")
        print(
            f"!!  - Claimed NDVI ({details['ndvi_claimed']}) vs Observed NDVI ({details['ndvi_observed']:.2f})"
        )
        print(f"!!  - This represents a {details['ndvi_percent_difference']:.0f}% discrepancy")
        print("!!")
        print("!!  RECOMMENDATION: Suspend credit issuance pending investigation")
        print("!!")
        print("!" * 70)
        print("\n")
        ...
        print("✓" * 70)
        print("  Project claims align with satellite observations.")
        print("  Carbon credits may be issued.")
        print("✓" * 70)
        print("\n")
    ...
    if stats.get("audit_trail_entries"):
        ...
        if audit_trail:
            ...
            for entry in audit_trail[-3:]:  # Show last 3
                print(
                    f"    - {entry['timestamp']}: drift={entry['drift_score']:.4f}, metric={entry['metric_used']}"
                )
...
Examples:
  ...
  python demo_audit.py --both       # Run both scenarios
    ...
    parser.add_argument(
        "--verified", action="store_true", help="Simulate healthy forest (verified scenario)"
    )

    parser.add_argument("--both", action="store_true", help="Run both fraud and verified scenarios")
