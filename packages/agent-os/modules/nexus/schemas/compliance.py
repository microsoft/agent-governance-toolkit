...
class ComplianceRecord(BaseModel):
    ...
    event_type: Literal[
        ...
        "policy_signed",
        "data_accessed",
    ] = Field(..., description="Type of compliance event")
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When the event occurred"
    )

    # Participants
    requester_did: Optional[str] = Field(default=None, description="DID of the requesting agent")
    provider_did: Optional[str] = Field(default=None, description="DID of the providing agent")
    organization_id: Optional[str] = Field(
        default=None, description="Organization that owns the agent(s)"
    )
    ...
    data_classification: Optional[Literal["public", "internal", "confidential", "pii"]] = Field(
        default=None, description="Classification of data involved"
    )
    duration_ms: Optional[int] = Field(default=None, ge=0, description="Duration of operation")
    outcome: Optional[str] = Field(default=None, description="Outcome of the operation")

    # Policy compliance
    policy_signed: Optional[str] = Field(
        default=None, description="Hash of signed data handling policy"
    )
    retention_policy: Optional[str] = Field(
        default=None, description="Data retention policy applied"
    )

    # Cryptographic attestation
    signature: Optional[str] = Field(default=None, description="Ed25519 signature of the record")
    previous_event_hash: Optional[str] = Field(
        default=None, description="Hash of previous event (blockchain-style chaining)"
    )
...
class ComplianceAuditReport(BaseModel):
    ...
    start_date: datetime = Field(..., description="Audit period start")
    end_date: datetime = Field(..., description="Audit period end")

    # Summary
    executive_summary: str = Field(..., description="High-level summary of compliance status")
    stats: ComplianceStats = Field(..., description="Aggregated statistics")

    # Events
    events: list[ComplianceRecord] = Field(
        default_factory=list, description="All compliance events in period"
    )
    ...
    warnings: list[dict[str, Any]] = Field(default_factory=list, description="Compliance warnings")
    recommendations: list[str] = Field(
        default_factory=list, description="Recommendations for improvement"
    )

    # Attestation
    report_hash: Optional[str] = Field(default=None, description="Hash of complete report")
    nexus_signature: Optional[str] = Field(
        default=None, description="Nexus signature attesting to report"
    )
...
class DataHandlingAudit(BaseModel):
    ...
    # Policy details
    policy_hash: str = Field(..., description="Hash of the signed data handling policy")
    ...
    # Compliance checks
    policy_signed: bool = Field(..., description="Whether policy was signed before data access")
    policy_signed_at: Optional[datetime] = None
    signer_did: str

    # Verification
    data_deleted_on_schedule: Optional[bool] = Field(
        default=None, description="Whether data was deleted per retention policy"
    )
