...
class JobReceipt(BaseModel):
    ...
    task_id: str = Field(..., description="ID of the task being receipted")
    ...
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="When the receipt was created"
    )
...
class JobCompletionReceipt(JobReceipt):
    ...
    duration_ms: int = Field(..., ge=0, description="Duration of job execution in milliseconds")
    output_hash: Optional[str] = Field(
        default=None, description="SHA-256 hash of the output (if any)"
    )
    error_code: Optional[str] = Field(default=None, description="Error code if outcome is failure")
    error_message: Optional[str] = Field(
        default=None, description="Error message if outcome is failure"
    )

    # SCAK validation
    scak_validated: bool = Field(default=False, description="Whether output was validated by SCAK")
    scak_drift_score: Optional[float] = Field(
        default=None, ge=0.0, le=1.0, description="SCAK drift score (0=no drift, 1=complete drift)"
    )
    scak_threshold: Optional[float] = Field(
        default=None, description="Drift threshold used for validation"
    )
...
class SignedReceipt(BaseModel):
    ...
    receipt_hash: str = Field(..., description="SHA-256 hash of the receipt")

    # Signatures
    requester_signature: Optional[str] = Field(
        default=None, description="Ed25519 signature from requester"
    )
    requester_signed_at: Optional[datetime] = Field(
        default=None, description="When requester signed"
    )
    provider_signature: Optional[str] = Field(
        default=None, description="Ed25519 signature from provider"
    )
    provider_signed_at: Optional[datetime] = Field(default=None, description="When provider signed")

    # Nexus attestation
    nexus_witnessed: bool = Field(
        default=False, description="Whether Nexus has witnessed this receipt"
    )
    nexus_signature: Optional[str] = Field(
        default=None, description="Nexus signature witnessing the receipt"
    )
    nexus_witnessed_at: Optional[datetime] = Field(default=None, description="When Nexus witnessed")
...
class DisputeReceipt(BaseModel):
    ...
    requester_logs_hash: Optional[str] = Field(
        default=None, description="Hash of requester's flight recorder logs"
    )
    provider_logs_hash: Optional[str] = Field(
        default=None, description="Hash of provider's flight recorder logs"
    )

    # Resolution
    resolved: bool = Field(default=False, description="Whether dispute has been resolved")
    ...
    arbiter_decision: Optional[str] = Field(
        default=None, description="Arbiter's decision explanation"
    )
    resolved_at: Optional[datetime] = Field(default=None, description="When dispute was resolved")

    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="When dispute was raised"
    )
