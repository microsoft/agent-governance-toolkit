...
class DataHandlingPolicy(BaseModel):
    ...
    allow_persistence: bool = Field(
        default=False, description="Whether data can be persisted to storage"
    )
    allow_training: bool = Field(
        default=False, description="Whether data can be used for model training"
    )
    allow_forwarding: bool = Field(
        default=False, description="Whether data can be forwarded to other agents"
    )
    allow_logging: bool = Field(
        default=True, description="Whether metadata can be logged (not content)"
    )

    # Audit
    audit_required: bool = Field(
        default=True, description="Whether all data access must be audited"
    )

    # Encryption
    require_encryption_at_rest: bool = Field(
        default=True, description="Whether data must be encrypted when stored"
    )
...
class DMZRequest(BaseModel):
    ...
    request_id: str = Field(default_factory=lambda: f"dmz_{uuid.uuid4().hex[:16]}")
    ...
    data_hash: str = Field(..., description="SHA-256 hash of the actual data")
    data_size_bytes: int = Field(..., ge=0, description="Size of the data in bytes")
    data_classification: Literal["public", "internal", "confidential", "pii"] = Field(
        default="internal", description="Classification level of the data"
    )
    ...
    encrypted_data_location: Optional[str] = Field(
        default=None, description="URL/URI where encrypted data is stored"
    )
    encryption_algorithm: str = Field(
        default="AES-256-GCM", description="Encryption algorithm used"
    )
