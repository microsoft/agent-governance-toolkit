...
class IATPInsufficientTrustException(NexusError):
    ...
    def __init__(
        ...
        self.improvement_url = (
            improvement_url or f"https://nexus.agent-os.dev/reputation/{peer_did}"
        )
...
class EscrowExpiredError(EscrowError):
    ...
    def __init__(self, escrow_id: str, expired_at: str):
        self.expired_at = expired_at
        super().__init__(f"Escrow {escrow_id} expired at {expired_at}", escrow_id=escrow_id)
...
class DisputeAlreadyResolvedError(DisputeError):
    ...
    def __init__(self, dispute_id: str):
        super().__init__(f"Dispute {dispute_id} has already been resolved", dispute_id=dispute_id)
...
class DisputeEvidenceError(DisputeError):
    ...
    def __init__(self, dispute_id: str, issue: str):
        self.issue = issue
        super().__init__(f"Evidence issue for dispute {dispute_id}: {issue}", dispute_id=dispute_id)
...
class AgentAlreadyRegisteredError(RegistryError):
    ...
    def __init__(self, agent_did: str):
        super().__init__(f"Agent already registered: {agent_did}", agent_did=agent_did)
...
class AgentNotFoundError(RegistryError):
    ...
    def __init__(self, agent_did: str):
        super().__init__(f"Agent not found in registry: {agent_did}", agent_did=agent_did)
...
class InvalidManifestError(RegistryError):
    ...
    def __init__(self, agent_did: str, validation_errors: list[str]):
        ...
        super().__init__(
            f"Invalid manifest for {agent_did}: {', '.join(validation_errors)}", agent_did=agent_did
        )
...
class PolicyNotSignedError(DMZError):
    ...
    def __init__(self, agent_did: str, policy_hash: str):
        ...
        super().__init__(f"Agent {agent_did} has not signed policy {policy_hash}")
...
class DataClassificationError(DMZError):
    ...
    def __init__(self, classification: str, reason: str):
        ...
        self.reason = reason
        super().__init__(f"Data classification '{classification}' blocked: {reason}")
