...
import atr
from atr import DockerExecutor, ExecutorError, LocalExecutor
...
def test_execute_tool_with_list_operations():
    ...
    result = atr.execute_tool(
        "test_reverse", {"items": [1, 2, 3, 4]}, executor=executor, timeout=10
    )
...
def test_execute_tool_sandboxed_vs_local():
    ...
    # Execute in Docker
    result_docker = atr.execute_tool(
        "test_calc", {"x": 5, "y": 3}, executor=docker_executor, timeout=10
    )
