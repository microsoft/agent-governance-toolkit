...
async def main():
    ...
    async with MessageBus() as bus:
        ...
        await bus.publish(
            "agent.thoughts",
            {"agent_id": "agent-1", "thought": "I am analyzing the user request..."},
        )

        await bus.publish(
            "agent.thoughts", {"agent_id": "agent-1", "thought": "I need to gather more context..."}
        )
        ...
        async def verification_handler(msg: Message):
            ...
            # Simulate approval
            await bus.reply(msg, {"approved": True, "reviewer": "human-supervisor"})
        ...
        response = await bus.request(
            "agent.verification",
            {"agent_id": "agent-1", "action": "delete_database", "requires_approval": True},
            timeout=5.0,
        )
        ...
        msg_id = await bus.publish(
            "agent.actions", {"action": "log_event", "priority": "low"}, wait_for_confirmation=False
        )
