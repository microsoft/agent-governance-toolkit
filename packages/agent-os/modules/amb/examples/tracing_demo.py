...
from amb_core import Message, MessageBus, get_tracer, initialize_tracing
...
async def agent_a(bus: MessageBus, tracer):
    ...
    with tracer.start_as_current_span("agent-a-thinking") as span:
        ...
        await bus.publish("agent.thoughts", {"thought": "I need to process data X"})
...
async def agent_b(bus: MessageBus):
    ...
    async def handle_thought(msg: Message):
        ...
        await bus.publish(
            "agent.tool_results",
            {"result": "Data X processed successfully", "original_trace_id": msg.trace_id},
