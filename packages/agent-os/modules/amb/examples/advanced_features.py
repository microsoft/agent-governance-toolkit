...
async def main():
    ...
    async with bus:
        ...
        with TraceContext.start("process_user_action") as ctx:
            ...
            await bus.publish("user.events", {"user_id": "user-123", "event_type": "login"})
            ...
            await bus.publish(
                "user.events", {"user_id": "user-123", "event_type": "session_validated"}
            )
        ...
        await bus.publish("unreliable.topic", {"data": "this will fail"})
        ...
        for i in range(3):
            await bus.publish("audit.events", {"event_num": i, "action": "test_action"})
        ...
        try:
            ...
            print("    Topic: fraud.alerts")
            print("    Expected fields: transaction_id, amount, risk_score, account_id")
