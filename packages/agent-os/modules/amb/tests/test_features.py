...
class TestSchemaValidation:
    ...
    def test_validate_valid_payload(self):
        ...
        payload = {"transaction_id": "tx123", "amount": 100.50, "risk_score": 0.95}
    ...
    async def test_bus_with_schema_validation(self):
        ...
        async with MessageBus(schema_registry=registry) as bus:
            ...
            msg_id = await bus.publish(
                "fraud.alerts", {"transaction_id": "tx123", "amount": 100.0, "risk_score": 0.95}
            )
...
class TestDeadLetterQueue:
    ...
    async def test_dlq_add_entry(self):
        ...
        message = Message(id="msg-1", topic="test.topic", payload={"data": "test"})

        entry = DLQEntry(
            message=message, reason=DLQReason.HANDLER_ERROR, error_message="Test error"
        )
...
class TestMessagePriority:
    ...
    def test_message_with_priority(self):
        ...
        message = Message(
            id="msg-1", topic="test.topic", payload={"data": "test"}, priority=Priority.CRITICAL
        )
    ...
    async def test_publish_with_priority(self):
        ...
        async with MessageBus() as bus:
            msg_id = await bus.publish(
                "fraud.alerts", {"alert": "high_risk"}, priority=Priority.CRITICAL
            )
            assert msg_id is not None
...
class TestIntegration:
    ...
    async def test_full_featured_bus(self):
        ...
        bus = MessageBus(persistence=True, schema_registry=registry, dlq_enabled=dlq)
        ...
        async with bus:
            ...
            with TraceContext.start("fraud_detection") as ctx:
                await bus.publish(
                    "fraud.alerts",
                    {"transaction_id": "tx123", "amount": 10000.0, "risk_score": 0.95},
