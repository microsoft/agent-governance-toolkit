...
class CloudEvent(BaseModel):
    ...
    specversion: str = Field(
        default=CLOUDEVENTS_SPEC_VERSION, description="CloudEvents spec version"
    )
    ...
    datacontenttype: Optional[str] = Field(
        default="application/json", description="Content type of the data attribute"
    )
    dataschema: Optional[str] = Field(
        None, description="URI identifying the schema that data adheres to"
    )
    subject: Optional[str] = Field(
        None, description="Subject of the event in the context of the event producer"
    )
    time: Optional[datetime] = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Timestamp of when the event happened (RFC 3339)",
    )
    data: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Event payload")

    # Extension attributes (Agent OS / AMB specific)
    # Prefixed with 'amb' namespace per CloudEvents extension naming conventions
    ambpriority: Optional[int] = Field(None, description="Message priority (AMB extension)")
    ambtraceid: Optional[str] = Field(None, description="Distributed trace ID (AMB extension)")
    ambspanid: Optional[str] = Field(None, description="Span ID within the trace (AMB extension)")
    ambparentspanid: Optional[str] = Field(None, description="Parent span ID (AMB extension)")
    ambttl: Optional[int] = Field(None, description="Time-to-live in seconds (AMB extension)")
    ambsender: Optional[str] = Field(None, description="Sender identifier (AMB extension)")
    ambcorrelationid: Optional[str] = Field(
        None, description="Correlation ID for request-response patterns (AMB extension)"
    )
    ambreplyto: Optional[str] = Field(None, description="Topic to reply to (AMB extension)")
...
def from_cloudevent(event: CloudEvent, *, type_prefix: str = AGENT_OS_TYPE_PREFIX) -> Message:
    """
