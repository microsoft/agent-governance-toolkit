...
class DeadLetterQueue:
    ...
    async def retry(self, message_id: str, handler: Callable[[Message], Awaitable[None]]) -> bool:
        """
        ...
        if entry.retry_count >= self._max_retries:
            raise ValueError(f"Message {message_id} has exceeded max retries ({self._max_retries})")
    ...
    async def retry_all(
        self, handler: Callable[[Message], Awaitable[None]], topic: Optional[str] = None
    ) -> Dict[str, bool]:
        """
