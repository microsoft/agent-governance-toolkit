...
try:
    ...
    raise ImportError(
        "NATS adapter requires 'nats-py' package. Install it with: pip install amb-core[nats]"
    )
...
class NATSBroker(BrokerAdapter):
    ...
    async def subscribe(self, topic: str, handler: MessageHandler) -> str:
        ...
        if self.use_jetstream and self._js:
            ...
            sub = await self._js.subscribe(
                subject, cb=message_callback, durable=f"amb-{subscription_id[:8]}"
            )
        else:
    ...
    async def request(self, message: Message, timeout: float = 30.0) -> Message:
        ...
        try:
            ...
            response = await self._nc.request(subject, message_bytes, timeout=timeout)
    ...
    async def get_pending_messages(self, topic: str, limit: int = 10) -> List[Message]:
        ...
        try:
            ...
            psub = await self._js.pull_subscribe(subject, durable=f"pending-{uuid.uuid4().hex[:8]}")
