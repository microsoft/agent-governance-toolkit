...
try:
    ...
    raise ImportError(
        "AWS SQS adapter requires 'aioboto3' package. Install it with: pip install amb-core[aws]"
    )
...
class AWSSQSBroker(BrokerAdapter):
        ...
        async with self._session.client("sqs") as sqs:
            if not self.queue_url:
                ...
                try:
                    ...
                    response = await sqs.create_queue(QueueName=queue_name, Attributes=attributes)
                    self.queue_url = response["QueueUrl"]
        ...
        async with self._session.client("sqs") as sqs:
            ...
            params = {
                ...
                "MessageAttributes": {
                    ...
                    "message_id": {"DataType": "String", "StringValue": message.id},
                },
            }
                    ...
                    for sqs_message in messages:
                        try:
                            ...
                            await sqs.delete_message(
                                QueueUrl=self.queue_url, ReceiptHandle=sqs_message["ReceiptHandle"]
                            )
    ...
    async def health_check(self) -> bool:
        ...
        try:
            async with self._session.client("sqs") as sqs:
                await sqs.get_queue_attributes(QueueUrl=self.queue_url, AttributeNames=["QueueArn"])
                return True
        except Exception:
            return False
