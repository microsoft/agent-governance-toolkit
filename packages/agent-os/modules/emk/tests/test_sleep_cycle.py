...
def test_memory_compressor_creation(temp_store, temp_rules_file):
    ...
    compressor = MemoryCompressor(
        store=temp_store, age_threshold_days=30, rules_filepath=temp_rules_file
    )
...
def test_identify_old_episodes(temp_store, temp_rules_file):
    ...
    compressor = MemoryCompressor(
        store=temp_store, age_threshold_days=7, rules_filepath=temp_rules_file
    )
...
def test_summarize_episodes(temp_store, temp_rules_file):
    ...
    compressor = MemoryCompressor(store=temp_store, rules_filepath=temp_rules_file)
...
def test_retrieve_rules_with_filters(temp_store, temp_rules_file):
    ...
    rule1 = type(rule1)(**{**rule1.to_dict(), "metadata": {"category": "database"}})
    compressor.store_rule(rule1)

    rule2 = compressor.summarize_episodes(episodes)
    rule2 = type(rule2)(**{**rule2.to_dict(), "metadata": {"category": "api"}})
