...
def test_retrieve_failures(temp_file):
    ...
    success_episode = Episode(
        goal="Success goal", action="Success action", result="Success", reflection="Worked well"
    )
...
def test_retrieve_with_anti_patterns(temp_file):
    ...
    for i in range(2):
        success = Episode(
            goal=f"API call {i}", action="GET /api/data", result="200 OK", reflection="Success"
        )
...
def test_retrieve_with_anti_patterns_no_failures(temp_file):
    ...
    for i in range(3):
        episode = Episode(
            goal=f"Goal {i}", action=f"Action {i}", result="Success", reflection="Done"
        )
...
def test_retrieve_with_anti_patterns_exclude_failures(temp_file):
    ...
    failure = Episode(
        goal="Failure", action="B", result="Error", reflection="Bad", metadata={"is_failure": True}
    )
    adapter.store(failure)
