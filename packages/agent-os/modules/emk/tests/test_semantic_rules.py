...
def test_semantic_rule_creation():
    ...
    rule = SemanticRule(
        rule="Use indexed user_id for database queries", source_episode_ids=["abc123", "def456"]
    )
...
def test_semantic_rule_id_generation():
    ...
    rule = SemanticRule(rule="Test rule", source_episode_ids=["test123"])
...
def test_semantic_rule_immutability():
    ...
    rule = SemanticRule(rule="Original rule", source_episode_ids=["test123"])
...
def test_semantic_rule_to_dict():
    ...
    rule = SemanticRule(rule="Test dict conversion", source_episode_ids=["test"])
...
def test_semantic_rule_confidence_validation():
    ...
    # Valid confidence
    rule = SemanticRule(rule="Valid confidence", source_episode_ids=["test"], confidence=0.5)
    assert rule.confidence == 0.5

    # Test edge cases
    rule_min = SemanticRule(rule="Min confidence", source_episode_ids=["test"], confidence=0.0)
    assert rule_min.confidence == 0.0

    rule_max = SemanticRule(rule="Max confidence", source_episode_ids=["test"], confidence=1.0)
    assert rule_max.confidence == 1.0

    # Invalid confidence should raise error
    with pytest.raises(Exception):  # Pydantic validation error
        SemanticRule(rule="Invalid confidence", source_episode_ids=["test"], confidence=1.5)


def test_semantic_rule_empty_source_ids():
    """Test semantic rule with empty source episode IDs."""
    rule = SemanticRule(rule="No source episodes", source_episode_ids=[])
...
def test_semantic_rule_timestamp():
    ...
    rule = SemanticRule(rule="Test timestamp", source_episode_ids=["test"])
