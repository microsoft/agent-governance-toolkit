...
class MemoryCompressor:
    ...
    def identify_old_episodes(self, episodes: List[Episode]) -> List[Episode]:
        ...
        old_episodes = [ep for ep in episodes if ep.timestamp < threshold]
        return old_episodes

    def summarize_episodes(
        self, episodes: List[Episode], summarizer: Optional[Callable[[List[Episode]], str]] = None
    ) -> SemanticRule:
        """
    ...
    def _default_summarize(self, episodes: List[Episode]) -> str:
        ...
        if failures:
            summary_parts.append(f"Warning: {len(failures)}/{len(episodes)} attempts failed")
        ...
        with open(self.rules_filepath, "r") as f:
            for line in f:
                ...
                try:
                    ...
                    if filters:
                        match = all(
                            rule.metadata.get(key) == value for key, value in filters.items()
                        )
                        if not match:
                            continue
    ...
    def compress_old_episodes(
        ...
        if errors:
            ...
            result["message"] = (
                f"Dry run: Would compress {compressed_count} episodes into {rules_created} rules"
            )
        else:
