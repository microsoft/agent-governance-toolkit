...
class SemanticRule(BaseModel):
    ...
    source_episode_ids: List[str] = Field(
        default_factory=list, description="IDs of episodes that contributed to this rule"
    )
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="When the rule was created"
    )
