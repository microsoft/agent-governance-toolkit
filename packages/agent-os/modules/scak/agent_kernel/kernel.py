...
class SelfCorrectingAgentKernel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("  Loop 1 (Runtime): Constraint Engine (Safety)")
        logger.info("  Loop 2 (Offline): Alignment Engine (Quality & Efficiency)")
        logger.info(f"    - Completeness Auditor: {self.completeness_auditor.teacher_model}")
        logger.info("    - Semantic Purge: Active")
        logger.info("    - Failure Triage: Active (Sync/Async routing)")
        logger.info(f"    - Semantic Analysis: {use_semantic_analysis}")
        logger.info("    - Nudge Mechanism: Active")
    ...
    def handle_failure(
        ...
        """
        logger.info("=" * 80)
        logger.info("AGENT FAILURE DETECTED - Starting enhanced self-correction process")
        ...
        if user_prompt:
            ...
            if strategy == FixStrategy.ASYNC_BATCH:
                ...
                logger.info(
                    ">> Critical Failure Detected. Entering Self-Correction Mode (User Waiting)..."
                )
        ...
        patch = self.patcher.create_patch(agent_id, analysis, simulation, diagnosis, shadow_result)

        # Classify patch for lifecycle management (Semantic Purge integration)
        classified_patch = self.semantic_purge.register_patch(
            patch=patch, current_model_version=self.current_model_version
        )
        ...
        return {
            ...
            "message": "Agent successfully patched"
            if patch_applied
            else "Patch created, awaiting manual approval",
        }
    ...
    def wake_up_and_fix(
        self, agent_id: str, error_message: str, context: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    def handle_outcome(
        ...
        if self.outcome_analyzer.should_trigger_audit(outcome):
            logger.info("🔍 Give-Up Signal detected! Triggering Completeness Auditor...")

            # Step 2a: Auto-nudge if enabled
            if auto_nudge and self.nudge_mechanism.should_nudge(outcome):
                logger.info("💡 Auto-nudge enabled - attempting nudge...")
                ...
                logger.info("✓ Nudge prompt generated (agent should be re-invoked)")
            ...
            if audit.teacher_found_data:
                logger.info("⚠️  LAZINESS DETECTED: Creating competence patch...")
                ...
                self.semantic_purge.register_completeness_audit(
                    audit=audit, current_model_version=self.current_model_version
                )

                # Apply patch
                if self.config.get("auto_patch", True):
                    self.patcher.apply_patch(patch)
                    logger.info("✓ Competence patch applied")
        else:
            logger.info("✓ No give-up signal detected - agent performing well")

        return result

    def _create_competence_patch(self, agent_id: str, audit: CompletenessAudit) -> CorrectionPatch:
        """
        ...
        from .models import AgentFailure, FailureSeverity, FailureType
    ...
    def upgrade_model(self, new_model_version: str) -> Dict[str, Any]:
        ...
        purge_result = self.semantic_purge.purge_on_upgrade(
            old_model_version=old_version, new_model_version=new_model_version
        )
    ...
    def process_async_queue(self, batch_size: int = 10) -> Dict[str, Any]:
        ...
        logger.info("=" * 80)
        logger.info("ASYNC QUEUE PROCESSING COMPLETE")
