...
class AgentPatcher:
    ...
    def _apply_rag_memory_patch(self, patch: CorrectionPatch):
        ...
        logger.info(
            f"Injected RAG memory for agent {patch.agent_id}: {memory['correct_logic'][:50]}..."
        )
    ...
    def _rollback_rag_memory(self, patch: CorrectionPatch):
        ...
        self.rag_memories = [m for m in self.rag_memories if m.get("patch_id") != patch.patch_id]
        logger.info(f"Removed RAG memory for patch {patch.patch_id}")

    def get_agent_state(self, agent_id: str) -> AgentState:
        """Get the current state of an agent."""
        if agent_id not in self.agent_states:
            self.agent_states[agent_id] = AgentState(agent_id=agent_id, status="unknown")
        return self.agent_states[agent_id]

    def _determine_patch_strategy(
        self, analysis: FailureAnalysis, diagnosis: Optional[DiagnosisJSON]
    ) -> PatchStrategy:
        """
    ...
    def _determine_patch_type(self, analysis: FailureAnalysis, strategy: PatchStrategy) -> str:
    ...
    def _generate_system_prompt_rule(
        self, diagnosis: Optional[DiagnosisJSON], analysis: FailureAnalysis
    ) -> str:
        """
        ...
        if diagnosis.cognitive_glitch == CognitiveGlitch.POLICY_VIOLATION:
            ...
            # Then check failure trace if available
                ...
                if any(
                    kw in prompt_lower
                    for kw in ["medical", "health", "diagnosis", "treatment", "medicine"]
                ):
                    domain = "medical issues"
                elif any(
                    kw in prompt_lower for kw in ["legal", "law", "attorney", "sue", "lawsuit"]
                ):
                    domain = "legal matters"
                elif any(
                    kw in prompt_lower for kw in ["investment", "financial", "stock", "invest"]
                ):
                    domain = "investment advice"
    ...
    def _generate_rag_memory_content(
        ...
        return {
            ...
            "correct_logic": shadow_result.output
            if shadow_result
            else analysis.suggested_fixes[0]
    ...
    def _generate_code_change_content(
        self, analysis: FailureAnalysis, simulation: SimulationResult
    ) -> Dict[str, Any]:
    ...
    def _update_agent_state(self, agent_id: str, patch: CorrectionPatch):
        ...
        if agent_id not in self.agent_states:
            self.agent_states[agent_id] = AgentState(agent_id=agent_id, status="running")
