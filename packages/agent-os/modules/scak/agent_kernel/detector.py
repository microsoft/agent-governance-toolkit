...
from .models import AgentFailure, FailureSeverity, FailureTrace, FailureType
...
class FailureDetector:
    ...
    def _classify_failure(
        ...
        if any(
            keyword in error_lower
            for keyword in ["resource", "memory", "disk", "quota", "limit exceeded"]
        ):
        ...
        if any(
            keyword in error_lower
            for keyword in ["assertion", "null pointer", "index out", "key error", "type error"]
        ):
    ...
    def _assess_severity(
        self, failure_type: FailureType, context: Optional[Dict[str, Any]]
    ) -> FailureSeverity:
