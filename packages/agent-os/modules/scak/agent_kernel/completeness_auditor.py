...
class CompletenessAuditor:
    ...
    def audit_give_up(self, outcome: AgentOutcome) -> CompletenessAudit:
        ...
        logger.info(
            f"   Give-up signal: {outcome.give_up_signal.value if outcome.give_up_signal else 'unknown'}"
        )
        ...
        if teacher_found_data:
            ...
            logger.warning("⚠️  LAZINESS DETECTED: Teacher found data that agent missed!")

            gap_analysis = self._analyze_gap(outcome, teacher_result)
            competence_patch = self._generate_competence_patch(
                outcome, gap_analysis, teacher_result
            )
            confidence = teacher_result["confidence"]
        else:
            # Teacher also couldn't find data - agent was correct
            logger.info("✓ Agent was correct: No data available")
    ...
    def _generate_competence_patch(
        self, outcome: AgentOutcome, gap_analysis: str, teacher_result: dict
    ) -> str:
        """
        ...
        if "log" in user_prompt_lower:
            ...
            patch = (
                f"Before reporting 'not found', verify all registry sources including {location}."
            )
            ...
            patch = (
                f"Expand search scope to include {location} before concluding data unavailability."
            )

        # Add context about the specific failure
        patch += " This prevents false negatives when data exists but requires deeper search."
    ...
    def get_audit_stats(self) -> dict:
        ...
        return {
            ...
            "laziness_rate": self.lazy_detection_count / self.audit_count
