...
class OutcomeAnalyzer:
    ...
    def analyze_outcome(
        ...
        if self.use_semantic_analysis:
            ...
            logger.debug(
                f"Semantic analysis: {semantic_analysis.semantic_category} "
        ...
        if outcome_type == OutcomeType.GIVE_UP:
            logger.warning(
                f"Give-up detected: signal={give_up_signal.value if give_up_signal else 'semantic'}"
            )
    ...
    def _determine_outcome_type(
        ...
        has_semantic_signal = (
            ...
            and semantic_analysis.is_refusal
            and semantic_analysis.refusal_confidence > 0.6
        )
        ...
        if has_regex_signal or has_semantic_signal:
            ...
            if tool_analysis["tools_called"] and tool_analysis["all_empty_results"]:
                ...
                logger.info(
                    "Give-up signal present but tools returned empty results - valid empty set"
                )
                ...
                logger.warning(
                    "Give-up with tool errors - potential laziness or error handling issue"
                )
    ...
    def _analyze_tool_execution(
        self, tool_telemetry: Optional[List[ToolExecutionTelemetry]]
    ) -> dict:
        """
        ...
        called_tools = [
            t for t in tool_telemetry if t.tool_status != ToolExecutionStatus.NOT_CALLED
        ]
        ...
        empty_results = [
            t for t in called_tools if t.tool_status == ToolExecutionStatus.EMPTY_RESULT
        ]

        errored_tools = [t for t in called_tools if t.tool_status == ToolExecutionStatus.ERROR]
