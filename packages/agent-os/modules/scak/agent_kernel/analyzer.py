...
from .models import AgentFailure, CognitiveGlitch, DiagnosisJSON, FailureAnalysis, FailureType
...
class FailureAnalyzer:
    ...
    def _load_known_patterns(self) -> Dict[str, dict]:
        ...
        return {
            ...
            FailureType.RESOURCE_EXHAUSTED: {
                "root_causes": ["Memory leak", "Unbounded resource allocation", "Missing cleanup"],
                "fixes": [
            ...
            FailureType.LOGIC_ERROR: {
                "root_causes": ["Incorrect algorithm", "Edge case not handled", "Type mismatch"],
                "fixes": ["Fix algorithm logic", "Add edge case handling", "Add type checking"],
            },
        }

    def analyze(
        self, failure: AgentFailure, similar_failures: Optional[List[AgentFailure]] = None
    ) -> FailureAnalysis:
        """
        ...
        if similar_failures:
            similar_failure_ids = [
                f.agent_id + "_" + str(f.timestamp) for f in similar_failures[:5]
            ]
        ...
        logger.info(
            f"Analysis complete. Root cause: {root_cause} (confidence: {confidence_score:.2f})"
        )
    ...
    def _calculate_confidence(
        self, failure: AgentFailure, similar_failures: Optional[List[AgentFailure]]
    ) -> float:
    ...
    def _identify_cognitive_glitch(self, failure: AgentFailure, trace) -> CognitiveGlitch:
        ...
        if "uuid" in error_lower or (
            "id" in error_lower
            and any(kw in error_lower for kw in ["invalid", "malformed", "not a valid", "format"])
        ):
        ...
        if trace.user_prompt:
            ...
            if any(
                ...
                if any(
                    keyword in error_lower
                    for keyword in ["blocked", "violation", "not permitted", "cannot", "policy"]
                ):
                    return CognitiveGlitch.POLICY_VIOLATION
    ...
    def _analyze_deep_problem(self, failure: AgentFailure, trace, glitch: CognitiveGlitch) -> str:
        ...
        if glitch == CognitiveGlitch.HALLUCINATION:
            ...
            return "Agent lacked necessary context (schema/permissions) to safely execute action"
        elif glitch == CognitiveGlitch.PERMISSION_ERROR:
            return "Agent attempted unauthorized action without checking permissions first"
        elif glitch == CognitiveGlitch.SCHEMA_MISMATCH:
            return "Agent referenced incorrect schema elements in action"
    ...
    def _calculate_diagnosis_confidence(
        self, failure: AgentFailure, trace, evidence: List[str]
    ) -> float:
        ...
        for past_failure in history:
            if past_failure.failure_type == failure.failure_type:
                ...
                similarity = self._calculate_similarity(
                    failure.error_message, past_failure.error_message
                )
                if similarity > 0.6:
