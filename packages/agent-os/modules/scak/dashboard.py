...
# Page config
st.set_page_config(
    page_title="SCAK Dashboard", page_icon="🤖", layout="wide", initial_sidebar_state="expanded"
)
...
with tab1:
    ...
    with col1:
        st.metric(label="Total Agents", value="12", delta="2 new")

    with col2:
        st.metric(label="Active Tasks", value="8", delta="-3")

    with col3:
        st.metric(label="Patches Applied", value="147", delta="5 today")

    with col4:
        st.metric(label="Security Events", value="23", delta="2 blocked")
    ...
    fig.update_layout(
        xaxis_title="Hours Ago", yaxis_title="Success Rate (%)", hovermode="x unified", height=300
    )
...
with tab2:
    ...
    fig.update_layout(barmode="group", xaxis_title="Lesson Type", yaxis_title="Count", height=400)
...
with tab3:
    ...
    events_data = {
        "Timestamp": [
            (datetime.now() - timedelta(minutes=i * 15)).strftime("%H:%M") for i in range(5)
        ],
...
with tab4:
    ...
    fig = go.Figure(data=[go.Bar(x=agents, y=workloads, marker_color="#636EFA")])

    fig.update_layout(xaxis_title="Agent", yaxis_title="Active Tasks", height=300)
