...
class TestOutcomeAnalyzer:
    ...
    def test_should_trigger_audit(self):
        ...
        outcome = analyzer.analyze_outcome(
            agent_id="test-agent", user_prompt="Find data", agent_response="No data found."
        )
...
class TestCompletenessAuditor:
    ...
    def test_audit_confirms_agent_correct(self):
        ...
        assert (
            "appropriate" in audit.gap_analysis.lower() or "correct" in audit.gap_analysis.lower()
        )
    ...
    def test_audit_stats(self):
        ...
        for i in range(3):
            outcome = analyzer.analyze_outcome(
                agent_id="test-agent", user_prompt=f"Find logs {i}", agent_response="No logs found."
            )
...
class TestSemanticPurge:
    ...
    def test_classify_tool_misuse_as_syntax(self):
        ...
        failure = AgentFailure(
            ...
            error_message="UUID error",
        )

        analysis = FailureAnalysis(
            failure=failure,
            root_cause="Tool misuse",
            suggested_fixes=[],
    ...
    def test_classify_hallucination_as_business(self):
        ...
        patch = CorrectionPatch(
            ...
            patch_content={"negative_constraint": "Project_Alpha does not exist"},
        )
        patch.diagnosis = diagnosis
...
class TestDualLoopIntegration:
    ...
    def test_full_dual_loop_workflow(self):
        ...
        kernel = SelfCorrectingAgentKernel(config={"model_version": "gpt-4o", "auto_patch": True})
