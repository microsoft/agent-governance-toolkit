...
class TestActionResultPair(unittest.TestCase):
    ...
    def test_signature_creation(self):
        ...
        pair = ActionResultPair(
            action="query_db", action_params={"table": "users", "limit": 10}, result="No data found"
        )
    ...
    def test_same_signature_for_identical_pairs(self):
        ...
        pair1 = ActionResultPair(action="search", action_params={"query": "test"}, result="empty")

        pair2 = ActionResultPair(action="search", action_params={"query": "test"}, result="empty")
    ...
    def test_different_signature_for_different_params(self):
        ...
        pair2 = ActionResultPair(action="search", action_params={"query": "other"}, result="empty")
...
class TestCircuitBreaker(unittest.TestCase):
    ...
    def test_single_action_no_loop(self):
        ...
        loop_detected = self.breaker.record_action_result(
            action="search", action_params={"query": "test"}, result="No results found"
        )
    ...
    def test_different_actions_no_loop(self):
        ...
        self.breaker.record_action_result(
            action="search", action_params={"query": "test"}, result="No results"
        )

        self.breaker.record_action_result(
            action="query_db", action_params={"table": "users"}, result="No data"
        )

        self.breaker.record_action_result(
            action="fetch_logs", action_params={"date": "2024-01-01"}, result="Empty"
        )
    ...
    def test_loop_detection_at_threshold(self):
        ...
        for i in range(3):
            loop_detected = self.breaker.record_action_result(
                action=action, action_params=params, result=result
            )
    ...
    def test_history_window_limit(self):
        ...
        for i in range(10):
            breaker.record_action_result(
                action=f"action_{i}", action_params={"id": i}, result=f"result_{i}"
            )
...
class TestCircuitBreakerRegistry(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.registry = CircuitBreakerRegistry(
            default_repetition_threshold=3, default_history_window=10
        )
...
class TestLoopDetectionScenarios(unittest.TestCase):
    ...
    def test_query_with_different_params_no_loop(self):
        ...
        breaker = CircuitBreaker(agent_id="query-agent", repetition_threshold=3)
    ...
    def test_api_retry_loop(self):
        ...
        breaker = CircuitBreaker(
            agent_id="api-agent", repetition_threshold=3, strategy=LoopDetectionStrategy.ESCALATE
        )
