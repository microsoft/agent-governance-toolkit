...
class TestFailureTriage(unittest.TestCase):
    ...
    def test_critical_tool_sync_jit(self):
        ...
        strategy = self.triage.decide_strategy(
            prompt="Delete the user records", tool_name="delete_resource"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Test with execute_payment
        strategy = self.triage.decide_strategy(
            prompt="Process refund for customer", tool_name="execute_payment"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Test with drop_table
        strategy = self.triage.decide_strategy(prompt="Clean up old data", tool_name="drop_table")
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

    def test_critical_action_in_context(self):
        """Test that critical actions in context trigger SYNC_JIT."""
        strategy = self.triage.decide_strategy(
            prompt="Delete some files", context={"action": "delete_file", "path": "/important/data"}
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        strategy = self.triage.decide_strategy(
            prompt="Update database", context={"action": "update_db", "table": "users"}
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)
    ...
    def test_high_effort_prompt_sync_jit(self):
        ...
        # Test with "carefully"
        strategy = self.triage.decide_strategy(
            prompt="Please carefully analyze the security logs", tool_name="read_logs"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Test with "critical"
        strategy = self.triage.decide_strategy(
            prompt="This is a critical operation for production", tool_name="query_db"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Test with "important"
        strategy = self.triage.decide_strategy(
            prompt="Important: Check all user permissions", tool_name="check_permissions"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Test with "urgent"
        strategy = self.triage.decide_strategy(
            prompt="Urgent request from customer", tool_name="fetch_data"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Test with "must"
        strategy = self.triage.decide_strategy(
            prompt="You must verify all entries before proceeding", tool_name="verify"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)
    ...
    def test_read_operations_async_batch(self):
        ...
        strategy = self.triage.decide_strategy(prompt="Get the latest logs", tool_name="read_logs")
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)

        # Query operation
        strategy = self.triage.decide_strategy(
            prompt="Find user with email test@example.com", tool_name="query_users"
        )
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)

        # Fetch operation
        strategy = self.triage.decide_strategy(prompt="Fetch recent data", tool_name="fetch_data")
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)

    def test_default_to_async_batch(self):
        """Test that non-critical operations default to ASYNC_BATCH."""
        strategy = self.triage.decide_strategy(
            prompt="Show me the dashboard", tool_name="render_dashboard"
        )
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)

        strategy = self.triage.decide_strategy(
            prompt="List all available options", tool_name="list_options"
        )
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)
    ...
    def test_is_critical_convenience_method(self):
        ...
        self.assertTrue(
            self.triage.is_critical(prompt="Delete records", tool_name="delete_resource")
        )

        # High effort prompt should return True
        self.assertTrue(
            self.triage.is_critical(prompt="This is critical operation", tool_name="some_tool")
        )

        # Non-critical should return False
        self.assertFalse(self.triage.is_critical(prompt="Show me data", tool_name="read_data"))

    def test_custom_critical_tools(self):
        """Test custom critical tools configuration."""
        custom_triage = FailureTriage(config={"critical_tools": ["custom_delete", "custom_update"]})

        # Custom critical tool
        strategy = custom_triage.decide_strategy(prompt="Do something", tool_name="custom_delete")
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Default critical tool not in custom list
        strategy = custom_triage.decide_strategy(
            prompt="Delete resource", tool_name="delete_resource"
        )
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)

    def test_custom_high_effort_keywords(self):
        """Test custom high effort keywords configuration."""
        custom_triage = FailureTriage(config={"high_effort_keywords": ["immediate", "priority"]})

        # Custom keyword
        strategy = custom_triage.decide_strategy(
            prompt="This needs immediate attention", tool_name="some_tool"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Default keyword not in custom list
        strategy = custom_triage.decide_strategy(prompt="This is critical", tool_name="some_tool")
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)
    ...
    def test_case_insensitive_keyword_matching(self):
        ...
        # Uppercase keyword
        strategy = self.triage.decide_strategy(
            prompt="CRITICAL operation needed", tool_name="some_tool"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # Mixed case
        strategy = self.triage.decide_strategy(
            prompt="Please handle this CareFully", tool_name="some_tool"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)
    ...
    def test_priority_order(self):
        ...
        strategy = self.triage.decide_strategy(
            prompt="Just delete this", tool_name="delete_resource"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # High effort keyword overrides default async
        strategy = self.triage.decide_strategy(
            prompt="Critical: fetch this data", tool_name="fetch_data"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

        # VIP user overrides default async
        strategy = self.triage.decide_strategy(
            prompt="Get my data", tool_name="fetch_data", user_metadata={"is_vip": True}
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)
...
class TestTriageEdgeCases(unittest.TestCase):
    ...
    def test_empty_prompt(self):
        ...
        strategy = self.triage.decide_strategy(prompt="", tool_name="some_tool")
        # Should default to ASYNC_BATCH
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)

    def test_none_context(self):
        """Test handling when context is None."""
        strategy = self.triage.decide_strategy(prompt="Do something", tool_name=None, context=None)
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)

    def test_empty_context(self):
        """Test handling of empty context dict."""
        strategy = self.triage.decide_strategy(prompt="Do something", context={})
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)
    ...
    def test_keyword_as_part_of_word(self):
        ...
        strategy = self.triage.decide_strategy(
            prompt="This is critically important", tool_name="some_tool"
        )
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)
    ...
    def test_partial_cognitive_trace_chain_only(self):
        ...
        strategy = self.triage.decide_strategy(
            prompt="Execute query",
            tool_name="query_db",
            context={"chain_of_thought": ["Step 1", "Step 2"]},
        )
    ...
    def test_partial_cognitive_trace_failed_action_only(self):
        ...
        strategy = self.triage.decide_strategy(
            prompt="Execute query",
            tool_name="query_db",
            context={"failed_action": {"action": "query", "params": {}}},
        )
        # Should not trigger cognitive failure rule
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)
...
class TestTriageRealWorldScenarios(unittest.TestCase):
    ...
    def test_dashboard_loading_failure(self):
        ...
        strategy = self.triage.decide_strategy(
            prompt="Load user dashboard", tool_name="fetch_dashboard", context={"user_id": 123}
        )
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)
...
class TestTriageConfiguration(unittest.TestCase):
    ...
    def test_empty_critical_tools_config(self):
        ...
        strategy = triage.decide_strategy(prompt="Delete resource", tool_name="delete_resource")
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)
    ...
    def test_empty_keywords_config(self):
        ...
        strategy = triage.decide_strategy(prompt="Critical operation needed", tool_name="some_tool")
        self.assertEqual(strategy, FixStrategy.ASYNC_BATCH)
    ...
    def test_extended_critical_tools(self):
        ...
        strategy = triage.decide_strategy(prompt="Deploy to prod", tool_name="deploy_to_production")
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)

    def test_domain_specific_keywords(self):
        """Test domain-specific high-effort keywords."""
        triage = FailureTriage(config={"high_effort_keywords": ["emergency", "outage", "incident"]})

        strategy = triage.decide_strategy(prompt="Emergency outage detected", tool_name="notify")
        self.assertEqual(strategy, FixStrategy.SYNC_JIT)
