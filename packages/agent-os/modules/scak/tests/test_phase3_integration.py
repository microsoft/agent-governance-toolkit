...
from src.kernel.schemas import FailureTrace, Lesson, MemoryTier, PatchRequest
...
class TestPhase3Integration(unittest.TestCase):
    ...
    def test_disaster_recovery_workflow(self):
        ...
        original_cache_keys = [
            k for k in self.controller.redis_cache.store.keys() if k.startswith("skill:")
        ]
    ...
    def test_semantic_fallback_and_evaluation(self):
        ...
        evaluation = self.rubric.evaluate(trace, lesson)
        self.assertIn(
            evaluation["tier"], [MemoryTier.TIER_2_SKILL_CACHE, MemoryTier.TIER_3_ARCHIVE]
        )
