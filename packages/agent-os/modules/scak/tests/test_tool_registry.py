...
class TestToolRegistry:
    ...
    def test_get_tool_schema(self):
        ...
        definition = ToolDefinition(
            ...
            parameters=[
                ...
                ToolParameter(
                    name="limit", type="integer", description="Result limit", required=False
                ),
            ],
            returns="results",
        )
    ...
    def test_get_tools_by_type(self):
        ...
        registry.register_tool(
            ToolDefinition(
                name="t1", description="", tool_type=ToolType.TEXT, parameters=[], returns=""
            ),
            lambda: None,
        )
        registry.register_tool(
            ToolDefinition(
                name="t2", description="", tool_type=ToolType.VISION, parameters=[], returns=""
            ),
            lambda: None,
        )
        registry.register_tool(
            ToolDefinition(
                name="t3", description="", tool_type=ToolType.TEXT, parameters=[], returns=""
            ),
            lambda: None,
        )
...
class TestToolDecorator:
    ...
    def test_decorator_with_multimodal(self):
        ...
        @tool(
            "vision_tool", "Process images", tool_type=ToolType.VISION, multimodal_inputs=["image"]
        )
...
class TestDefaultRegistry:
    ...
    async def test_default_tool_execution(self):
        ...
        result = await registry.execute_tool("web_search", {"query": "test", "max_results": 5})
