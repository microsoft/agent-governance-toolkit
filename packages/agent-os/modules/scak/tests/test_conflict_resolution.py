...
import pytest
from src.agents.conflict_resolution import AgentVote, ConflictResolver, ConflictType, VoteType
...
class TestConflictResolver:
    ...
    def resolver(self):
        ...
        return ConflictResolver(
            default_vote_type=VoteType.MAJORITY, supervisor_agent_id="supervisor-001"
        )
    ...
    async def test_majority_vote_clear_winner(self, resolver):
        ...
        resolution = await resolver.resolve_conflict(
            "conflict-001", ConflictType.DECISION, votes, VoteType.MAJORITY
        )
    ...
    async def test_majority_vote_tie(self, resolver):
        ...
        resolution = await resolver.resolve_conflict(
            "conflict-002", ConflictType.DECISION, votes, VoteType.MAJORITY
        )
    ...
    async def test_weighted_vote(self, resolver):
        ...
        votes = [
            ...
            AgentVote(agent_id="novice-2", option="option_b", confidence=0.4),
        ]

        resolution = await resolver.resolve_conflict(
            "conflict-003", ConflictType.INTERPRETATION, votes, VoteType.WEIGHTED
        )
    ...
    async def test_supermajority_met(self, resolver):
        ...
        votes = [
            AgentVote(agent_id=f"agent-{i}", option="approve", confidence=0.8) for i in range(7)
        ] + [
            AgentVote(agent_id=f"agent-{i}", option="reject", confidence=0.8) for i in range(7, 10)
        ]

        resolution = await resolver.resolve_conflict(
            "conflict-004", ConflictType.DECISION, votes, VoteType.SUPERMAJORITY
        )
    ...
    async def test_supermajority_not_met(self, resolver):
        ...
        resolution = await resolver.resolve_conflict(
            "conflict-005", ConflictType.DECISION, votes, VoteType.SUPERMAJORITY
        )
    ...
    async def test_unanimous_success(self, resolver):
        ...
        votes = [
            AgentVote(agent_id=f"agent-{i}", option="proceed", confidence=0.9) for i in range(5)
        ]

        resolution = await resolver.resolve_conflict(
            "conflict-006", ConflictType.DECISION, votes, VoteType.UNANIMOUS
        )
    ...
    async def test_unanimous_failure(self, resolver):
        ...
        resolution = await resolver.resolve_conflict(
            "conflict-007", ConflictType.DECISION, votes, VoteType.UNANIMOUS
        )
    ...
    async def test_ranked_choice_immediate_majority(self, resolver):
        ...
        resolution = await resolver.resolve_conflict(
            "conflict-008", ConflictType.PRIORITY, votes, VoteType.RANKED_CHOICE
        )
    ...
    async def test_resolution_history(self, resolver):
        ...
        await resolver.resolve_conflict("conflict-009", ConflictType.DECISION, votes)
    ...
    async def test_empty_votes_raises_error(self, resolver):
        ...
        with pytest.raises(ValueError, match="zero votes"):
            await resolver.resolve_conflict("conflict-010", ConflictType.DECISION, [])
...
class TestVotingEdgeCases:
    ...
    async def test_single_vote(self, resolver):
        ...
        votes = [AgentVote(agent_id="agent-1", option="approve", confidence=0.9)]

        resolution = await resolver.resolve_conflict(
            "conflict-single", ConflictType.DECISION, votes
        )
    ...
    async def test_all_different_options(self, resolver):
        ...
        resolution = await resolver.resolve_conflict(
            "conflict-all-diff", ConflictType.DECISION, votes
        )
