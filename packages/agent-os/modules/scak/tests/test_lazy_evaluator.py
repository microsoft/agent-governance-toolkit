...
class TestLazyEvaluator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.evaluator = LazyEvaluator(
            agent_id=self.agent_id, enable_lazy_eval=True, max_deferred_tasks=100
        )
    ...
    def test_should_defer_disabled(self):
        ...
        evaluator = LazyEvaluator(agent_id="test", enable_lazy_eval=False)

        decision = evaluator.should_defer(
            description="Expensive operation", context={}, estimated_cost_ms=5000
        )
    ...
    def test_should_defer_speculative_computation(self):
        ...
        decision = self.evaluator.should_defer(
            description="Check if user might need archived data", context={}
        )
        ...
        decision = self.evaluator.should_defer(
            description="Optional analysis of historical trends", context={}
        )
    ...
    def test_should_defer_missing_context(self):
        ...
        decision = self.evaluator.should_defer(
            description="Need more information from user before proceeding", context={}
        )
    ...
    def test_should_defer_low_priority(self):
        ...
        decision = self.evaluator.should_defer(
            description="Background cleanup task", context={"priority": 3, "low_priority": True}
        )
    ...
    def test_should_defer_archive_query(self):
        ...
        decision = self.evaluator.should_defer(
            description="Fetch historical data from archive", context={}
        )
    ...
    def test_should_not_defer_immediate_archive_query(self):
        ...
        decision = self.evaluator.should_defer(
            description="Fetch historical data from archive", context={"immediate": True}
        )
    ...
    def test_should_not_defer_normal_operation(self):
        ...
        decision = self.evaluator.should_defer(
            description="Fetch current user data", context={}, estimated_cost_ms=500
        )
    ...
    def test_resolve_token(self):
        ...
        # Defer a computation
        token = self.evaluator.defer(
            description="Compute result", reason=DeferralReason.NOT_NEEDED_NOW, context={}
        )
    ...
    def test_get_statistics(self):
        ...
        # Create some tokens
        token1 = self.evaluator.defer(
            "Task 1", DeferralReason.NOT_NEEDED_NOW, {}, estimated_cost_ms=1000
        )
        token2 = self.evaluator.defer(
            "Task 2", DeferralReason.TOO_EXPENSIVE, {}, estimated_cost_ms=2000
        )
...
class TestLazyEvaluatorRegistry(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        self.registry = LazyEvaluatorRegistry(enable_lazy_eval=True, max_deferred_tasks=100)
...
class TestLazyEvaluationScenarios(unittest.TestCase):
    ...
    def test_archive_query_deferral(self):
        ...
        decision = evaluator.should_defer(
            description="Fetch logs from 2023 archive", context={}, estimated_cost_ms=5000
        )

        self.assertTrue(decision.should_defer)
        self.assertIn(
            decision.reason, [DeferralReason.TOO_EXPENSIVE, DeferralReason.NOT_NEEDED_NOW]
        )
    ...
    def test_speculative_analysis_deferral(self):
        ...
        decision = evaluator.should_defer(
            description="The user might need a detailed breakdown, so let's compute it", context={}
        )
