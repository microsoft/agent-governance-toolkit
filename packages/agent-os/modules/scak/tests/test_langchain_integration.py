...
from agent_kernel.models import GiveUpSignal
...
class TestSCAKMemory(unittest.TestCase):
    ...
    def test_load_memory_variables_basic(self):
        ...
        inputs = {"input": "Find logs for error 500", "tools": []}
    ...
    def test_load_memory_variables_with_tools(self):
        ...
        inputs = {"input": "Search for error logs", "tools": [mock_tool]}
    ...
    def test_clear(self):
        ...
        # Add some history
        self.memory.save_context({"input": "Hello"}, {"output": "Hi there!"})
    ...
    def test_format_history_as_string(self):
        ...
        self.memory.save_context({"input": "Hello"}, {"output": "Hi!"})
...
class TestSCAKCallbackHandler(unittest.TestCase):
    ...
    def test_on_agent_finish_no_give_up(self):
        ...
        # Run callback
        asyncio.run(
            self.handler.on_agent_finish(
                finish, run_id="test-run-123", inputs={"input": "Do something"}
            )
        )
    ...
    def test_on_agent_finish_with_give_up(self):
        ...
        asyncio.run(
            self.handler.on_agent_finish(
                finish, run_id="test-run-123", inputs={"input": "Find logs"}
            )
        )
...
class TestSelfCorrectingRunnable(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.runnable = SelfCorrectingRunnable(agent=self.base_agent, triage=self.triage)
...
class TestCreateSCAKAgent(unittest.TestCase):
    ...
    def test_create_scak_agent_with_callback(self):
        ...
        scak_agent = create_scak_agent(base_agent, callback_handler=handler)
    ...
    def test_create_scak_agent_no_correction(self):
        ...
        scak_agent = create_scak_agent(base_agent, enable_correction=False)
