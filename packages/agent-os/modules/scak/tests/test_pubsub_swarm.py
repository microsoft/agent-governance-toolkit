...
import pytest
from src.agents.pubsub import AgentSwarm, InMemoryPubSub, PubSubMessage
...
class TestInMemoryPubSub:
    ...
    async def test_publish_subscribe(self, pubsub):
        ...
        # Publish
        msg = PubSubMessage(
            topic="test-topic", from_agent="agent-001", payload={"message": "Hello"}
        )
    ...
    async def test_multiple_subscribers(self, pubsub):
        ...
        # Publish message
        msg = PubSubMessage(topic="topic", from_agent="sender", payload={"data": "test"})
    ...
    async def test_message_history(self, pubsub):
        ...
        # Publish messages
        for i in range(5):
            msg = PubSubMessage(topic="history-test", from_agent=f"agent-{i}", payload={"index": i})
    ...
    async def test_dead_letter_queue(self, pubsub):
        ...
        msg = PubSubMessage(topic="fail-topic", from_agent="sender", payload={})
...
class TestAgentSwarm:
    ...
    def swarm(self):
        ...
        return AgentSwarm("test-swarm", pubsub, ["agent-1", "agent-2", "agent-3"])

    @pytest.mark.asyncio
    async def test_broadcast(self, swarm):
        """Test swarm broadcast."""
        # Broadcast message
        await swarm.broadcast("agent-1", "Test message", {"data": "value"})
    ...
    async def test_consensus_vote(self, swarm):
        ...
        result = await swarm.request_consensus(
            "agent-1", "Should we proceed?", {"confidence": 0.9}, required_votes=2
        )
    ...
    async def test_work_distribution(self, swarm):
        ...
        tasks = [{"task": "analyze", "id": i} for i in range(10)]

        assignments = await swarm.distribute_work("supervisor", tasks, strategy="round_robin")
