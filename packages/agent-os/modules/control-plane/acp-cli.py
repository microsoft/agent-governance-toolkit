...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="acp-cli", description="Agent Control Plane Command Line Interface"
    )
...
def cmd_agent_create(args, control_plane):
    """Create a new agent"""
    from agent_control_plane import ActionType, PermissionLevel
...
def cmd_audit_show(args, control_plane):
    ...
    try:
        ...
        if args.format == "json":
            ...
            for event in events:
                print(
                    f"  [{event.get('timestamp')}] {event.get('event_type')}: {event.get('agent_id')}"
                )
