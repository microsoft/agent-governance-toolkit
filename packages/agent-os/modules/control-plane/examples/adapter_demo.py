    ...
    class MockCompletions:
        def create(self, **kwargs):
            ...
            tool_calls = [
                ...
                MockOpenAIClient.MockToolCall(
                    "write_file", json.dumps({"path": "/data/output.txt", "content": "Results"})
                ),
            ]
...
def example_1_basic_usage():
    ...
    print("  Permissions: DATABASE_QUERY (read), FILE_READ (read), FILE_WRITE (none)\n")
    ...
    governed_client = ControlPlaneAdapter(
        control_plane=control_plane, agent_context=agent_context, original_client=original_client
    )
    ...
    for i, tool_call in enumerate(response.choices[0].message.tool_calls, 1):
        ...
        if tool_call.function.name == "blocked_action":
            print("     ⚠️  THIS ACTION WAS BLOCKED BY THE CONTROL PLANE")
        else:
            print("     ✓ This action was allowed")
        print()
...
def example_4_callback_on_block():
    ...
    def on_action_blocked(tool_name, tool_args, result):
        ...
        blocked_actions.append(
            {"tool": tool_name, "args": tool_args, "reason": result.get("error")}
        )

    control_plane = AgentControlPlane()
    agent_context = control_plane.create_agent(
        "demo-agent-4", {ActionType.DATABASE_QUERY: PermissionLevel.READ_ONLY}
    )
