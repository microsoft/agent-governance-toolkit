...
from agent_control_plane import AgentControlPlane, create_read_only_agent, create_standard_agent
...
def example_basic_usage():
    ...
    result = control_plane.execute_action(
        agent_context, ActionType.FILE_READ, {"path": "/data/sample.txt"}
    )
...
def example_permission_control():
    ...
    # This should succeed
    result = control_plane.execute_action(
        agent_context, ActionType.FILE_READ, {"path": "/data/public.txt"}
    )
    print(f"✓ File read (allowed): {result['success']}")

    # This should fail due to insufficient permissions
    result = control_plane.execute_action(
        agent_context, ActionType.FILE_WRITE, {"path": "/data/output.txt", "content": "test"}
    )
...
def example_rate_limiting():
    ...
    agent_context = control_plane.create_agent(
        "rate-limited-agent", {ActionType.FILE_READ: PermissionLevel.READ_ONLY}, custom_quota
    )

    print("✓ Created rate-limited agent")
    print(f"  Max requests per minute: {custom_quota.max_requests_per_minute}\n")

    # Make several requests
    for i in range(5):
        result = control_plane.execute_action(
            agent_context, ActionType.FILE_READ, {"path": f"/data/file{i}.txt"}
        )
...
def example_policy_enforcement():
    ...
    print("✓ Created agent with default security policies\n")

    # Try to access system files (should be blocked by policy)
    result = control_plane.execute_action(
        agent_context, ActionType.FILE_READ, {"path": "/etc/passwd"}
    )
    ...
    result = control_plane.execute_action(
        agent_context, ActionType.FILE_READ, {"path": "/data/user_file.txt"}
    )
...
def example_audit_logging():
    ...
    control_plane.execute_action(agent_context, ActionType.FILE_READ, {"path": "/data/file1.txt"})

    control_plane.execute_action(
        agent_context, ActionType.CODE_EXECUTION, {"code": "print('test')", "language": "python"}
    )

    # Retrieve audit log
    audit_log = control_plane.get_audit_log(limit=10)

    print("✓ Audit log entries (last 10):")
...
def example_risk_management():
    ...
    risk_policy = RiskPolicy(
        max_risk_score=0.5, deny_above=0.8, blocked_domains=["malicious.com", "dangerous.net"]
    )
    ...
    print("✓ Created agent with strict risk policy")
    print(f"  Max risk score: {risk_policy.max_risk_score}")
    print(f"  Deny above: {risk_policy.deny_above}\n")

    # Low risk action (file read)
    result = control_plane.execute_action(
        agent_context, ActionType.FILE_READ, {"path": "/data/safe.txt"}
    )
    print("✓ Low risk action (file read):")
    print(f"  Success: {result['success']}")
    print(f"  Risk score: {result.get('risk_score', 'N/A')}\n")
    ...
    print("✓ Higher risk action (code execution):")
    print(f"  Success: {result['success']}")
    print(f"  Risk score: {result.get('risk_score', 'N/A')}\n")
