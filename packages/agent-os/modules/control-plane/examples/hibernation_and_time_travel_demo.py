...
def example_agent_hibernation():
    ...
    control_plane = AgentControlPlane(
        enable_hibernation=True, hibernation_config=hibernation_config
    )
    ...
    result2 = control_plane.execute_action(
        agent_context, ActionType.API_CALL, {"url": "https://api.example.com/data", "method": "GET"}
    )
    ...
    print("✓ Agent hibernated successfully")
    ...
    print("✓ Agent woken successfully")
...
def example_time_travel_debugging():
    ...
    time_travel_config = TimeTravelConfig(
        enabled=True, enable_state_snapshots=True, snapshot_interval_seconds=10
    )

    control_plane = AgentControlPlane(
        enable_time_travel=True, time_travel_config=time_travel_config
    )
    ...
    actions = [
        ...
        (
            ...
            {"path": "/data/output.txt", "content": "Result data"},
        ),
    ]
    ...
    for idx, (name, action_type, params) in enumerate(actions, 1):
        ...
        control_plane.capture_agent_state_snapshot(
            agent_context.agent_id, agent_context, metadata={"action": name, "step": idx}
        )
    ...
    replay_session = control_plane.replay_agent_history(agent_context.agent_id, minutes=1)
    ...
    print("Step-by-step replay (press Enter to advance):")
    print()
...
def example_combined_features():
    ...
    control_plane = AgentControlPlane(
        ...
        hibernation_config=HibernationConfig(
            idle_timeout_seconds=3, storage_path="/tmp/combined_demo_hibernation"
        ),
        time_travel_config=TimeTravelConfig(enabled=True, enable_state_snapshots=True),
    )
    ...
    for i in range(3):
        ...
        control_plane.capture_agent_state_snapshot(
            agent_context.agent_id, agent_context, metadata={"iteration": i}
        )
        time.sleep(0.5)
    ...
    if hibernated:
        ...
        print("✓ Agent restored and ready for new work")
        print()
