...
from agent_control_plane.constraint_graphs import DataGraph, GraphNode, GraphNodeType, TemporalGraph
...
def use_case_1_data_analysis_pipeline():
    ...
    quota = ResourceQuota(
        agent_id="data-analyst", max_requests_per_minute=30, max_requests_per_hour=500
    )
    ...
    result2 = control_plane.execute_action(
        agent, ActionType.FILE_READ, {"path": "/data/reports/sales/monthly.csv"}
    )
...
def use_case_2_content_moderation():
    ...
    # Analyze content
    result1 = control_plane.execute_action(
        agent, ActionType.FILE_READ, {"path": "/content/submissions/post_12345.json"}
    )
...
def use_case_3_testing_in_shadow_mode():
    ...
    shadow_config = ShadowModeConfig(enabled=True, log_reasoning=True)
    ...
    for i, (action_type, params) in enumerate(actions, 1):
        ...
        print(
            f"  {i}. {action_type.value}: Simulated (would have {'succeeded' if result['success'] else 'failed'})"
        )
    ...
    print("  - No real side effects occurred")
    print("  - Ready to analyze agent behavior")
    print()
...
def use_case_4_multi_tenant_saas():
    ...
    for tenant in tenants:
        ...
        if tenant == "acme-corp":
            # Premium tier
            quota = ResourceQuota(
                agent_id=f"{tenant}-agent", max_requests_per_minute=100, max_requests_per_hour=5000
            )
        else:
            # Standard tier
            quota = ResourceQuota(
                agent_id=f"{tenant}-agent", max_requests_per_minute=30, max_requests_per_hour=1000
            )
    ...
    for tenant in tenants:
        ...
        if agent:
            result = control_plane.execute_action(
                agent, ActionType.FILE_READ, {"path": f"/data/tenants/{tenant}/config.json"}
            )
            quota_status = control_plane.policy_engine.get_quota_status(f"{tenant}-agent")
            print(
                f"  {tenant}: Action {'succeeded' if result['success'] else 'failed'}, "
