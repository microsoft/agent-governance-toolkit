...
def demo_basic_interception():
    ...
    print("Note: In shadow mode, NO actual execution happens!")
...
def demo_constraint_graph():
    ...
    policy.add_constraint(role="finance_agent", allowed_tools=["read_balance", "calculate_tax"])
    ...
    print("\n--- Test 1: ALLOWED Action ---")
    result = kernel.intercept_tool_execution(
        agent_id="finance_agent", tool_name="read_balance", tool_args={"account_id": "12345"}
    )
    print("Tool: read_balance")
    print(f"Result: {result}")
...
def demo_argument_validation():
    ...
    policy.add_constraint(role="data_agent", allowed_tools=["write_file", "read_file"])
    ...
    print("Tool: write_file (path=/data/report.txt)")
    print(f"Result: {result}")
    ...
    print("Tool: write_file (path=/etc/passwd)")
    print(f"Result: {result}")
    print(f"Status: {result['status']}")
    print(f"Error: {result['error']}")
...
def demo_scale_by_subtraction():
    ...
    policy.add_constraint(role="sql_agent", allowed_tools=["execute_query"])
    ...
    result = kernel.intercept_tool_execution(
        agent_id="sql_agent", tool_name="execute_delete", tool_args={"query": "DELETE FROM users"}
    )
    ...
    result = kernel.intercept_tool_execution(
        agent_id="sql_agent", tool_name="backup_database", tool_args={}
    )
    print(f"Result: {result['status'] if result else 'ALLOWED'}")
    if result:
        print(f"Reason: {result['error']}")
