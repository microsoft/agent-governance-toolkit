...
def example_mute_agent():
    ...
    result1 = control_plane.execute_action(
        agent_context, ActionType.DATABASE_QUERY, {"query": "SELECT * FROM users WHERE id = 1"}
    )
    ...
    result2 = control_plane.execute_action(
        agent_context, ActionType.DATABASE_QUERY, {"query": "DROP TABLE users"}
    )
    print("Test 2 - Invalid DROP TABLE query:")
    print(f"  Success: {result2['success']}")
    print(f"  Error: {result2.get('error', 'N/A')}")
    print("  ➜ Agent returns NULL instead of hallucinating!\n")

    # Out of scope request: File operations
    result3 = control_plane.execute_action(
        agent_context, ActionType.FILE_READ, {"path": "/data/file.txt"}
    )
    print("Test 3 - Out of scope (file read):")
    print(f"  Success: {result3['success']}")
    print(f"  Error: {result3.get('error', 'N/A')}")
    print("  ➜ Agent knows it can't do this, returns NULL!\n")
...
def example_constraint_graphs():
    ...
    control_plane.add_policy_constraint(
        "pii_protection", "No PII in output", applies_to=["table:users"], rule_type="deny"
    )
    control_plane.add_policy_constraint(
        "finance_approval",
    ...
    result1 = control_plane.execute_action(
        agent_context, ActionType.DATABASE_QUERY, {"table": "users", "query": "SELECT * FROM users"}
    )
    print(f"  Success: {result1['success']}")
    ...
    if not result2["success"]:
        ...
        print("  ➜ Constraint Graph blocked access!\n")
    ...
    if not result3["success"]:
        ...
        print("  ➜ Path not in accessible data graph!\n")
...
def example_supervisor_agents():
    ...
    for i in range(3):
        control_plane.execute_action(agent1, ActionType.FILE_READ, {"path": f"/data/file{i}.txt"})
...
def example_integrated_workflow():
    ...
    result = control_plane.execute_action(
        agent, ActionType.DATABASE_QUERY, {"table": "customers", "query": "SELECT * FROM customers"}
    )
    print(f"  Success: {result['success']}")
    print(f"  Status: {result.get('status')}")
    ...
    print(f"  Success: {result2['success']}")
    print("  Status: Production execution\n")
