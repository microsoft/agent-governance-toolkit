...
def example_vision_analysis():
    ...
    # Analyze image
    result = vision.analyze_image(image=image, prompt="Describe what you see in this image")
...
def example_vector_store():
    ...
    vector_store = VectorStoreIntegration(
        store_type=VectorStoreType.IN_MEMORY, collection_name="knowledge_base"
    )
...
def example_rag_pipeline():
    ...
    vector_store = VectorStoreIntegration(
        store_type=VectorStoreType.IN_MEMORY, collection_name="ai_knowledge"
    )
    ...
    result = rag.query(query_text=query, query_embedding=query_embedding, top_k=2)
...
def example_multimodal_input():
    ...
    from agent_control_plane.multimodal import MultimodalInput
    ...
    mm_input = MultimodalInput(
        ...
        images=[
            ImageInput(
                image_data="medical_scan_image", format=ImageFormat.PNG, metadata={"type": "x-ray"}
            )
        ],
        audio=[
            AudioInput(
                audio_data="doctor_notes_audio", format=AudioFormat.WAV, duration_seconds=120.0
            )
        ],
    )
...
def example_rag_with_metadata_filtering():
    ...
    vector_store = VectorStoreIntegration(store_type=VectorStoreType.IN_MEMORY)
    ...
    results = vector_store.search(
        query_embedding=query_embedding, top_k=5, filter_metadata={"category": "technical"}
    )
...
def example_integrated_multimodal():
    ...
    print("  ✅ Vision Capability")
    print("  ✅ Audio Capability")
