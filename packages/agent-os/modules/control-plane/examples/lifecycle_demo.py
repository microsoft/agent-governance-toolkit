...
class ClaimsVerificationAgent:
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {"processed_claims": self._processed_claims, "custom_state": self._state.copy()}
...
async def demo_basic_lifecycle():
    ...
    control_plane.register(
        ...
        resources=AgentResourceQuota(  # Resource limits
            memory_mb=512, cpu_percent=25, max_concurrent_operations=10
        ),
    )
...
async def demo_scaling():
    ...
    control_plane.register(
        ClaimsVerificationAgent,
        replicas=1,
        resources=AgentResourceQuota(memory_mb=256, cpu_percent=20),
    )
...
async def demo_dependency_graph():
    ...
    control_plane.register(
        ClaimsVerificationAgent, agent_id="claims-agent", dependencies=["database", "message-bus"]
    )
...
async def demo_graceful_shutdown():
    ...
    op1 = control_plane.shutdown_manager.register_operation(
        agent_id="claims-agent", operation_type="verification", data={"claim_id": "CLM-001"}
    )
    op2 = control_plane.shutdown_manager.register_operation(
        agent_id="claims-agent", operation_type="verification", data={"claim_id": "CLM-002"}
    )
...
async def demo_observability():
    ...
    control_plane.observability.increment_counter(
        "claims-agent", "requests_total", labels={"status": "success"}
    )
    control_plane.observability.increment_counter(
        "claims-agent", "requests_total", labels={"status": "success"}
    )
    control_plane.observability.increment_counter(
        "claims-agent", "requests_total", labels={"status": "failure"}
    )

    control_plane.observability.set_gauge("claims-agent", "active_connections", 5.0)

    control_plane.observability.observe_histogram("claims-agent", "request_latency_ms", 150.0)
    control_plane.observability.observe_histogram("claims-agent", "request_latency_ms", 200.0)

    # Log some messages
    control_plane.observability.log("claims-agent", "info", "Processing claim CLM-001")
    control_plane.observability.log(
        "claims-agent", "warning", "High latency detected", context={"latency_ms": 500}
    )
...
async def demo_resource_quotas():
    ...
    control_plane.register(
        ...
        resources=AgentResourceQuota(
            memory_mb=512, cpu_percent=25, max_concurrent_operations=3, max_operations_per_minute=10
        ),
    )
...
async def demo_full_integration():
    ...
    control_plane = create_control_plane(
        ...
        auto_recovery=True,
        circuit_breaker=CircuitBreaker(name="default", failure_threshold=5, recovery_timeout=30.0),
    )
    ...
    control_plane.register(
        ...
        resources=AgentResourceQuota(memory_mb=512, cpu_percent=25, max_concurrent_operations=10),
    )
    ...
    for i in range(5):
        ...
        if agent:
            control_plane.observability.increment_counter("claims-verifier", "requests_total")
