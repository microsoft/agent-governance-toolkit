...
def example_prometheus_metrics():
    ...
    exporter.increment_counter(
        "agent_requests_total", value=1, labels={"agent_id": "agent1", "status": "blocked"}
    )
...
def example_alerting():
    ...
    for scenario in test_scenarios:
        ...
        print(
            f"  Metrics: error_rate={scenario['error_rate']:.2f}, "
        ...
        if alerts:
            ...
            for alert in alerts:
                ...
                print(
                    f"     {severity_icon[alert.severity]} [{alert.severity.value}] {alert.message}"
                )
        else:
            print("  ✅ No alerts")
        print()
...
def example_distributed_tracing():
    ...
    trace_id = collector.start_trace(
        "agent_request", metadata={"user_id": "user123", "request_type": "tool_execution"}
    )
    ...
    policy_span = collector.start_span(
        trace_id=trace_id, operation_name="policy_check", tags={"policy": "rate_limit"}
    )
    time.sleep(0.01)
    collector.end_span(trace_id, policy_span, tags={"result": "allowed"})

    execution_span = collector.start_span(
        trace_id=trace_id, operation_name="tool_execution", tags={"tool": "read_file"}
    )
...
def example_observability_dashboard():
    ...
    alert_mgr.add_rule("test_alert", lambda m: False, AlertSeverity.INFO, "Test")
    ...
    status_icon = {"healthy": "🟢", "warning": "🟡", "degraded": "🟠", "critical": "🔴"}
...
def example_monitoring_workflow():
    ...
    prometheus.increment_counter("agent_requests_total", labels={"status": "processing"})
    ...
    prometheus.observe_histogram("agent_request_duration_ms", value=trace.duration_ms)

    # Update counter
    prometheus.increment_counter("agent_requests_total", labels={"status": "success"})

    # Check alerts
    current_metrics = {"error_rate": 0.01, "avg_latency_ms": trace.duration_ms}
...
def example_metrics_export_endpoint():
    ...
    exporter.increment_counter(
        "http_requests_total", value=1250, labels={"method": "POST", "endpoint": "/api/agent"}
    )
    exporter.increment_counter(
        "http_requests_total", value=3200, labels={"method": "GET", "endpoint": "/api/agent"}
    )
