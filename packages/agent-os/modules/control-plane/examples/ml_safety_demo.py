...
from agent_control_plane import AnomalyDetector, JailbreakDetector, create_ml_safety_suite
...
def example_anomaly_detection():
    ...
    for i in range(10):
        detector.record_behavior("agent1", {"type": "read", "resource": f"file{i % 3}.txt"})

    # Test normal behavior
    result = detector.detect_anomaly("agent1", {"type": "read", "resource": "file1.txt"})
    print(f"Normal behavior - Anomalous: {result.is_threat}")
    print(f"  Score: {result.confidence:.2f}")
    print()

    # Test anomalous behavior (novel action)
    result = detector.detect_anomaly("agent1", {"type": "delete", "resource": "important_file.txt"})
...
def example_behavioral_analysis():
    ...
    print("Prompt with suspicious behavior pattern:")
