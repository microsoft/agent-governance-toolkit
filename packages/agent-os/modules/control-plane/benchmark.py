...
from benchmark.red_team_dataset import RedTeamPrompt, get_all_prompts, get_dataset_stats
...
class ControlPlaneAgent:
    ...
    def __init__(self):
        ...
        self.agent_context = self.control_plane.create_agent("benchmark-sql-agent", permissions)
    ...
    def process_request(self, prompt: str) -> tuple[bool, str, int]:
        ...
        result = self.control_plane.execute_action(self.agent_context, action_type, parameters)
    ...
    def _classify_prompt(self, prompt: str) -> tuple[ActionType, Dict[str, Any]]:
        ...
        # Detect SQL queries
        if any(
            sql_kw in prompt_upper
            for sql_kw in ["SELECT", "DROP", "DELETE", "UPDATE", "INSERT", "TRUNCATE", "ALTER"]
        ):
...
def run_benchmark(
    prompts: List[RedTeamPrompt], verbose: bool = False
...
def print_metrics_comparison(baseline_metrics: AggregateMetrics, cp_metrics: AggregateMetrics):
    ...
    # Token Efficiency
    print(
        f"{'Avg Output Tokens per Request':<40} "
        f"{baseline_metrics.avg_output_tokens:>6.1f} "
        f"{' ' * 8} {cp_metrics.avg_output_tokens:>6.1f}"
    )

    token_reduction = (
        (baseline_metrics.avg_output_tokens - cp_metrics.avg_output_tokens)
        / baseline_metrics.avg_output_tokens
        * 100
    )
    print(f"{'  → Token Reduction':<40} {' ' * 6} {' ' * 8} {token_reduction:>6.1f}%")
    ...
    if cp_metrics.safety_violation_rate == 0:
        print("✓ Control Plane achieved ZERO safety violations (100% enforcement)")
