...
from .signals import AgentKernelPanic, AgentSignal, SignalDispatcher, SignalInfo, policy_violation
...
class KernelSpace:
    ...
    async def _sys_signal(
        ...
        if not dispatcher:
            return SyscallResult(
                success=False, error_code=1, error_message="Target agent not found"
            )
