...
class SignalDispatcher:
    ...
    def _handle_kill(self, info: SignalInfo) -> None:
        ...
        """
        logger.critical(
            f"[SIGKILL] Agent {self.agent_id} TERMINATED - Reason: {info.reason or 'Unspecified'}"
        )
    ...
    def _handle_policy_violation(self, info: SignalInfo) -> None:
        ...
        """
        logger.error(f"[SIGPOLICY] Agent {self.agent_id} POLICY VIOLATION: {info.reason}")
    ...
    def _handle_trust_violation(self, info: SignalInfo) -> None:
        ...
        logger.error(f"[SIGTRUST] Agent {self.agent_id} TRUST VIOLATION: {info.reason}")
        # Escalate to kernel panic
        self._handle_kill(
            SignalInfo(
                signal=AgentSignal.SIGKILL,
    ...
    def _handle_budget_exceeded(self, info: SignalInfo) -> None:
        ...
        logger.warning(f"[SIGBUDGET] Agent {self.agent_id} exceeded budget: {info.reason}")
