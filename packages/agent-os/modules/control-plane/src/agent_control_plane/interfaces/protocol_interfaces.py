...
class MessageSecurityInterface(ABC):
    ...
    def register_agent_keys(
        self, agent_id: str, public_key: bytes, key_type: str = "ed25519"
    ) -> bool:
        """
...
class VerificationInterface(ABC):
    ...
    @abstractmethod
    def create_proof(
        self, data: Any, verification_type: VerificationType = VerificationType.SIGNATURE
    ) -> bytes:
        """
    ...
    @abstractmethod
    def verify_chain(self, data_chain: List[Any], proofs: List[bytes]) -> List[VerificationResult]:
        """
