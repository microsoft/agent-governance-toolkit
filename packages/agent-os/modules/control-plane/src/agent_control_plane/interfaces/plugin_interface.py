...
class ValidatorInterface(PluginInterface[ValidationResult]):
    ...
    def validate_parameters(self, action_type: str, parameters: Dict[str, Any]) -> ValidationResult:
        """
...
class ExecutorInterface(PluginInterface[ExecutionResult]):
    ...
    @abstractmethod
    def execute(self, request: Any, context: Optional[Dict[str, Any]] = None) -> ExecutionResult:
        """
...
class ContextRouterInterface(PluginInterface[RoutingDecision]):
    ...
    @abstractmethod
    def route(
        self, request: Any, available_targets: List[str], context: Optional[Dict[str, Any]] = None
    ) -> RoutingDecision:
        """
    ...
    def register_target(
        self, target_id: str, capabilities: List[str], metadata: Optional[Dict[str, Any]] = None
    ) -> None:
...
class PolicyProviderInterface(PluginInterface[PolicyDefinition]):
    ...
    @abstractmethod
    def get_policies(
        self, agent_id: Optional[str] = None, action_type: Optional[str] = None
...
class SupervisorInterface(PluginInterface[ValidationResult]):
    ...
    @abstractmethod
    def supervise(
        self, execution_log: List[Dict[str, Any]], audit_log: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
