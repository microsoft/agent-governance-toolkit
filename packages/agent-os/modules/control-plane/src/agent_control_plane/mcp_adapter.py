...
class MCPAdapter:
    ...
    def __init__(
        ...
        self.logger.info(f"Initialized MCPAdapter for agent {agent_context.agent_id}")
    ...
    def _handle_tools_call(self, params: Dict) -> Dict:
        ...
        check_result = self.control_plane.execute_action(self.agent_context, action_type, arguments)
        ...
        return {"content": [{"type": "text", "text": json.dumps(check_result.get("result", {}))}]}
    ...
    def _handle_resources_read(self, params: Dict) -> Dict:
        ...
        check_result = self.control_plane.execute_action(
            self.agent_context, action_type, {"uri": uri}
        )
    ...
    def _handle_prompts_get(self, params: Dict) -> Dict:
        ...
        return {"messages": [], "description": f"Prompt: {prompt_name}"}
    ...
    def _map_tool_to_action(self, tool_name: str) -> Optional[ActionType]:
        ...
        # Pattern matching
        if any(p in tool_name_lower for p in ["read", "get", "fetch", "load"]) and any(
            p in tool_name_lower for p in ["file", "document"]
        ):
            return ActionType.FILE_READ

        if any(p in tool_name_lower for p in ["write", "save", "create", "update"]) and any(
            p in tool_name_lower for p in ["file", "document"]
        ):
    ...
    def _check_permission(self, action_type: ActionType, parameters: Dict) -> bool:
        ...
        check_result = self.control_plane.execute_action(
            self.agent_context, action_type, parameters
        )
...
class MCPServer:
    ...
    def __init__(
        ...
        self.adapter = MCPAdapter(
            control_plane=control_plane, agent_context=agent_context, logger=self.logger
        )
