...
class MuteAgentValidator(CapabilityValidatorInterface):
    ...
    def validate_request(
        ...
        matching_capabilities = [
            cap for cap in self.config.capabilities if action_type in cap.action_types
        ]

        if not matching_capabilities:
            reason = self._format_rejection_reason(
                action_type, "Action type not in agent capabilities"
            )
        ...
        for capability in matching_capabilities:
            if capability.validator and hasattr(request, "action_type"):
                if not capability.validator(request):
                    reason = self._format_rejection_reason(
                        action_type, f"Parameters do not match capability: {capability.name}"
                    )
                    self._log_rejection(request_id, action_type, reason, timestamp)
                    return ValidationResult(
                        is_valid=False, reason=reason, details={"capability": capability.name}
                    )
    ...
    def _log_rejection(
        self, request_id: str, action_type: ActionType, reason: str, timestamp: datetime
    ):
