...
class ChatCompletionsWrapper:
    ...
    def create(self, **kwargs) -> Any:
        ...
        for tool_call in choice.message.tool_calls:
            ...
            check_result = self.control_plane.execute_action(
                self.agent_context, action_type, tool_args
            )
    ...
    def _map_tool_to_action(self, tool_name: str) -> Optional[ActionType]:
        ...
        if any(pattern in tool_name_lower for pattern in ["read", "get", "fetch", "load"]) and any(
            pattern in tool_name_lower for pattern in ["file", "document"]
        ):
            return ActionType.FILE_READ

        if any(
            pattern in tool_name_lower for pattern in ["write", "save", "create", "update"]
        ) and any(pattern in tool_name_lower for pattern in ["file", "document"]):
            return ActionType.FILE_WRITE

        if any(
            pattern in tool_name_lower for pattern in ["exec", "run", "execute", "eval"]
        ) and any(
            pattern in tool_name_lower
            for pattern in ["code", "python", "script", "command", "bash"]
        ):
            return ActionType.CODE_EXECUTION

        if any(pattern in tool_name_lower for pattern in ["select", "query", "search"]) and any(
            pattern in tool_name_lower for pattern in ["database", "db", "sql", "table"]
        ):
            return ActionType.DATABASE_QUERY

        if any(
            pattern in tool_name_lower
            for pattern in ["insert", "update", "delete", "drop", "create", "alter"]
        ) and any(pattern in tool_name_lower for pattern in ["database", "db", "sql", "table"]):
            return ActionType.DATABASE_WRITE

        if any(
            pattern in tool_name_lower
            for pattern in ["api", "http", "request", "call", "post", "get"]
        ):
...
class ChatWrapper:
    ...
    def __init__(
        ...
        self.completions = ChatCompletionsWrapper(
            original_chat.completions, control_plane, agent_context, tool_mapping, logger, on_block
        )
...
class ControlPlaneAdapter:
    ...
    def __init__(
        ...
        self.logger.info(f"Initialized ControlPlaneAdapter for agent {agent_context.agent_id}")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "execution_history": self.control_plane.get_execution_history(
                agent_id=self.agent_context.agent_id, limit=100
            ),
        }
