...
class PolicyGraph(ConstraintGraph):
    ...
    def add_policy_rule(
        ...
        node = GraphNode(
            ...
            name=name,
            properties={"rule_type": rule_type, "applies_to": applies_to, "validator": validator},
        )
        self.add_node(node)

        # Create edges to resources this policy applies to
        for resource_id in applies_to:
            edge = GraphEdge(from_node=node.node_id, to_node=resource_id, edge_type=rule_type)
            self.add_edge(edge)
    ...
    def check_policy_violations(self, agent_role: str, resource_id: str) -> List[str]:
        ...
        for edge in self.get_edges_to(resource_id):
            if edge.edge_type == "deny":
                ...
                violations.append(
                    f"Policy '{policy_node.name}' blocks access to resource '{resource_id}' for role '{agent_role}'"
                )
...
class TemporalGraph(ConstraintGraph):
    ...
    def add_maintenance_window(
        self, window_id: str, start_time: time, end_time: time, blocked_actions: List[ActionType]
    ):
    ...
    def add_business_hours(
        self, hours_id: str, start_time: time, end_time: time, required_for: List[ActionType]
    ):
    ...
    def add_freeze_period(
        self, freeze_id: str, start_date: datetime, end_date: datetime, reason: str
    ):
        ...
        node = GraphNode(
            ...
            properties={"start_date": start_date, "end_date": end_date, "reason": reason},
        )
        self.add_node(node)
...
class ConstraintGraphValidator:
    ...
    def __init__(
        self, data_graph: DataGraph, policy_graph: PolicyGraph, temporal_graph: TemporalGraph
    ):
    ...
    def _validate_policy_graph(self, request: ExecutionRequest) -> Tuple[bool, List[str]]:
        ...
        if resource_id:
            policy_violations = self.policy_graph.check_policy_violations(
                request.agent_context.agent_id, resource_id
            )
