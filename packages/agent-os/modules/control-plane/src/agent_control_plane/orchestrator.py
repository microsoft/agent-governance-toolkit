...
class AgentOrchestrator:
    ...
    def register_agent(
        ...
        agent = AgentNode(
            agent_id=agent_id, role=role, capabilities=capabilities or [], metadata=metadata or {}
        )
    ...
    def create_workflow(
        self, name: str, orchestration_type: OrchestrationType = OrchestrationType.SEQUENTIAL
    ) -> WorkflowState:
        """
        ...
        workflow = WorkflowState(
            workflow_id=workflow_id, status="pending", agents={}, messages=[], results={}, errors=[]
        )
    ...
    def add_agent_to_workflow(
        self, workflow_id: str, agent_id: str, dependencies: Optional[Set[str]] = None
    ) -> bool:
        """
    ...
    def add_supervisor(self, supervisor_id: str, watched_agents: List[str]):
        """
    ...
    async def execute_workflow(
        self, workflow_id: str, input_data: Dict[str, Any], timeout: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        Execute a workflow.
        ...
        try:
            ...
            return {"success": True, "results": results, "workflow_id": workflow_id}
    ...
    async def _execute_agents_in_order(
        self, workflow: WorkflowState, input_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def list_workflows(self) -> List[Dict[str, Any]]:
        """List all workflows"""
        return [
            {"workflow_id": wid, "status": w.status, "agent_count": len(w.agents)}
...
def create_rag_pipeline(orchestrator: AgentOrchestrator) -> str:
    ...
    orchestrator.register_agent(
        "retriever", AgentRole.SPECIALIST, capabilities=["document_search", "vector_search"]
    )

    orchestrator.register_agent(
        "reasoner", AgentRole.SPECIALIST, capabilities=["analysis", "generation"]
    )

    orchestrator.register_agent(
        "validator", AgentRole.SUPERVISOR, capabilities=["quality_check", "safety_check"]
    )
    ...
    orchestrator.add_agent_to_workflow(workflow.workflow_id, "reasoner", dependencies={"retriever"})
