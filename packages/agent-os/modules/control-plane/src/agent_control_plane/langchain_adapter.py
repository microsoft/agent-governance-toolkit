...
class LangChainAdapter:
    ...
    def __init__(
        ...
        self.logger.info(f"Initialized LangChainAdapter for agent {agent_context.agent_id}")
    ...
    def _check_tool_call(self, tool_call: Dict):
        ...
        if action_type is None:
            ...
            raise PermissionError(
                f"Unknown tool: {tool_name}. Tool must be mapped to an ActionType."
            )
        ...
        check_result = self.control_plane.execute_action(self.agent_context, action_type, tool_args)
    ...
    def _map_tool_to_action(self, tool_name: str) -> Optional[ActionType]:
        ...
        if any(
            pattern in tool_name_lower
            for pattern in ["api", "http", "request", "search", "serpapi", "google", "wikipedia"]
        ):
