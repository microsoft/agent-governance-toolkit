...
class HibernationManager:
    ...
    def __init__(self, config: Optional[HibernationConfig] = None):
        ...
        self.logger.info(
            f"HibernationManager initialized with storage at {self.config.storage_path}"
        )
    ...
    def hibernate_agent(
        ...
        try:
            ...
            self.logger.info(
                f"Agent {agent_id} hibernated successfully. State saved to {file_path} ({state_size} bytes)"
            )
    ...
    def record_agent_activity(self, agent_id: str):
        ...
        self.agent_activity[agent_id] = datetime.now()
        if (
            agent_id not in self.agent_states
            or self.agent_states[agent_id] == AgentState.HIBERNATED
        ):
            self.agent_states[agent_id] = AgentState.ACTIVE
