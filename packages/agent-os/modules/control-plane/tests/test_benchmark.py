...
class TestRedTeamDataset(unittest.TestCase):
    ...
    def test_direct_violations_all_blocked(self):
        ...
        for prompt in prompts:
            self.assertTrue(
                prompt.expected_blocked, f"Direct violation should be blocked: {prompt.prompt}"
            )
    ...
    def test_valid_requests_all_allowed(self):
        ...
        for prompt in prompts:
            self.assertFalse(
                prompt.expected_blocked, f"Valid request should be allowed: {prompt.prompt}"
            )
    ...
    def test_prompt_injection_all_blocked(self):
        ...
        for prompt in prompts:
            self.assertTrue(
                prompt.expected_blocked, f"Prompt injection should be blocked: {prompt.prompt}"
            )
    ...
    def test_contextual_confusion_all_blocked(self):
        ...
        for prompt in prompts:
            self.assertTrue(
                prompt.expected_blocked, f"Contextual confusion should be blocked: {prompt.prompt}"
            )
    ...
    def test_jailbreak_diversity(self):
        ...
        self.assertGreaterEqual(
            len(unique_strategies), 3, "Jailbreaks should use diverse strategies"
        )
