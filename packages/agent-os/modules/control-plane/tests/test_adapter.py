...
class TestOpenAIAdapter(unittest.TestCase):
    ...
    def test_adapter_blocks_unauthorized_action(self):
        ...
        tool_calls = [
            MockOpenAIClient.MockToolCall(
                "write_file", json.dumps({"path": "/test.txt", "content": "data"})
            )
        ]
        ...
        governed = ControlPlaneAdapter(
            control_plane=control_plane, agent_context=agent_context, original_client=mock_client
        )

        # Make API call
        response = governed.chat.completions.create(
            model="gpt-4", messages=[{"role": "user", "content": "Write a file"}]
        )
    ...
    def test_adapter_allows_authorized_action(self):
        ...
        tool_calls = [MockOpenAIClient.MockToolCall("read_file", json.dumps({"path": "/test.txt"}))]
        mock_client = MockOpenAIClient(tool_calls)
        ...
        # Make API call
        response = governed.chat.completions.create(
            model="gpt-4", messages=[{"role": "user", "content": "Read a file"}]
        )
    ...
    def test_adapter_mixed_permissions(self):
        ...
        tool_calls = [
            MockOpenAIClient.MockToolCall(
                "database_query", json.dumps({"query": "SELECT * FROM users"})
            ),
            MockOpenAIClient.MockToolCall(
                "database_write", json.dumps({"query": "DELETE FROM users"})
            ),
        ]
        mock_client = MockOpenAIClient(tool_calls)
        ...
        # Make API call
        response = governed.chat.completions.create(
            model="gpt-4", messages=[{"role": "user", "content": "Query and delete"}]
        )
    ...
    def test_custom_tool_mapping(self):
        ...
        tool_calls = [
            MockOpenAIClient.MockToolCall("my_custom_db_tool", json.dumps({"query": "SELECT 1"}))
        ]
        mock_client = MockOpenAIClient(tool_calls)
    ...
    def test_on_block_callback(self):
        ...
        def on_block(tool_name, tool_args, result):
            blocked_calls.append({"tool": tool_name, "args": tool_args, "result": result})

        tool_calls = [
            MockOpenAIClient.MockToolCall("write_file", json.dumps({"path": "/test.txt"}))
        ]
        mock_client = MockOpenAIClient(tool_calls)
    ...
    def test_create_governed_client_convenience(self):
        ...
        tool_calls = [
            MockOpenAIClient.MockToolCall("database_query", json.dumps({"query": "SELECT 1"}))
        ]
        mock_client = MockOpenAIClient(tool_calls)
    ...
    def test_adapter_statistics(self):
        ...
        for _ in range(3):
            governed.chat.completions.create(
                model="gpt-4", messages=[{"role": "user", "content": "Read"}]
            )
    ...
    def test_pattern_matching_tool_names(self):
        ...
        for tool_name in tool_names:
            tool_calls = [
                MockOpenAIClient.MockToolCall(tool_name, json.dumps({"path": "/test.txt"}))
            ]
            mock_client = MockOpenAIClient(tool_calls)
            ...
            tool_call = response.choices[0].message.tool_calls[0]
            self.assertEqual(
                tool_call.function.name, tool_name, f"Pattern matching failed for {tool_name}"
            )
