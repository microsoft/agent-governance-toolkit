...
class TestAgentKernel(unittest.TestCase):
    ...
    def test_permission_check_success(self):
        ...
        result = self.control_plane.execute_action(
            agent_context, ActionType.FILE_READ, {"path": "/data/test.txt"}
        )
    ...
    def test_permission_check_failure(self):
        ...
        result = self.control_plane.execute_action(
            agent_context, ActionType.FILE_WRITE, {"path": "/data/test.txt", "content": "test"}
        )
...
class TestPolicyEngine(unittest.TestCase):
    ...
    def test_rate_limiting(self):
        ...
        agent_context = self.control_plane.create_agent(
            "rate-test-agent", {ActionType.FILE_READ: PermissionLevel.READ_ONLY}, quota
        )

        # First two requests should succeed
        result1 = self.control_plane.execute_action(
            agent_context, ActionType.FILE_READ, {"path": "/data/file1.txt"}
        )
        self.assertTrue(result1["success"])

        result2 = self.control_plane.execute_action(
            agent_context, ActionType.FILE_READ, {"path": "/data/file2.txt"}
        )
        self.assertTrue(result2["success"])

        # Third request should fail due to rate limit
        result3 = self.control_plane.execute_action(
            agent_context, ActionType.FILE_READ, {"path": "/data/file3.txt"}
        )
...
class TestControlPlane(unittest.TestCase):
    ...
    def test_end_to_end_workflow(self):
        ...
        # Execute action
        result = self.control_plane.execute_action(
            agent_context, ActionType.FILE_READ, {"path": "/data/workflow.txt"}
        )
        self.assertTrue(result["success"])
    ...
    def test_multiple_agents_isolation(self):
        ...
        result1 = self.control_plane.execute_action(
            agent1, ActionType.FILE_READ, {"path": "/data/agent1.txt"}
        )

        result2 = self.control_plane.execute_action(
            agent2, ActionType.FILE_READ, {"path": "/data/agent2.txt"}
        )
