...
class TestVisionCapability(unittest.TestCase):
    ...
    def test_analyze_safe_image(self):
        ...
        image = ImageInput(image_data="base64_encoded_image_data", format=ImageFormat.PNG)

        result = self.vision.analyze_image(image=image, prompt="Describe this image")
    ...
    def test_check_image_safety_pass(self):
        ...
        image = ImageInput(image_data="small_image_data", format=ImageFormat.JPEG)
    ...
    def test_check_image_safety_nsfw(self):
        ...
        image = ImageInput(image_data="image_data", format=ImageFormat.PNG, metadata={"nsfw": True})
...
class TestAudioCapability(unittest.TestCase):
    ...
    def test_transcribe_audio(self):
        ...
        audio = AudioInput(
            audio_data="base64_encoded_audio", format=AudioFormat.MP3, duration_seconds=30.0
        )
    ...
    def test_check_audio_safety_pass(self):
        ...
        audio = AudioInput(
            audio_data="small_audio_data", format=AudioFormat.WAV, duration_seconds=60.0
        )
    ...
    def test_check_audio_safety_too_long(self):
        ...
        audio = AudioInput(audio_data="audio_data", format=AudioFormat.MP3, duration_seconds=400.0)
...
class TestVectorStoreIntegration(unittest.TestCase):
    ...
    def setUp(self):
        self.vector_store = VectorStoreIntegration(
            store_type=VectorStoreType.IN_MEMORY, collection_name="test_collection"
        )
    ...
    def test_add_documents(self):
        ...
        docs = [
            VectorDocument(
                doc_id="doc1", content="AI safety is important", embedding=[0.1, 0.2, 0.3]
            ),
            VectorDocument(
                doc_id="doc2", content="Governance is crucial", embedding=[0.4, 0.5, 0.6]
            ),
        ]
    ...
    def test_search_documents(self):
        ...
        # Add documents
        docs = [
            VectorDocument(doc_id="doc1", content="AI safety", embedding=[0.1, 0.2, 0.3]),
            VectorDocument(doc_id="doc2", content="Machine learning", embedding=[0.9, 0.8, 0.7]),
        ]
        self.vector_store.add_documents(docs)

        # Search
        results = self.vector_store.search(query_embedding=[0.1, 0.2, 0.3], top_k=2)
    ...
    def test_delete_documents(self):
        ...
        docs = [VectorDocument(doc_id="doc1", content="Test", embedding=[0.1, 0.2, 0.3])]
        self.vector_store.add_documents(docs)
    ...
    def test_get_document(self):
        ...
        doc = VectorDocument(doc_id="doc1", content="Test content", embedding=[0.1, 0.2, 0.3])
    ...
    def test_list_documents(self):
        ...
        docs = [
            VectorDocument(doc_id=f"doc{i}", content=f"Content {i}", embedding=[0.1, 0.2, 0.3])
            for i in range(5)
        ]
        self.vector_store.add_documents(docs)
...
class TestRAGPipeline(unittest.TestCase):
    ...
    def setUp(self):
        self.vector_store = VectorStoreIntegration(store_type=VectorStoreType.IN_MEMORY)
    ...
    def test_query_with_retrieval(self):
        ...
        result = self.rag.query(
            query_text="What is AI safety?", query_embedding=[0.1, 0.2, 0.3], top_k=2
        )
    ...
    def test_rag_prompt_format(self):
        ...
        docs = [VectorDocument(doc_id="doc1", content="Test content", embedding=[0.1, 0.2, 0.3])]
        self.vector_store.add_documents(docs)

        result = self.rag.query(query_text="Test query", query_embedding=[0.1, 0.2, 0.3])
