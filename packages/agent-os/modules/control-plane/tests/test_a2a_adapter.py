...
class TestA2AAdapter(unittest.TestCase):
    ...
    def test_create_adapter(self):
        ...
        agent_card = {"name": "Test Agent", "capabilities": []}

        adapter = A2AAdapter(
            control_plane=control_plane, agent_context=agent_context, agent_card=agent_card
        )
    ...
    def test_register_capability(self):
        ...
        adapter = A2AAdapter(
            control_plane=control_plane, agent_context=agent_context, agent_card={"name": "Test"}
        )
    ...
    def test_discovery_message(self):
        ...
        message = {"id": "msg-1", "type": "discovery", "from": "other-agent", "payload": {}}
    ...
    def test_handshake_message(self):
        ...
        message = {
            ...
            "type": "handshake",
            "from": "peer-agent",
            "payload": {"agent_card": {"name": "Peer Agent"}},
        }
    ...
    def test_task_delegation(self):
        ...
        message = {
            ...
            "from": "delegator",
            "payload": {"target_agent": "specialist", "task_type": "analysis", "parameters": {}},
        }
    ...
    def test_query_message(self):
        ...
        message = {
            ...
            "type": "query",
            "from": "querier",
            "payload": {"query_type": "database_query", "parameters": {"table": "users"}},
        }
    ...
    def test_send_task_request(self):
        ...
        a2a_agent = A2AAgent(
            agent_id="sender", control_plane=control_plane, agent_context=agent_context
        )

        message = a2a_agent.send_task_request(
            to_agent="receiver", task_type="process_data", parameters={"data": [1, 2, 3]}
        )
    ...
    def test_map_capability_to_action(self):
        ...
        adapter = A2AAdapter(
            control_plane=control_plane, agent_context=agent_context, agent_card={"name": "Mapper"}
        )

        # Test various capability names
        self.assertEqual(adapter._map_capability_to_action("read_file"), ActionType.FILE_READ)
        self.assertEqual(
            adapter._map_capability_to_action("execute_task"), ActionType.CODE_EXECUTION
        )
        self.assertEqual(
            adapter._map_capability_to_action("database_query"), ActionType.DATABASE_QUERY
        )
    ...
    def test_negotiate_message(self):
        ...
        message = {
            ...
            "type": "negotiate",
            "from": "negotiator",
            "payload": {"parameters": {"timeout": 30, "priority": "high"}},
        }
