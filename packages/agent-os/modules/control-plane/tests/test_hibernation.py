...
class TestHibernationManager:
    ...
    def test_initialization(self):
        ...
        config = HibernationConfig(enabled=True, storage_path=tempfile.mkdtemp())
    ...
    def test_hibernate_and_wake_json(self):
        ...
        config = HibernationConfig(storage_path=tempfile.mkdtemp(), format=HibernationFormat.JSON)
        manager = HibernationManager(config)
        ...
        metadata = manager.hibernate_agent(
            "test-agent", agent_context, caas_pointer="context://test"
        )
        ...
        # Verify file is JSON
        with open(metadata.state_file_path) as f:
            data = json.load(f)
    ...
    def test_idle_detection(self):
        ...
        config = HibernationConfig(storage_path=tempfile.mkdtemp(), idle_timeout_seconds=1)
        manager = HibernationManager(config)
...
class TestControlPlaneHibernation:
    ...
    def test_enable_hibernation(self):
        ...
        cp = AgentControlPlane(enable_hibernation=True, hibernation_config=config)
    ...
    def test_hibernate_agent_via_control_plane(self):
        ...
        metadata = cp.hibernate_agent(
            agent_context.agent_id, agent_context, caas_pointer="context://test"
        )
