...
class TestToolInterception(unittest.TestCase):
    ...
    def test_shadow_mode_intercepts_all_tools(self):
        ...
        result = kernel.intercept_tool_execution(
            agent_id="test-agent", tool_name="dangerous_tool", tool_args={"action": "delete_all"}
        )
    ...
    def test_no_policy_allows_execution(self):
        ...
        result = kernel.intercept_tool_execution(
            agent_id="test-agent", tool_name="any_tool", tool_args={}
        )
    ...
    def test_blocked_tool_returns_mute_response(self):
        ...
        result = kernel.intercept_tool_execution(
            agent_id="restricted-agent", tool_name="write_data", tool_args={}
        )
...
class TestConstraintGraph(unittest.TestCase):
    ...
    def test_check_violation_blocks_unauthorized_tool(self):
        ...
        violation = policy.check_violation(agent_role="finance", tool_name="delete", args={})
    ...
    def test_check_violation_allows_authorized_tool(self):
        ...
        violation = policy.check_violation(agent_role="finance", tool_name="read", args={})
    ...
    def test_argument_validation_blocks_dangerous_paths(self):
        ...
        violation = policy.check_violation(
            agent_role="file-agent", tool_name="write_file", args={"path": "/etc/passwd"}
        )
    ...
    def test_argument_validation_allows_safe_paths(self):
        ...
        violation = policy.check_violation(
            agent_role="file-agent", tool_name="write_file", args={"path": "/data/report.txt"}
        )
    ...
    def test_path_traversal_attack_blocked(self):
        ...
        violation = policy.check_violation(
            agent_role="file-agent", tool_name="write_file", args={"path": "/data/../etc/passwd"}
        )
    ...
    def test_code_execution_blocks_dangerous_patterns(self):
        ...
        # Test rm -rf pattern
        violation = policy.check_violation(
            agent_role="code-agent", tool_name="execute_code", args={"code": "rm -rf /"}
        )
        ...
        violation = policy.check_violation(
            agent_role="code-agent", tool_name="execute_code", args={"code": "DROP TABLE users"}
        )
    ...
    def test_scale_by_subtraction_default_deny(self):
        ...
        violation = policy.check_violation(
            agent_role="unknown-agent", tool_name="any_tool", args={}
        )
...
class TestIntegratedInterception(unittest.TestCase):
    ...
    def test_end_to_end_blocking(self):
        ...
        # Allowed: SELECT
        result = kernel.intercept_tool_execution(
            agent_id="sql-agent", tool_name="SELECT", tool_args={"query": "SELECT * FROM users"}
        )
        self.assertIsNone(result)  # Allowed

        # Blocked: DROP
        result = kernel.intercept_tool_execution(
            agent_id="sql-agent", tool_name="DROP", tool_args={"query": "DROP TABLE users"}
        )
        self.assertIsNotNone(result)
        self.assertEqual(result["status"], "blocked")
        self.assertTrue(result["mute"])
    ...
    def test_multiple_agents_different_permissions(self):
        ...
        # read-agent can read
        result = kernel.intercept_tool_execution(
            agent_id="read-agent", tool_name="read", tool_args={}
        )
        self.assertIsNone(result)

        # read-agent cannot write
        result = kernel.intercept_tool_execution(
            agent_id="read-agent", tool_name="write", tool_args={}
        )
        self.assertIsNotNone(result)
        self.assertEqual(result["status"], "blocked")
        ...
        result = kernel.intercept_tool_execution(
            agent_id="write-agent", tool_name="write", tool_args={}
        )
        self.assertIsNone(result)
