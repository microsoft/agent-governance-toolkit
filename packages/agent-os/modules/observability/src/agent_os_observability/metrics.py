...
class KernelMetrics:
    ...
    def __init__(self, namespace: str = "agent_os"):
        ...
        self.requests_total = Counter(
            f"{namespace}_requests_total", "Total requests processed", ["action", "status"]
        )

        self.active_agents = Gauge(f"{namespace}_active_agents", "Number of active agents")
        ...
        self.signals_sent = Counter(
            f"{namespace}_signals_total", "Signals sent to agents", ["signal", "reason"]
        )

        self.sigkill_count = Counter(
            f"{namespace}_sigkill_total", "SIGKILL signals issued", ["agent_id", "reason"]
        )
        ...
        self.recovery_success = Counter(
            f"{namespace}_recovery_total", "Recovery attempts", ["status"]
        )
        ...
        self.kernel_uptime = Gauge(f"{namespace}_kernel_uptime_seconds", "Kernel uptime in seconds")

        self.agent_crashes = Counter(
            f"{namespace}_agent_crashes_total", "Agent crashes (user space)", ["agent_id", "reason"]
        )

        self.kernel_crashes = Counter(
            f"{namespace}_kernel_crashes_total", "Kernel crashes (should be 0)"
        )
        ...
        self.cmvk_consensus_ratio = Gauge(
            f"{namespace}_cmvk_consensus_ratio", "Current model consensus ratio (0.0-1.0)"
        )
        ...
        self.agent_errors = Counter(
            f"{namespace}_agent_errors_total", "Agent errors by type", ["agent_id", "error_type"]
        )
        ...
        self.kernel_info = Info(f"{namespace}_kernel", "Kernel version and configuration")
        self.kernel_info.info({"version": "1.1.0", "policy_mode": "strict"})
    ...
    def record_violation(self, agent_id: str, action: str, policy: str, severity: str = "high"):
        ...
        self.violations_total.labels(
            agent_id=agent_id, action=action, policy=policy, severity=severity
        ).inc()
    ...
    def record_cmvk_verification(
        ...
        self.cmvk_verification_duration.labels(model_count=str(model_count)).observe(
            duration_seconds
        )
