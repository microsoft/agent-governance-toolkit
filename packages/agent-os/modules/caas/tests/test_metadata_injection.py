...
from caas.models import ContentFormat, Document, DocumentType, Section
...
def test_metadata_enrichment_basic():
    ...
    section = Section(title="North America", content="It increased by 5%.", chapter="Revenue")
    ...
    enriched_chunk = enricher.get_enriched_chunk(section, document.title, document.detected_type)
...
def test_enrichment_with_parent_sections():
    ...
    enricher = MetadataEnricher()
    enriched = enricher.get_enriched_chunk(section1, document.title, document.detected_type)
...
def main():
    ...
    try:
        ...
        print(
            '  After:  "[Document: Q3 Earnings] [Chapter: Revenue] [Section: North America] It increased by 5%."'
        )
        print("=" * 70)
