...
from caas.models import ContentFormat, ContentTier
...
"""
    ...
    for section in document.sections:
        print(
            f"  - {section.title[:50]}: Tier={section.tier.value if section.tier else 'None'}, Weight={section.weight}x"
        )
    ...
    print(
        f"✓ Tier distribution: Tier 1={tier_counts['tier_1']}, Tier 2={tier_counts['tier_2']}, Tier 3={tier_counts['tier_3']}"
    )
...
def test_tier_based_retrieval():
    ...
    print("✓ Extracted context (limited to 200 tokens)")
    ...
    if "Footnotes" in [s.title for s in document.sections]:
        ...
        endpoint_section = [
            s for s in document.sections if "Endpoint" in s.title or "POST" in s.title
        ]
        ...
        if endpoint_section and footnote_sections:
            ...
            print("✓ Tier 1 content properly prioritized over Tier 3")
...
def test_semantic_similarity_with_tiers():
    ...
    if "Authentication API Contract" in weights and "Developer Comments" in weights:
        ...
        print(
            f"✓ API Contract (Tier 1) weight {api_weight}x > Comments (Tier 3) weight {comment_weight}x"
        )
        print("✓ Tier boosting works even with same semantic similarity!")
