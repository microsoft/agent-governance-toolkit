...
from caas.pragmatic_truth import CitationFormatter, ConflictDetector, SourceDetector
...
def test_conflict_detection():
    ...
    conflicts = detector.detect_conflicts([official_section, practical_section], {})
...
def test_context_extraction_with_citations():
    ...
    extractor = ContextExtractor(
        store, enrich_metadata=False, enable_citations=True, detect_conflicts=False
    )

    context, metadata = extractor.extract_context(
        doc_id_official, query="rate limit", max_tokens=1000
    )
...
def test_conflict_detection_in_extraction():
    ...
    extractor = ContextExtractor(
        store, enrich_metadata=False, enable_citations=True, detect_conflicts=True
    )

    context, metadata = extractor.extract_context(doc_id, query="server restart", max_tokens=2000)
