...
def test_cold_context():
    ...
    cold_context = manager.get_cold_context(
        query="authentication", max_tokens=500, include_metadata=True
    )
    ...
    cold_context_no_match = manager.get_cold_context(query="unrelated_topic", max_tokens=500)
...
def test_full_context_triad():
    ...
    manager.add_cold_context(
        "Previous authentication bug fixed in 2023", metadata={"date": "2023-06-15"}, priority=1.0
    )

    # Test 1: Hot + Warm only (default behavior)
    print("\n--- Test 1: Hot + Warm Context (Default) ---")
    result = manager.get_full_context(include_hot=True, include_warm=True, include_cold=False)
    ...
    result = manager.get_full_context(
        include_hot=True, include_warm=True, include_cold=True, cold_query="authentication"
    )
    ...
    result = manager.get_full_context(
        include_hot=False, include_warm=False, include_cold=True, cold_query=None
    )
    ...
    print("\n--- Test 4: Only Hot Context ---")
    result = manager.get_full_context(include_hot=True, include_warm=False, include_cold=False)
