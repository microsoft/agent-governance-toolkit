...
from caas.models import ContentTier, Document, DocumentType, Section
...
class WeightTuner:
    ...
    def tune(self, document: Document) -> Document:
        ...
        base_weights = self.TYPE_SPECIFIC_WEIGHTS.get(document.detected_type, {"default": 1.0})

        # Calculate weights for each section
        for section in document.sections:
            weight = self._calculate_section_weight(
                section, document.detected_type, base_weights, document
            )
            section.weight = weight
            section.importance_score = self._calculate_importance_score(section)

        # Store overall weights in document
        document.weights = {section.title: section.weight for section in document.sections}
    ...
    def _analyze_content_factors(self, section: Section) -> Dict[str, Any]:
        ...
        has_code = bool(re.search(r"```|<code>|def |function |class ", content))

        # Check for definitions
        has_definitions = bool(re.search(r"defined as|definition|means|refers to", content))
        ...
        important_words = ["important", "critical", "must", "required", "key", "essential"]
        words = content.split()
...
class CorpusAnalyzer:
    ...
    def _generate_suggestions(self) -> List[str]:
        ...
        if len(self.documents) < 5:
            suggestions.append("Add more documents to improve weight optimization accuracy")

        # Check for document type diversity
        type_counts = Counter(doc.detected_type for doc in self.documents)
        if len(type_counts) == 1:
            suggestions.append("Corpus contains only one document type; consider adding variety")
