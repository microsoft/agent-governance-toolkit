...
class SourceDetector:
    ...
    def detect_source_type(
        self, document: Document, section: Optional[Section] = None
    ) -> SourceType:
        """
    ...
    def create_citation(
        ...
        if section:
            excerpt = (
                section.content[:150] + "..." if len(section.content) > 150 else section.content
            )
...
class ConflictDetector:
    ...
    def detect_conflicts(
        self, sections: List[Section], documents: Dict[str, Document]
    ) -> List[SourceConflict]:
        """
        ...
        for official_sec in official_sections:
            for practical_sec in practical_sections:
                conflict = self._detect_conflict_between_sections(official_sec, practical_sec)
                if conflict:
    ...
    def _detect_conflict_between_sections(
        self, official_section: Section, practical_section: Section
...
class CitationFormatter:
    ...
    def enrich_context_with_citations(self, context: str, citations: List[SourceCitation]) -> str:
        """
