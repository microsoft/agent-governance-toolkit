...
from caas.models import ContentTier, DocumentType, Section
...
class StructureParser:
    ...
    def parse_and_assign_tiers(
        self, sections: List[Section], doc_type: DocumentType, content: str = ""
    ) -> List[Section]:
        """
    ...
    def _is_tier_3_content(self, content: str, title: str) -> bool:
        ...
        for pattern in self.TIER_3_PATTERNS:
            if re.search(pattern, combined_text, re.MULTILINE | re.IGNORECASE):
                ...
                comment_lines = len(
                    re.findall(pattern, combined_text, re.MULTILINE | re.IGNORECASE)
                )
