...
class BaseProcessor(ABC):
    ...
    def _extract_sections(self, text: str) -> List[Section]:
        ...
        if not matches:
            ...
            return [Section(title="Main Content", content=text, start_pos=0, end_pos=len(text))]
        ...
        for i, match in enumerate(matches):
            ...
            sections.append(Section(title=title, content=content, start_pos=start, end_pos=end))
...
class CodeProcessor(BaseProcessor):
    ...
    def process(self, content: bytes, metadata: Dict[str, Any]) -> Document:
        ...
        if not sections:
            sections = [Section(title="Source Code", content=text, start_pos=0, end_pos=len(text))]
