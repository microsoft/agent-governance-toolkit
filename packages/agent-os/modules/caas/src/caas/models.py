...
class ContextRequest(BaseModel):
    ...
    enable_citations: bool = Field(
        default=True, description="Include source citations for transparency (default: True)"
    )
...
class ContextResponse(BaseModel):
    ...
    source_citations: List[SourceCitation] = Field(
        default=[], description="Citations for all sources used in the context"
    )
    source_conflicts: List[Dict[str, Any]] = Field(
        default=[], description="Conflicts between official and practical sources"
    )
...
class ContextTriadRequest(BaseModel):
    ...
    include_cold: bool = Field(
        default=False, description="Include cold context (historical archive) - on-demand only"
    )
    max_tokens_per_layer: Dict[str, int] = Field(
        default={"hot": 1000, "warm": 500, "cold": 1000}, description="Token limits per layer"
    )
    query: Optional[str] = Field(
        default=None, description="Optional query for cold context retrieval"
    )
...
class SourceConflict(BaseModel):
    ...
    conflict_severity: str = Field(default="medium", description="Severity: low, medium, high")
...
class VFSState(BaseModel):
    ...
    files: Dict[str, FileNode] = Field(
        default_factory=dict, description="Mapping of file paths to FileNode objects"
    )
