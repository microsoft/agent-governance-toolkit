...
class DocumentStore:
    ...
    def list_by_type(self, doc_type: DocumentType) -> List[Document]:
        ...
        """
        return [doc for doc in self.documents.values() if doc.detected_type == doc_type]
    ...
    def search(
        self, query: str, enable_time_decay: bool = True, decay_rate: float = 1.0
    ) -> List[Document]:
        """
            ...
            if match_score > 0:
                ...
                if enable_time_decay:
                    decay_factor = calculate_decay_factor(
                        doc.ingestion_timestamp, reference_time=None, decay_rate=decay_rate
                    )
...
class ContextExtractor:
    ...
    def extract_context(
        self, document_id: str, query: str = "", max_tokens: int = 2000
        ...
        sorted_sections = sorted(adjusted_sections, key=lambda s: s.weight, reverse=True)
        ...
        metadata = {
            ...
            "tiers_applied": {
                s.title: s.tier.value if s.tier else "unknown" for s in sorted_sections
            },
