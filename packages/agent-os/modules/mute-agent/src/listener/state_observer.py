...
class StateObserver:
    ...
    def __init__(
        ...
        self._samples: Dict[MetricType, deque] = {
            metric_type: deque(maxlen=self.MAX_SAMPLES_PER_METRIC) for metric_type in MetricType
        }
    ...
    def _collect_graph_metrics(self) -> Dict[MetricType, float]:
        ...
        for dim_name, subgraph in self.knowledge_graph.subgraphs.items():
            ...
            for edges in subgraph._adjacency_list.values():
                ...
                conflict_edges += sum(1 for e in edges if e.edge_type == EdgeType.CONFLICTS_WITH)
    ...
    def _collect_session_metrics(self) -> Dict[MetricType, float]:
        ...
        completed = sum(1 for s in sessions.values() if s.state == HandshakeState.COMPLETED)
        failed = sum(1 for s in sessions.values() if s.state == HandshakeState.FAILED)
        rejected = sum(1 for s in sessions.values() if s.state == HandshakeState.REJECTED)
    ...
    def _collect_performance_metrics(self) -> Dict[MetricType, float]:
        ...
        if self.router.routing_history:
            ...
            metrics[MetricType.ACTION_SPACE_SIZE] = float(len(last_routing.pruned_action_space))
        else:
    ...
    def _calculate_derived_metrics(
        self, metrics: Dict[MetricType, float], context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, float]:
    ...
    def get_metric_history(
        self, metric_type: MetricType, window_seconds: Optional[float] = None
    ) -> List[MetricSample]:
        """
    ...
    def get_observation_history(self, count: Optional[int] = None) -> List[ObservationResult]:
