...
class ListenerAgent:
    ...
    def __init__(
        ...
        self._interventions: deque = deque(maxlen=self.config.max_observation_history)
    ...
    def evaluate_thresholds(
        self, observation: ObservationResult
        ...
        max_level = self.config.thresholds.get_maximum_intervention_level(triggered_rules)
        ...
        if action == "emit_warning":
            ...
            for session_id, session in list(self.protocol.sessions.items()):
                if session.state in [
                    ...
                    self.protocol.reject_proposal(
                        session_id, reason=f"Listener intervention: {level.value}"
                    )
                    blocked += 1
            ...
            for session_id, session in list(self.protocol.sessions.items()):
                if session.state != HandshakeState.COMPLETED:
                    try:
                        self.protocol.reject_proposal(
                            session_id, reason="EMERGENCY: System halt by Listener"
                        )
                        halted += 1
                    except ValueError:
    ...
    def register_intervention_callback(self, callback: Callable[[InterventionEvent], None]) -> None:
        """Register a callback to be notified of interventions."""
        self._intervention_callbacks.append(callback)

    def register_state_change_callback(
        self, callback: Callable[[ListenerState, ListenerState], None]
    ) -> None:
        """Register a callback to be notified of state changes."""
        self._state_change_callbacks.append(callback)

    def get_intervention_history(self, count: Optional[int] = None) -> List[InterventionEvent]:
    ...
    def update_threshold(self, threshold_type: ThresholdType, new_value: float) -> None:
