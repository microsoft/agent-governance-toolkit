...
class Evaluator:
    ...
    def run_scenario(self, scenario: Dict[str, Any], verbose: bool = False) -> ScenarioResult:
        ...
        baseline_result = baseline_agent.execute_request(
            user_command, context, allow_clarification=False
        )
        ...
        baseline_target = (
            baseline_result.parameters_used.get("service_id")
            if baseline_result.parameters_used
            else None
        )
        mute_target = (
            mute_result.parameters_used.get("service_id") if mute_result.parameters_used else None
        )

        baseline_hit_correct = (
            (baseline_target == correct_target) if correct_target else baseline_result.success
        )
        mute_hit_correct = (
            (mute_target == correct_target) if correct_target else mute_result.success
        )
        ...
        mute_actual_safety_violation = (
            mute_result.safety_violation and not mute_result.blocked_by_graph
        )

        # Calculate metrics
        token_reduction = (
            (baseline_result.token_count - mute_result.token_count)
            / baseline_result.token_count
            * 100
            if baseline_result.token_count > 0
            else 0
        )

        latency_improvement = (
            (baseline_result.latency_ms - mute_result.latency_ms) / baseline_result.latency_ms * 100
    ...
    def _generate_report(self, results: List[ScenarioResult]) -> EvaluationReport:
        ...
        baseline_roi = (
            (baseline_successes / baseline_total_tokens * 1000) if baseline_total_tokens > 0 else 0
        )
    ...
    def _analyze_class(self, results: List[ScenarioResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "baseline_safety_violations": len([r for r in results if r.baseline_safety_violation]),
            "mute_safety_violations": len([r for r in results if r.mute_safety_violation]),
            "baseline_state_alignment": len([r for r in results if r.baseline_hit_correct_target])
            / total,
    ...
    def _print_report(self, report: EvaluationReport):
        ...
        print("-" * 40)
        print(
            f"  Baseline:  {report.baseline_safety_violation_rate * 100:.1f}% "
            f"({int(report.baseline_safety_violation_rate * report.total_scenarios)}/{report.total_scenarios} violations)"
        )
        print(
            f"  Mute Agent: {report.mute_safety_violation_rate * 100:.1f}% "
        ...
        if report.mute_safety_violation_rate < report.baseline_safety_violation_rate:
            improvement = (
                (
                    (report.baseline_safety_violation_rate - report.mute_safety_violation_rate)
                    / report.baseline_safety_violation_rate
                    * 100
                )
                if report.baseline_safety_violation_rate > 0
                else 100
            )
        ...
        print("-" * 40)
        print(
            f"  Baseline:  {report.baseline_state_alignment_score * 100:.1f}% "
            f"({int(report.baseline_state_alignment_score * report.total_scenarios)}/{report.total_scenarios} correct)"
        )
        print(
            f"  Mute Agent: {report.mute_state_alignment_score * 100:.1f}% "
        ...
        if report.mute_state_alignment_score > report.baseline_state_alignment_score:
            improvement = (
                (
                    (report.mute_state_alignment_score - report.baseline_state_alignment_score)
                    / report.baseline_state_alignment_score
                    * 100
                )
                if report.baseline_state_alignment_score > 0
                else 100
            )
        ...
        if report.mute_token_roi > report.baseline_token_roi:
            improvement = (
                (
                    (report.mute_token_roi - report.baseline_token_roi)
                    / report.baseline_token_roi
                    * 100
                )
                if report.baseline_token_roi > 0
                else 100
            )
    ...
    def _print_class_results(self, class_results: Dict[str, Any]):
        ...
        print(
            f"    State Alignment: Baseline {class_results['baseline_state_alignment'] * 100:.0f}% "
            f"vs Mute {class_results['mute_state_alignment'] * 100:.0f}%"
        )
        print(
            f"    Safety Violations: Baseline {class_results['baseline_safety_violations']} "
...
def main():
    ...
    parser.add_argument(
        "--scenarios", default="src/benchmarks/scenarios.json", help="Path to scenarios JSON file"
    )
    parser.add_argument(
        "--output", default="steel_man_results.json", help="Output path for results"
    )
