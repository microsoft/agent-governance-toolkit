...
class GraphDebugger:
                        ...
                        if not NETWORKX_AVAILABLE:
                                        ...
                                        if not PYVIS_AVAILABLE:
                                                        ...
                                                        if not MATPLOTLIB_AVAILABLE:
                                                                        ...
                                                                        logger.warning(
                                                                                            "Visualization libraries not available: %s. "
                                                                                            "Install with: pip install matplotlib networkx pyvis",
                                                                                            ", ".join(missing),
                                                                        )
                                                                    ...
                                                                    format: 'html' (interactive) or 'png' (static image)
                                                                            ...
                                                                            Returns:
                                                                                        Path to the generated visualization
                                                                                                """
        if not VISUALIZATION_AVAILABLE:
                        logger.warning("Cannot generate visualization: libraries not installed")
            return ""
    ...
    def _generate_interactive_html(self, trace: ExecutionTrace, output_path: str) -> str:
        ...
        logger.info("Interactive visualization saved to: %s", output_path)
        return output_path
    ...
    def _generate_static_png(self, trace: ExecutionTrace, output_path: str) -> str:
        ...
        logger.info("Static visualization saved to: %s", output_path)
        return output_path
    ...
    def _create_html_legend(self, trace: ExecutionTrace) -> str:
        ...
        legend = f"""
        <div style="position: absolute; top: 10px; right: 10px; background: white; padding: 15px;
                    border: 2px solid #ccc; border-radius: 5px; font-family: Arial; max-width: 300px;">
                        <h3 style="margin: 0 0 10px 0;">Execution Trace Legend</h3>
                            <p style="margin: 5px 0;"><span style="color: #2ecc71;">&#9679;</span>
                                    <strong>Green:</strong> Traversed successfully ({success_count})</p>
                                        <p style="margin: 5px 0;"><span style="color: #e74c3c;">&#9679;</span>
                                                <strong>Red:</strong> Constraint failed ({failure_count})</p>
                                                    <p style="margin: 5px 0;"><span style="color: #95a5a6;">&#9679;</span>
                                                            <strong>Grey:</strong> Unreachable ({unreachable_count})</p>
    ...
    def _inject_legend_into_html(self, html_path: str, legend_html: str):
        ...
        try:
            ...
            logger.warning("Could not find HTML file: %s", html_path)
except PermissionError:
            logger.warning("Permission denied writing to: %s", html_path)
except Exception as e:
            logger.warning("Could not inject legend: %s: %s", type(e).__name__, e)
    ...
    def generate_comparison_visualization(
        ...
        logger.info("Comparison visualization saved to: %s", output_path)
        return output_path
