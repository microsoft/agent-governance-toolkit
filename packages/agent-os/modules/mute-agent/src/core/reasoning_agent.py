...
from ..super_system.router import RoutingResult, SuperSystemRouter
...
class ReasoningAgent:
    ...
    def propose_action(
        ...
        proposal = ActionProposal(
            action_id=action_id, parameters=parameters, context=context, justification=justification
        )
    ...
    def _validate_proposal(self, proposal: ActionProposal) -> ValidationResult:
        ...
        action_available = any(
            node.id == proposal.action_id for node in routing_result.pruned_action_space
        )

        if not action_available:
            errors.append(f"Action {proposal.action_id} not available in pruned action space")
        ...
        all_missing_deps = self.knowledge_graph.find_all_missing_dependencies(
            proposal.action_id, routing_result.selected_dimensions, proposal.context
        )
        ...
        if all_missing_deps:
            ...
            for dim_name in routing_result.selected_dimensions:
                ...
                constraints = self.knowledge_graph.get_action_constraints(
                    proposal.action_id, dim_name
                )

                for constraint in constraints:
                    if not self._check_constraint(constraint, proposal):
                        warnings.append(f"Constraint {constraint.id} may not be fully satisfied")
    ...
    def _check_constraint(self, constraint: Node, proposal: ActionProposal) -> bool:
    ...
    def select_best_action(
        self, context: Dict[str, Any], selection_criteria: Optional[Dict[str, Any]] = None
    ) -> Optional[Node]:
        """
    ...
    def _score_action(self, action: Node, criteria: Dict[str, Any]) -> float:
