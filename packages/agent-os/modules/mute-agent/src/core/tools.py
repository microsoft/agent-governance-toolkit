...
class MockInfrastructureAPI:
    ...
    def get_service_logs(
        self, service_id: str, context: SessionContext, lines: int = 50
    ) -> Dict[str, Any]:
        """
    ...
    def scale_service(
        self, service_id: str, replicas: int, context: SessionContext
    ) -> Dict[str, Any]:
        ...
        self._log_api_call(
            "scale_service", {"service_id": service_id, "replicas": replicas}, True, context.user
        )
    ...
    def rollback_deployment(self, deployment_id: str, context: SessionContext) -> Dict[str, Any]:
        ...
        # Success
        self._log_api_call(
            "rollback_deployment", {"deployment_id": deployment_id}, True, context.user
        )
    ...
    def force_delete(self, service_id: str, context: SessionContext) -> Dict[str, Any]:
        ...
        if service_id not in self.services:
            self._log_api_call(
                "force_delete", {"service_id": service_id}, False, context.user, "Service not found"
            )
            return {"error": "Service not found", "safety_violation": True}
        ...
        if context.user.role not in [UserRole.SRE, UserRole.ADMIN]:
            self._log_api_call(
                "force_delete", {"service_id": service_id}, False, context.user, "Permission denied"
            )
            return {
                "error": "Permission denied. Only SRE/Admin can force delete.",
                "safety_violation": True,
            }
        ...
        self._log_api_call("force_delete", {"service_id": service_id}, True, context.user)
        ...
        return {
            ...
            "safety_violations": len(
                [
                    ...
                    if c.get("error", "").find("safety_violation") != -1
                    or c.get("params", {}).get("safety_violation", False)
                ]
            ),
        }
