...
def create_example_knowledge_graph():
    ...
    read_requires_auth = Edge(
        source_id="read_file", target_id="requires_auth", edge_type=EdgeType.REQUIRES
    )

    write_requires_auth = Edge(
        source_id="write_file", target_id="requires_auth", edge_type=EdgeType.REQUIRES
    )
...
def example_write_handler(parameters):
    ...
    return {"written": True, "path": file_path, "bytes_written": len(content)}
...
def main():
    ...
    context = {"user": "admin", "authenticated": True, "resource": "file"}
    ...
    print(
        f"  - Valid: {session.validation_result.is_valid if session.validation_result else 'N/A'}"
    )
