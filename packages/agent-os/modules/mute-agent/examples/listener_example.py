...
def main():
    ...
    print("   ✅ Handshake protocol initialized")
    print("   ✅ Super system router ready")
    ...
    for i in range(3):
        ...
        print(
            f"      - Rejection rate: {observation.derived_metrics.get('action_rejection_rate', 0):.2%}"
        )
        time.sleep(0.2)
...
""")
    ...
    if interventions:
        ...
        for event in interventions:
            print(
                f"  - {event.timestamp.isoformat()}: {event.action_taken} ({event.intervention_level.value})"
            )
    else:
