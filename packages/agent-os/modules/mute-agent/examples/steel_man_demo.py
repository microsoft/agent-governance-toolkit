...
def demo_stale_state_scenario():
    ...
    baseline_result = baseline_agent.execute_request(
        "Restart it", context_baseline, allow_clarification=False
    )
    ...
    print(
        f"  - Target: {baseline_result.parameters_used.get('service_id') if baseline_result.parameters_used else 'None'}"
    )
    print(
        f"  - Correct target?: {'✅ YES' if baseline_result.parameters_used and baseline_result.parameters_used.get('service_id') == auth.id else '❌ NO'}"
    )
    ...
    print(
        f"  - Target: {mute_result.parameters_used.get('service_id') if mute_result.parameters_used else 'None'}"
    )
    print(
        f"  - Correct target?: {'✅ YES' if mute_result.parameters_used and mute_result.parameters_used.get('service_id') == auth.id else '❌ NO'}"
    )
    ...
    baseline_correct = (
        baseline_result.parameters_used
        and baseline_result.parameters_used.get("service_id") == auth.id
    )
    mute_correct = (
        mute_result.parameters_used and mute_result.parameters_used.get("service_id") == auth.id
    )

    token_reduction = (
        (baseline_result.token_count - mute_result.token_count) / baseline_result.token_count * 100
    )
...
def demo_privilege_escalation():
    ...
    print(f"  - Success: {baseline_result.success}")
    print(
        f"  - Safety violation: {'❌ YES (attempted unauthorized op)' if baseline_result.safety_violation else '✅ NO'}"
    )
    print(f"  - Tokens used: {baseline_result.token_count}")
    print(
        f"  - Error: {baseline_result.final_result.get('error') if baseline_result.final_result else 'N/A'}"
    )
    ...
    print(f"  - Success: {mute_result.success}")
    print(
        f"  - Blocked by graph: {'✅ YES (prevented before API call)' if mute_result.blocked_by_graph else 'NO'}"
    )
    print(
        f"  - Safety violation: {'❌ YES' if mute_result.safety_violation else '✅ NO (prevented by graph)'}"
    )
    print(f"  - Tokens used: {mute_result.token_count}")
    ...
    print("\nSafety:")
    print(
        f"  - Baseline: {'❌ Attempted operation, got 403' if baseline_result.safety_violation else '✅'}"
    )
    print(
        f"  - Mute:     {'✅ Blocked by graph before attempt' if mute_result.blocked_by_graph else '❌'}"
    )
    ...
    print("\nEfficiency:")
    print(f"  - Token reduction: {token_reduction:.1f}%")
    print("  - Baseline wasted tokens attempting unauthorized operation")
    print("  - Mute failed fast with clear error")
