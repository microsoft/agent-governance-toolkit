...
def test_simple_request():
    ...
    try:
        response = httpx.post(
            ...
            json={"task": "book_flight", "data": {"destination": "NYC", "date": "2026-02-15"}},
        )
...
def test_warning_and_override():
    ...
    try:
        response = httpx.post(
            "http://localhost:8001/proxy",
            json={"task": "book_flight", "data": {"destination": "NYC"}},
        )
        print(f"Status: {response.status_code}")
...
def test_trace_retrieval():
    ...
    try:
        response = httpx.post("http://localhost:8001/proxy", json={"task": "test", "data": {}})
