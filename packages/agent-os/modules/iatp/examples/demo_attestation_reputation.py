...
def demo_attestation():
    ...
    is_valid, error = validator.validate_attestation(tampered_attestation, verify_signature=False)
...
def demo_reputation_slashing():
    ...
    manager.record_hallucination(
        ...
        details={"reason": "Made up financial transaction data", "context": "Banking query"},
    )

    score = manager.get_score(agent_id)
    print(f"❌ After critical hallucination: Score = {score.score:.2f}/10 (slashed by 2.0)")
    print(f"  Trust Level: {score.get_trust_level().value}")
    print("  🚨 Agent is now UNTRUSTED - other agents stop listening!")
    ...
    for event in score.recent_events[-3:]:
        ...
        print(
            f"{symbol} {event.event_type} ({event.severity}): "
...
def demo_network_propagation():
    ...
    node1.record_hallucination(
        agent_id="data-agent-x", severity="high", details={"reason": "Fabricated statistics"}
    )
    ...
    if score2:
        ...
        print("   🌐 Reputation propagated across network!")
    ...
    print("Node 1 view: 'trusted-agent-y' = 8.0/10")
    print("Node 2 view: 'trusted-agent-y' = 3.0/10")
    ...
    print("✓ System prefers safety: when in doubt, trust less")
...
def demo_integration_with_manifest():
    ...
    manifest = CapabilityManifest(
        ...
        capabilities=AgentCapabilities(idempotency=True, reversibility=ReversibilityLevel.PARTIAL),
    ...
    for i in range(3):
        manager.record_hallucination(agent_id="analytics-agent", severity="medium")
    ...
    print("  🔒 Other agents can use this to reject requests!")

    # Show the decision process
    if reputation_trust == TrustLevel.UNTRUSTED:
        print("\n❌ REQUEST REJECTED:")
        print("   Agent 'analytics-agent' has been slashed to UNTRUSTED")
