...
def create_reviewer_agent_manifest() -> CapabilityManifest:
    ...
    return CapabilityManifest(
        ...
        capabilities=AgentCapabilities(
            idempotency=True, reversibility=ReversibilityLevel.NONE, sla_latency="1000ms"
        ),
        privacy_contract=PrivacyContract(
...
def validate_agent(manifest: CapabilityManifest, operation: str, required_scopes: List[str]):
    ...
    is_compatible, error_msg = engine.validate_handshake(manifest, required_scopes=required_scopes)
