...
@app.post("/")
async def process_banking_request(request: dict, x_agent_trace_id: Optional[str] = Header(None)):
    """
...
async def compensate_transaction(transaction_id: str):
    ...
    if time.time() - transaction["timestamp"] > UNDO_WINDOW_SECONDS:
        raise HTTPException(status_code=400, detail="Undo window expired (5 minutes)")
