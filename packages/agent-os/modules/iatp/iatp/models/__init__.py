...
class PrivacyContract(BaseModel):
    ...
    retention: RetentionPolicy = Field(..., description="How long the agent stores data")
    storage_location: Optional[str] = Field(
        None, description="Geographic location of data storage (e.g., 'us-west')"
    )
    human_review: bool = Field(False, description="Whether humans may review the data")
    encryption_at_rest: bool = Field(True, description="Whether data is encrypted at rest")
    encryption_in_transit: bool = Field(True, description="Whether data is encrypted in transit")


class AgentCapabilities(BaseModel):
    """Capabilities advertised by an agent."""

    idempotency: bool = Field(False, description="Whether duplicate requests are handled safely")
    reversibility: ReversibilityLevel = Field(
        ReversibilityLevel.NONE, description="Level of transaction reversibility support"
    )
    undo_window: Optional[str] = Field(
        None, description="Time window for undo operations (e.g., '24h', '7d')"
    )
    sla_latency: Optional[str] = Field(
        None, description="Promised response latency (e.g., '2000ms', '5s')"
    )
    rate_limit: Optional[int] = Field(None, description="Maximum requests per minute")
...
class CapabilityManifest(BaseModel):
    ...
    trust_level: TrustLevel = Field(TrustLevel.STANDARD, description="Trust level of the agent")
    capabilities: AgentCapabilities = Field(..., description="Capabilities supported by the agent")
    privacy_contract: PrivacyContract = Field(..., description="Privacy policies of the agent")
    ...
    def calculate_trust_score(self) -> int:
        ...
        if self.privacy_contract.retention == RetentionPolicy.EPHEMERAL:
            score += 2
        elif self.privacy_contract.retention in [
...
class TracingContext(BaseModel):
    ...
    trace_id: str = Field(..., description="Unique trace ID for the request")
    parent_trace_id: Optional[str] = Field(
        None, description="Parent trace ID if this is part of a chain"
    )
    timestamp: str = Field(..., description="ISO 8601 timestamp")
    agent_id: str = Field(..., description="ID of the agent processing this request")
...
class AttestationRecord(BaseModel):
    ...
    agent_id: str = Field(..., description="Unique identifier for the agent")
    codebase_hash: str = Field(..., description="SHA-256 hash of the agent's codebase")
    config_hash: str = Field(..., description="SHA-256 hash of the agent's configuration")
    signature: str = Field(..., description="Digital signature from Control Plane (base64 encoded)")
    signing_key_id: str = Field(
        ..., description="Identifier for the public key used to verify signature"
    )
    timestamp: str = Field(..., description="ISO 8601 timestamp when attestation was created")
    expires_at: Optional[str] = Field(
        None, description="ISO 8601 timestamp when attestation expires"
    )
...
class ReputationEvent(BaseModel):
    ...
    trace_id: Optional[str] = Field(
        None, description="Associated trace ID if event was part of a request"
    )
    details: Optional[Dict[str, Any]] = Field(
        None, description="Additional context about the event"
    )
    detected_by: Optional[str] = Field(
        None, description="Component that detected the event (e.g., 'cmvk', 'iatp')"
    )
...
class ReputationScore(BaseModel):
    ...
    score: float = Field(5.0, ge=0.0, le=10.0, description="Current reputation score (0.0 to 10.0)")
    initial_score: float = Field(5.0, description="Starting reputation score")
    total_events: int = Field(0, description="Total number of reputation events")
    positive_events: int = Field(0, description="Number of positive reputation events")
    negative_events: int = Field(0, description="Number of negative reputation events")
    last_updated: str = Field(..., description="ISO 8601 timestamp of last update")
    recent_events: List[ReputationEvent] = Field(
        default_factory=list, description="Recent reputation events (up to 100)"
    )
