...
class TypedPipe(Generic[T_In, T_Out]):
    ...
    def __init__(
        ...
        self._buffer: asyncio.Queue[PipeMessage] = asyncio.Queue(maxsize=self.config.buffer_size)
    ...
    def set_policy_check(self, check: Callable[[PipeMessage], Awaitable[bool]]) -> None:
        """Set the policy check callback."""
        self._policy_check = check

    def set_audit_callback(self, callback: Callable[[PipeMessage, str], None]) -> None:
    ...
    async def send(self, message: PipeMessage[T_In]) -> bool:
        ...
        if self.config.require_policy_check:
            if self._policy_check:
                try:
                    ...
                    if not allowed:
                        logger.warning(
                            f"[Pipe] {self.name} rejected message {message.message_id} by policy"
                        )
        ...
        try:
            await asyncio.wait_for(self._buffer.put(message), timeout=self.config.timeout_seconds)
    ...
    async def receive(self) -> Optional[PipeMessage[T_Out]]:
        ...
        try:
            message = await asyncio.wait_for(
                self._buffer.get(), timeout=self.config.timeout_seconds
            )
...
class AgentPipelineStage:
    ...
    async def process(self, message: PipeMessage) -> Optional[PipeMessage]:
        ...
        if not method:
            raise AttributeError(f"Agent {self.agent_id} has no method '{self._process_method}'")
...
class Pipeline:
    ...
    async def execute(
        ...
        self._trace_id = (
            trace_id
            or hashlib.sha256(str(datetime.now(timezone.utc).timestamp()).encode()).hexdigest()[:16]
        )
        ...
        for i, stage in enumerate(self.stages):
            ...
            if i + 1 < len(self.stages):
                ...
                current_message.target_agent = getattr(
                    next_stage, "agent_id", getattr(next_stage, "name", None)
                )
