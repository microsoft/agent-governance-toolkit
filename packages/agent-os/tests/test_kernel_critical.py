...
class TestVFSPermissions:
    ...
    def test_agent_can_read_own_memory(self):
        ...
        try:
            vfs.write("/mem/working/state.json", data_to_write)
            # Read back
            data = vfs.read("/mem/working/state.json")
...
class TestCMVKDriftDetection:
    ...
    @pytest.mark.skipif(
        sys.version_info < (3, 11), reason="CMVK uses datetime.UTC which requires Python 3.11+"
    )
...
class TestStatelessArchitecture:
    ...
    async def test_context_in_request_not_server(self):
        ...
        result1 = await kernel1.execute(action="test", params={}, context=ctx1)

        # Execute same context on kernel2 - should work identically
        result2 = await kernel2.execute(action="test", params={}, context=ctx1)
    ...
    async def test_no_bypass_possible(self):
        ...
        for attempt in bypass_attempts:
            result = await kernel.execute(
                action=attempt["action"], params=attempt["params"], context=ctx
            )
