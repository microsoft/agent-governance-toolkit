...
class DisputeResolverWorker:
    ...
    async def _resolution_loop(self):
        ...
        while self._running:
            try:
                ...
                ready_disputes = [
                    d
                    for d in self._pending_disputes
                    if d.requester_logs_hash
                    and d.provider_logs_hash
                ...
                for dispute in self._pending_disputes:
                    ...
                    if age > self.auto_resolve_timeout:
                        if dispute.dispute_id not in self._active_resolutions:
                            ...
                            asyncio.create_task(self._auto_resolve_timeout(dispute))
    ...
    async def _resolve_dispute(self, dispute: PendingDispute):
        ...
        try:
            ...
            self._pending_disputes = [
                d for d in self._pending_disputes if d.dispute_id != dispute_id
            ]
    ...
    async def _apply_resolution(
        ...
        logger.info(f"Applied resolution for {dispute.dispute_id}: {outcome}, credits distributed")
    ...
    async def _auto_resolve_timeout(self, dispute: PendingDispute):
        ...
        try:
            ...
            # Remove from pending
            self._pending_disputes = [
                d for d in self._pending_disputes if d.dispute_id != dispute.dispute_id
            ]
    ...
    def submit_evidence(
        ...
        for dispute in self._pending_disputes:
            if dispute.dispute_id == dispute_id:
                ...
                logger.info(f"Evidence submitted for {dispute_id} by {party}")
                return
