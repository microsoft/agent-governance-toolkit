...
class AICardIdentity(BaseModel):
    ...
    key_id: str | None = Field(None, description="Key identifier for rotation")
...
class AICard(BaseModel):
    ...
    homepage: str | None = Field(None, description="Documentation URL")

    # Identity
    identity: AICardIdentity | None = None
    ...
    # Card signature
    card_signature: AICardSignature | None = None
    ...
    def from_identity(
        ...
        identity: Any,
        description: str = "",
        services: list[AICardService] | None = None,
    ) -> AICard:
        ...
        if "verifiable" in data:
            ...
            for k, att in v.get("capability_attestations", {}).items():
                attestations[k] = CapabilityAttestation(
                    ...
                    expires_at=datetime.fromisoformat(att["expires_at"])
