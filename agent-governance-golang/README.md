...
This module does not ship framework-specific adapters. Hosts can pass
`NewTrustedSkillMetadataSource(...)` values derived from framework-owned state
to `LogWithSkillAuditMetadata(...)`; request payload fields are never used as
skill identity. Context snapshots are stored as stable SHA-256 hashes, and
unsupported values omit only their hash.
