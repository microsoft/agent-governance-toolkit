...
Required non-advisory stages must configure at least one nonblank allowed identity or role.
Rejected unauthorized votes leave an open request pending for an authorized approver;
the failed call does not authorize execution.

The default in-memory store retains records until explicitly cleaned up. For a long-running
service, pass a store with `WithApprovalStore(store)` and schedule
`store.PruneExpired(time.Now().UTC(), retention)` using a trusted clock and a nonnegative
retention duration. This removes the request, entries, and resolution together, only after
both expiry and resolution (if present) plus retention. Zero retention still preserves
records until expiry; records without an expiry are retained. Cleanup starts no background
goroutine. Export audit records before cleanup or use a durable `ApprovalStore` when needed.
Lookups and execution attempts for removed records fail closed as missing requests.

Stored results report `Allowed` only for an unexpired allowed request. A successful
`RequestApproval` execution can return `Allowed: true` with a consumed request because its
`Execution` records that one authorized execution; subsequent retries cannot reuse it.
