...
| [02 — Capability Scoping](policy-as-code/02-capability-scoping.md) | Restricting tool access | Give different agents different permissions |
...
| [04 — Conditional Policies](policy-as-code/04-conditional-policies.md) | Policy composition | Layer base + environment policies with conflict strategies |
| [05 — Approval Workflows](policy-as-code/05-approval-workflows.md) | Human-in-the-loop | Route dangerous actions to a human before execution |
| [06 — Policy Testing](policy-as-code/06-policy-testing.md) | Systematic validation | Test every role + action + environment combination |
| [07 — Policy Versioning](policy-as-code/07-policy-versioning.md) | Safe rollout | Compare v1 vs v2 behavior, catch regressions before deploying |
| [MCP Governance](policy-as-code/mcp-governance.md) | Supplemental | Governing MCP tool access with the proxy and OWASP-aligned rules |
...
- **Python 3.10+** for Python tutorials (01–18, 24–31) and Policy-as-Code chapters
