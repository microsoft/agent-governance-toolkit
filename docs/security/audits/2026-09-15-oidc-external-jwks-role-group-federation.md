...
This PR went through three rounds of maintainer review
(MohammadHaroonAbuomar), each surfacing real gaps that are fixed here
rather than deferred — see "Specific mitigations applied" below.
...
| **`audience` misconfiguration footguns** | **Rejected at construction.** `""`, `[]`, `[""]`, and `["", "x"]` are all rejected by a `field_validator` on `TrustedEndpoint.audience` — each would otherwise either match a token's own empty `aud` (silently accepting an audience-less token) or lock out every token with no signal that the field was misconfigured. |
...
- **`TrustedEndpoint.audience` field validator.** Rejects `""`, `[]`, `[""]`, `["", "x"]` at construction. Extended across review rounds 2 and 3 as narrower footguns (a list containing an empty-string member) were found.
...
- **Round 1** (`5200914330`): DCO trailer missing; no `aud` check; `nbf` unchecked; Ed25519-only docstring; `role_claim_path=""` didn't disable default extraction; dotted-client-id group paths unaddressable; PS256/RS512/ES384 rejection undocumented. Fixed in `d48413ab`.
...
- **Round 3** (`5214954370`, this doc): stale test assumption after `#3924` changed the policy evaluator's fail-closed behavior for unrecognized conditions; missing license headers; this security-audit doc. Fixed in the commits accompanying this document.
...
- All 58 tests in `test_external_jwks.py` pass against current `main` (post-rebase, post-`#3924`). The `cspell` added-lines gate and this document satisfy the two remaining CI findings from review round 3 alongside the license headers added in the same change.
