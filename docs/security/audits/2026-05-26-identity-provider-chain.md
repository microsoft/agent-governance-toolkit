...
| Token stashing on adapter | `ExternalJWKSProviderAdapter.set_pending_token()` holds a JWT briefly | The token is cleared after every `resolve()` call (consumed or not). The adapter is not shared across concurrent requests in the current single-threaded handshake model. If concurrency is introduced later, the adapter should be made request-scoped. |
...
No new third-party packages were added. The chain uses only stdlib (`abc`, `dataclasses`, `logging`) and existing `agentmesh` types.
