...
| [Agentmetry](https://agentmetry.ai) | AI Agent Audit / Detection Engineering | Ingests AGT `FileAuditSink` audit files into a hash-chained local trail and runs MITRE ATT&CK-tagged sequence detection across the session, surfacing correlations no per-call decision can see -- a credential read followed by network egress across three individually-allowed calls raises one critical finding. Verifies AGT's content-hash chain and HMAC signatures before ingest, using a stdlib reimplementation of the algorithm (see #3643). | Aug 2026 | [@blitzcrieg1](https://github.com/blitzcrieg1) |
