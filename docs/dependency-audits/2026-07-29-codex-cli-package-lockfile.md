...
| `js-yaml` | 4.3.2 (transitive, via npm `overrides`) | production | SDK requests 4.1.1; overridden to 4.3.2 to address the current dependency-review advisories |
...
CVE-2026-53550); 4.3.0 is itself vulnerable to two later advisories. Because
this package's lockfile is created in this change, it adopts 4.3.2 directly,
and the four sibling packages' overrides are raised to 4.3.2 as well — matching
...
(`agent-governance-opencode` still locks SDK 3.7.0). js-yaml 4.3.0 → 4.3.2 is
a patch release within the 4.x series that addresses the dependency-review
advisories; this
...
including the Codex process-boundary e2e hooks.

## Rollback Plan

To roll back this dependency update, revert the five `package.json` overrides
and regenerate each corresponding `package-lock.json`. This is not recommended:
older versions remain vulnerable to current advisories, and the
`dependency-review` CI gate will fail on the Codex package's lockfile.
