...
`chacha20` enters the tree as the ChaCha12 CSPRNG backend of `rand`
0.10's `ThreadRng`, pinned to `0.10.2`. Version `0.10.1` was yanked from
crates.io for undefined behaviour (an SSE4.1 intrinsic reached from the
SSE2-only backend, fixed in `0.10.2`), so the lock resolves the non-yanked
`0.10.2`.
