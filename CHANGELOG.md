...
- **Signed HTTP trust verification** - Flask, FastAPI, and Django trust middleware no longer accept a caller-supplied DID or capabilities as proof of identity or authorization. Requests must prove possession of a registry-resolved Ed25519 key over the server-selected request envelope, protected routes require an authenticated result, and exempt Django routes no longer expose an unverified DID ([#3813](https://github.com/microsoft/agent-governance-toolkit/pull/3813)).
- **Django replay-cache isolation** - the middleware resolves Django's cache backend for each request instead of sharing one potentially unpooled Memcached client across threads. Memcached nonce claims include a one-second expiry allowance so second-resolution expiration cannot reopen the replay window early.
