...
`guard_foundry_agent()` (alias `guard_azure_ai_agents()`) governs an Azure AI Foundry hosted agent. The Foundry seam is the manual run loop rather than a single client method, so the adapter returns a thin proxy over the real `AgentsClient` whose governed driver (`create_thread_and_run(...)` and `run_until_complete(thread_id, run_id)`) drives the `requires_action -> submit_tool_outputs` loop. Each required function tool call named in `tools` is routed through `control.run_tool` so `pre_tool_call` gates the arguments and `post_tool_call` gates the result before the output is submitted. A deny submits a policy rejection output and never executes the callable, an escalate is routed to the approval resolver and is never auto-allowed, and a transform submits the rewritten arguments or result. The SDK auto-function-call path bypasses governance, so `enable_auto_function_calls` and `runs.create_and_process` are blocked through the governed handle. `azure-ai-agents` is an optional dependency loaded lazily. Install it with `pip install azure-ai-agents`.
