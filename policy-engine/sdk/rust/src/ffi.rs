...
// Sourced from the engine rather than this crate's own deprecated aliases,
// which exist for downstream callers and warn at every use.
use crate::{HostEvaluation, InterceptionPoint};
use agent_control_spec::{
    AnnotatorDispatcher, AnnotatorInvocation, Decision, EnforcementMode, JsonValue, Limits,
    Manifest, PerfTelemetry, PolicyDispatcher, PreparedPolicyInvocation, Runtime, RuntimeError,
};
use serde_json::json;
#[cfg(feature = "opa")]
...
/// Stands in when the manifest declares no annotators. `dispatch` is
/// unreachable because the runtime only dispatches annotators a manifest
/// names; it fails closed rather than returning a value a policy might trust.
...
impl AnnotatorDispatcher for NoAnnotatorDispatcher {
    fn dispatch(
        ...
        Err(RuntimeError::PolicyInvocationFailed(format!(
            "annotator `{annotator_name}` was dispatched but no annotator dispatcher is registered"
        )))
    }
}
...
fn resolve_default_annotator_dispatcher(
    ...
    if !builder.enable_default_annotations {
        // A manifest that declares no annotators never reaches a dispatcher,
        // so it must not be forced to opt into the credential reading one.
        // Matches the fallback `AgentControl` uses on the Rust side.
        if _manifest.annotators.is_empty() {
            return Ok(Some(Arc::new(NoAnnotatorDispatcher)));
        }
        return Ok(None);
    }
...
fn resolve_default_policy_dispatcher(
    ...
    #[cfg(not(all(feature = "bundled-dispatchers", feature = "opa")))]
    {
        let _ = manifest;
...
/// `host_error:context_invalid`. A missing `mode` defaults to `enforce`. If
...
pub unsafe extern "C" fn acs_runtime_evaluate(
    ...
    ffi_guard!(ptr_with_err, err, {
        ...
        let intervention_point = match InterceptionPoint::from_str(intervention_point_str) {
            Ok(value) => value,
            Err(_) => {
                return json_to_c(&json!({
                    "verdict": agent_hooks::Verdict::host_error(
                        agent_hooks::HostError::ContextInvalid,
                        Some(format!(
                            "unknown interception point '{intervention_point_str}'"
                        )),
                    ),
        ...
        let mode = match object.get("mode") {
            ...
            // `EnforcementMode::from_str` is gone; the wire names are the
            // two serde variants and anything else fails closed.
            Some(JsonValue::String(mode_str)) => match mode_str.as_str() {
                "enforce" => EnforcementMode::Enforce,
                "evaluate_only" => EnforcementMode::EvaluateOnly,
                _ => return request_invalid_response(),
            },
        ...
        // Applying the transform, honouring evaluate_only, and deriving the
        // identity trio are host obligations now, so the engine result is
...
pub unsafe extern "C" fn acs_validate_artifacts(
    ...
    ffi_guard!(ptr_with_err, err, {
        ...
        if rego_modules_json.len() > 8_388_608 {
            let result = agent_control_specification_core::ArtifactValidationResult {
                valid: false,
                diagnostics: vec![agent_control_specification_core::ValidationDiagnostic {
        ...
        let result = agent_control_specification_core::validate_acs_artifacts(
            manifest_yaml,
            &modules,
            opa_path,
        );
...
unsafe fn validation_result_to_c(
    result: &agent_control_specification_core::ArtifactValidationResult,
    err: *mut *mut c_char,
) -> *mut c_char {
