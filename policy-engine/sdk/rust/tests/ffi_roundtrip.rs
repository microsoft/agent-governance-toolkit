...
use agent_control_specification::ffi::{
    ...
    acs_runtime_free, AcsBuilder, AcsPolicyCallback, AcsRuntime,
};
...
const MANIFEST_YAML: &str = r#"agent_control_specification_version: 0.4.0-alpha.1
metadata:
...
const BASE_CHAIN_YAML: &str = r#"agent_control_specification_version: 0.4.0-alpha.1
policies:
  input_custom_policy:
    type: custom
    adapter: basic_host_mock
...
const OVERLAY_CHAIN_YAML: &str = r#"agent_control_specification_version: 0.4.0-alpha.1
metadata:
  name: ffi-chain-test
intervention_points:
  input:
...
const PATH_CHILD_YAML: &str = r#"agent_control_specification_version: 0.4.0-alpha.1
extends:
  - base.yaml
metadata:
  name: ffi-path-test"#;

const UNRESOLVED_EXTENDS_YAML: &str = r#"agent_control_specification_version: 0.4.0-alpha.1
extends:
  - ./base.yaml
policies:
  input_custom_policy:
...
unsafe fn build_runtime() -> *mut AcsRuntime {
...
unsafe fn build_runtime_from_builder(builder: *mut AcsBuilder) -> *mut AcsRuntime {
    unsafe { build_runtime_from_builder_with_policy(builder, policy_callback) }
}

unsafe fn build_runtime_from_builder_with_policy(
    builder: *mut AcsBuilder,
    policy: AcsPolicyCallback,
) -> *mut AcsRuntime {
    let mut err = ptr::null_mut();
...
fn ffi_evaluate_rejects_unknown_intervention_point() {
    unsafe {
        ...
        assert_eq!(result["verdict"]["reason"], "host_error:context_invalid");
        assert!(result["verdict"]["message"]
            .as_str()
            .unwrap()
            .contains("not_a_point"));
...
fn ffi_builder_from_path_resolves_extends_and_builds_runtime() {
    unsafe {
        let directory = tempfile::tempdir().expect("create manifest fixture directory");
        fs::write(directory.path().join("base.yaml"), BASE_CHAIN_YAML)
            .expect("write base manifest");
        let child_path = directory.path().join("child.yaml");
        fs::write(&child_path, PATH_CHILD_YAML).expect("write child manifest");

        let path =
            CString::new(child_path.to_string_lossy().into_owned()).expect("path contains no NUL");
        let mut err = ptr::null_mut();
...
unsafe fn evaluate_raw_request(runtime: *mut AcsRuntime, request_json: &str) -> Value {
