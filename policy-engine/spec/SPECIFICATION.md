...
A loader MAY fetch the top level manifest itself from an HTTPS URL rather than the file system. A URL load takes the URL and a `sha256` pin and applies the same fetch path and trust gate as a URL `extends` entry defined in section 2.2, so the fetch is HTTPS only, carries no ambient credentials, and is bounded by the same body size limit. Unlike an `extends` entry the pin is mandatory, because the top level manifest is the root of trust and an unpinned remote root would let a network attacker swap the entire policy. The `sha256` MUST be a 64 character hexadecimal SHA-256 digest over the fetched bytes. A non HTTPS URL, a missing or malformed pin, a fetch error, a body size breach, or a hash mismatch MUST fail closed. After the fetch the manifest is parsed, its own `extends` are resolved by the same loader, and the merged manifest is validated as a whole. A reference under a URL sourced manifest resolves against the URL, so a relative `extends` entry becomes a sibling URL and a URL sourced manifest never reaches the local file system. Filesystem relative fields such as a rego `bundle`, an annotator `system_prompt_file`, or adapter `data` paths are not rebased for a URL sourced manifest, mirroring URL `extends`, so a URL sourced manifest SHOULD reference remote artefacts through the `bundle_url` and `system_prompt_url` forms.
