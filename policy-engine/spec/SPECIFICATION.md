...
ACS evaluates one intervention point of one agent at a time. The host assembles a complete JSON snapshot for that intervention point and calls the runtime. The runtime selects the value under evaluation, attaches host supplied annotations, calls a host supplied policy dispatcher, normalizes the result into a verdict, and validates effects. In enforce mode the runtime also produces a transformed policy target from policy target scoped effects.
...
**Policy target.** The value selected from the snapshot that a policy evaluates and that effects may transform. The optional `policy_target_kind` is a descriptive label recorded on it.
...
**Verdict.** The normalized decision the runtime returns, one of `allow`, `warn`, `deny`, or `escalate`, together with a reason.

**Effect.** A transformation the runtime applies to the policy target to produce the transformed policy target.
...
| effect `path` | `$policy_target` |

A manifest path that uses a root outside its allowed set MUST fail closed with `runtime_error:manifest_invalid`. An effect path outside `$policy_target` MUST fail closed with `runtime_error:effect_target_forbidden`. An annotation `from` path that reads `$pi.annotations` MUST fail closed with `runtime_error:manifest_invalid`, because annotator outputs do not exist when annotator inputs are resolved.
...
An evaluation request carries one mode. In `enforce` mode the runtime applies the effects of an `allow` or `warn` verdict and returns the transformed policy target. In `evaluate_only` mode the runtime runs the same pipeline and validates effects but applies none and returns no transformed policy target, which lets a host observe what the policy would decide without acting on it. The computed verdict, including `deny` and `escalate`, is the same in both modes. A runtime error fails closed to a `deny` verdict in both modes. Acting on a `deny` or an `escalate` verdict is the host obligation defined in section 17 and applies in `enforce` mode.
...
9. Validate effects per section 14. In `enforce` mode, apply the effects of an `allow` or `warn` verdict and return the transformed policy target.

Annotation collection completes before the policy dispatcher is called. Effect handling runs after the verdict is normalized.
...
A `policies` entry is a named, reusable policy definition. The schema defines three types.

A `rego` policy targets Open Policy Agent. Its `query` and `bundle` members are optional in the schema. Rego is the one type with a runtime execution path. The runtime offers an optional dispatcher that runs the `opa` executable when it is present, and the host MAY supply its own dispatcher instead.
...
| --- | --- | --- | --- |
| `decision` | yes | string | One of `allow`, `deny`, `warn`, `escalate`. |
| `reason` | no | string | MUST NOT start with `runtime_error:`. |
| `message` | no | string | Free form text for a caller. |
| `effects` | no | array | Policy target scoped effects per section 14. |
...
Normalization MUST fail closed with `runtime_error:policy_output_invalid` when the output is not an object, when `decision` is absent or is not one of the four values, when `reason` starts with the reserved `runtime_error:` prefix, when `reason`, `message`, or `effects` has the wrong JSON type, or when `result_labels` is present and is not an array of strings.

### 13.1 Decisions

`allow` permits the action and applies effects in enforce mode. `warn` permits the action, records a warning, and applies effects in enforce mode. `deny` refuses the action and applies no effects. `escalate` defers the action to the host approval path and applies no effects. Only `allow` and `warn` apply effects.

The runtime derives an action identity for each successful evaluation from the SHA-256 digest of the canonical policy input JSON. The digest is encoded as `sha256:` followed by lowercase hexadecimal bytes. This identity covers the intervention point, policy target, full snapshot, annotations, and projected tool data that the policy evaluated.
...
| --- | --- | --- |
| `replace` | `value` | Set the target to `value`. |
| `append` | `value` | Append `value` to a string or array target. |
| `prepend` | `value` | Prepend `value` to a string or array target. |
| `redact` | One of `pattern`, `values`, or `spans` | Replace portions of a string target. |

An `append` or `prepend` on a string target requires a string `value`. On an array target it inserts `value` at the end or the start. Applied to any other type it MUST fail closed with `runtime_error:effect_invalid`.

A `redact` effect MUST provide exactly one of `pattern`, `values`, or `spans`. The `pattern` form provides a string regular expression and an optional string `replacement`. The regular expression uses the Rust `regex` crate syntax. The runtime compiles the expression deterministically, finds all non empty matches in the string target including overlapping matches, converts match byte offsets to character offsets, sorts the spans, merges overlapping or adjacent spans, and replaces each merged span with `replacement` or `[REDACTED]` when `replacement` is absent. A regular expression that fails to compile MUST fail closed with `runtime_error:effect_invalid`.
...
| `runtime_error:effect_invalid` | An effect was malformed or did not apply to its target. |
| `runtime_error:effect_target_forbidden` | An effect path pointed outside `$policy_target`. |
...
A conformant runtime MUST evaluate in the order defined in section 6, build the policy input defined in section 7, serialize it canonically as defined in section 8, normalize results into the verdicts defined in section 13, validate and apply effects as defined in section 14, enforce the resource limits in section 15, and report failures using only the reserved reasons in section 16. It MUST be stateless and deterministic as defined in section 1.1 and MUST fail closed on every error.
...
Approvals bind to action identity. An `escalate` verdict carries the identity of the action under control, and the SDK rederives that identity before proceeding and fails closed on a mismatch. This prevents an approval granted for one action from authorizing a different action and closes a time of check to time of use gap.
