...
redact_ssn := warn_effect("redact_ssn_in_response", "SSN detected in agent output.", "[SSN-REDACTED]", m[0]) if {
	m := regex.find_n(`\b\d{3}-\d{2}-\d{4}\b`, output_text, 1)
	count(m) > 0
}
redact_card := warn_effect("redact_card_in_response", "Card number detected in agent output.", "[CARD-REDACTED]", m[0]) if {
