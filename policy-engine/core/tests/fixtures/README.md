...
- `extends/` — path-loader fixtures for ordered, relative, duplicate-identical, conflict, cycle, and missing-file `extends` behavior.
...
- `extends` is an ordered array resolved by file-based loaders before final manifest validation, policies are declared at top level, annotators are top-level `classifier | llm | endpoint`, and annotations are configured per intervention point.
