...
export const USER_POLICY_ENV = "AGT_ANTIGRAVITY_POLICY_PATH";
export const AUDIT_PATH_ENV = "AGT_ANTIGRAVITY_AUDIT_PATH";

const USER_POLICY_RELATIVE_PATH = [".antigravity", "agt", "policy.json"];
const USER_AUDIT_RELATIVE_PATH = [".antigravity", "agt", "audit-log.json"];
const DEFAULT_AGENT_ID = "antigravity-cli";
...
export async function loadPolicy({
  ...
  const antigravityHome = resolveRuntimeAntigravityHome(homeDirectory, env);
  const configuredPolicyPath = policyPath
    ? resolve(String(policyPath))
    : join(antigravityHome, "agt", "policy.json");
  const auditPath = resolve(
    String(env[AUDIT_PATH_ENV] ?? join(antigravityHome, "agt", "audit-log.json")),
  );
    ...
    if (decision.effectiveDecision === "review") {
      return {
        ...
        permissionDecisionReason:
          ...
          "Antigravity CLI hooks cannot pause for manual approval, so AGT denied the tool instead.",
      };
    }
...
function resolveRuntimeAntigravityHome(homeDirectory, env) {
