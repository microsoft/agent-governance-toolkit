...
- Credential boundary functions (`is_left_boundary_char` / `is_right_boundary_char` in `redactor.rs`) must only reject ASCII alphanumerics. `_` and `-` must not block detection — a secret annotated `_old` or preceded by `session_` must still be caught.
