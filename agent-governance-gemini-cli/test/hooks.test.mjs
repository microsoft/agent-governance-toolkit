...
test("before-tool fails closed with a Gemini system block when hook bootstrap breaks", async () => {
  const root = await mkdtemp(join(tmpdir(), "agt-gemini-hook-block-"));
  const geminiHome = join(root, ".gemini");

  await installPackage({ geminiHome, packageRoot: PACKAGE_ROOT });
  const extensionRoot = join(geminiHome, "extensions", "agt-global-policy");
  await rm(
    join(
      extensionRoot,
      "vendor",
...
test("audit history survives reloads of the installed extension runtime", async () => {
  const root = await mkdtemp(join(tmpdir(), "agt-gemini-audit-history-"));
  const geminiHome = join(root, ".gemini");
  ...
  process.env[policyModule.USER_POLICY_ENV] = join(geminiHome, "agt", "policy.json");
  process.env[policyModule.AUDIT_PATH_ENV] = join(geminiHome, "agt", "audit-log.json");
  ...
  try {
    ...
    assert.equal(JSON.parse(await readFile(join(geminiHome, "agt", "audit-log.json"), "utf8")).length, 2);
  } finally {
