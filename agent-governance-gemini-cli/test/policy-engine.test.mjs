...
test("default packaged policy keeps the hardened Gemini developer-protection baseline", async () => {
...
test("checkArbitraryText does not inherit prior detector state from the runtime", () => {
  ...
  const state = {
    ...
    auditPath: join(tmpdir(), "agt-gemini-policy-engine-audit.json"),
  ...
  const result = checkArbitraryText(state, "Summarize the Gemini governance files.");
  assert.equal(result.promptPoisoning.suspicious, false);
});

test("extractCommandText prefers command fields from Gemini shell invocations", () => {
  assert.equal(
...
test("evaluatePreToolUse denies review-only tools because Gemini hooks cannot pause for approval", async () => {
  const state = {
    auditLogger: {
      length: 0,
      log() {
    ...
    auditPath: join(tmpdir(), "agt-gemini-policy-engine-review-audit.json"),
    bundledDefaultError: undefined,
    configuredPolicyError: undefined,
    policy: compilePolicy({
      blockedToolCalls: [],
...
test("formatPolicySummary reports Gemini runtime context", () => {
